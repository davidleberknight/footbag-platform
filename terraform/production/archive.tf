# =============================================================================
# archive.footbag.org — the members-only static mirror of the legacy site
#
# A second, self-contained CloudFront stack: its own bucket, its own certificate,
# its own distribution and its own DNS records. Nothing here is gated on
# enable_cloudfront or on the apex alias flag, and nothing in the main stack
# depends on anything here. That independence is the point: archive.footbag.org
# is a name that has never resolved, so it can be stood up, proved and loaded
# long before the apex and www move, and the parts that can only be exercised
# against a real domain (certificate issuance, DNS, the parent-domain cookie)
# stop being first attempts made during the cutover.
#
...
# distribution itself. enable_archive_custom_domain adds the pieces that need the
# real domain: the certificate, the distribution aliases and the DNS records.
# The split lets the whole credential-and-refusal machinery be proved in an
# environment with no domain at all, and in production before the zone move.
# =============================================================================

variable "enable_archive" {
  description = "Create the archive stack: bucket, key group, signing-key parameter, edge function, gate pages, distribution and access logging, served on the distribution's own cloudfront.net name until enable_archive_custom_domain adds the real one. Independent of enable_cloudfront and of the apex alias flag, so the archive can go live ahead of the main cutover. Requires archive_signing_public_key when true."
  type        = bool
  default     = false
}
...
resource "aws_s3_bucket" "archive" {
  ...
  # time rather than silently deleting the mirror; the plan fails whole, so
  # nothing else is touched either. Retiring the archive for real is a deliberate
  # sequence: empty the bucket, remove prevent_destroy, and flip the flag off in
  # that same change.
...
resource "aws_cloudfront_public_key" "archive" {
  ...
  comment     = "Public half of the keypair signing archive.footbag.org cookies"
...
resource "aws_cloudfront_key_group" "archive" {
  ...
  comment = "Trusted signers for archive.footbag.org"
...
# keeps the address out of every committed file and out of operator hands: no
# hand-edited host env line to forget or mistype. Absent when the archive is
# off, and the deploy then leaves the variable unset, which hides the card
...
# way as archive_url; absent means the direct-link default, which is this
# tree's permanent state while the edge gate stays on.
...
# enable_archive_custom_domain, not on enable_archive.
#
...
# ── DNS ──────────────────────────────────────────────────────────────────────
# Gated on the custom-domain flag, deliberately not on enable_apex_alias_records.
# The apex and www records wait for the cutover; this name is new, points at
# nothing today, and is what lets the archive be proved and loaded months ahead
# of it.
