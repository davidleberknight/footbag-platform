...
resource "aws_cloudfront_function" "strip_media_prefix" {
  ...
  # use create-before-destroy: Terraform creates the new function (different
  # AWS-side `name` is required, which is true when the rename is the cause
  # of replacement), updates the distribution to reference the new ARN, then
  # destroys the old function once nothing uses it.
  lifecycle {
