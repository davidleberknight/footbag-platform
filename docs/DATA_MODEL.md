...
- **`admin.club_cleanup.*`**: enumerated rather than left open, so the inventory can be checked rather than merely read. Club-level resolutions `demote_inactive`, `archive`, `dismiss`, `park`; candidate resolutions `candidate_park`, `candidate_dismiss`, `candidate_demote`, `candidate_archive`, `candidate_confirm_junk`, `candidate_promote_dormant`; plus `contact_members` (a leader-recruitment mailing to a club's members) and `delist_residue` (unconfirmed legacy affiliations moved to former-only). The club and candidate action names are interpolated into the value at the call site, so they are not literals the convention gate can see; this list is what a reader and a reviewer check against.
...
- **`email.*`**: `sns_subscription_pending` (a mail-feedback notification-topic subscription awaiting out-of-band operator confirmation).
