...
- Legacy tier-state evidence; derived fields recording the account's standing at cutover, read by the claim-time tier grant (USER_STORIES `M_Claim_Legacy_Account`): `legacy_ever_paid_tier2`, `legacy_ever_paid_tier1_lifetime`, and `legacy_tier1_annual_active_at_cutover` (INTEGER 0/1, paid history), populated by the loader. The Tier 3 board evidence — a board-at-cutover flag and the pre-board paid tier supplying the Tier 3 underlying derivation (`'none'` / `'tier1'` / `'tier2'`, NULL for non-board) — lands with the board derivation; the two are set together, the underlying tier non-NULL exactly when the board flag is set.
...
>>>>>>> e738aa1a (fix(freestyle): decide trick media from one view so browse and detail agree)
