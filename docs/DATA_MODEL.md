...
- **`alarm.*`**: `raised`, `cleared`, `acknowledged` (platform alarm state changes ingested from the notification webhook, and an admin's acknowledgment via `A_Acknowledge_Alarm`), `subscription_pending` (a notification-topic subscription awaiting out-of-band operator confirmation).
