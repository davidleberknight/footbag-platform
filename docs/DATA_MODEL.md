...
- **`freestyle.*`**: `trick.updated`; `trick_alias.created`, `trick_alias.updated`, `trick_alias.deleted`; `trick_modifier_link.created`, `trick_modifier_link.deleted`; `trick_source.created`, `trick_source_link.created`, `trick_source_link.deleted`; `record.created`, `record.updated`; `consecutive_record.created`, `consecutive_record.updated`, `consecutive_record.deleted`; `trick_tip.edited`, `trick_tip.hidden`, `trick_tip.restored`, `trick_tip.remapped`, `trick_tip.reordered`; `adjudication_notation.authored` (curation of the freestyle corpus, moderation of member-submitted tips, and authoring a movement notation onto an Emerging Vocabulary ruling, per A_Edit_Freestyle_Trick and its siblings).
