...
- **`email.*`**: `sns_subscription_pending` (a mail-feedback notification-topic subscription awaiting out-of-band operator confirmation), `bounce_recorded` and `complaint_recorded` (a bounce or complaint the mail provider reported against a member's address).
