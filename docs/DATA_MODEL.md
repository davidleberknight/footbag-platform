...
Freeform tags are unrestricted beyond security input validation (HTML stripping, Unicode normalization, max 100 characters). An administrator can retire an abusive one: `retired_at` and `retired_by_member_id` record the act, the tag is detached from every media item and gallery criterion that named it, and it can never be applied again.
...
**Standard-tag permanence:** Standard tags are permanent identities and must not be hard-deleted. The global unique index enforces extant-row uniqueness, but permanent reservation of standard-tag normalized forms across time is workflow/application-enforced (see APP-024). No tag row is ever deleted. `retired_at` and `retired_by_member_id` mark a freeform tag an administrator has taken out of circulation, and the row survives precisely so its normalized form stays reserved and cannot be recreated under a new id.
...
`discipline_category` carries the top-level family and is constrained to `net`, `freestyle`, `golf`, and `sideline` (legacy `other` values normalize to `sideline`). Sub-discipline structure is the discipline's game, held in its own column against a seeded vocabulary per category: 2-Square, 4-Square, 5-Square and Timed Consecutives under sideline; Routines, 30 Second Shred, Sick 3, Sick Trick, Circle, Battle, Last Man Standing and Request under freestyle; Footbag Net and Footbag Golf under their own categories. Untimed consecutive-kick attempts are world records rather than a contested discipline, so the vocabulary carries the timed form. Historical disciplines keep their original names as text and carry the structured parts where those can be inferred confidently. IFPA ratifies the vocabulary through the published competition rules, and it is seeded reference data that changes without a software release.

`team_type` encodes the entry size for each discipline: `'singles'` (default) or `'doubles'`. Gender eligibility is a separate column carrying `'open'`, `'women'`, or `'mixed'`, where `'mixed'` pairs with `'doubles'`; a mixed doubles discipline is that pairing. Used at registration time to enforce partner requirements: doubles requires partner info; a `'women'` discipline requires every member on the entry to have `members.gender` set to `'female'`; a `'mixed'` discipline requires the two member partners to hold `'male'` and `'female'`; an `'undisclosed'` value is ineligible for either and eligible for `'open'`, which admits anyone. The discipline's class column carries `'novice'`, `'intermediate'`, `'open'`, or `'masters'` and records the class the competitor entered, because class is self-selection in this sport.
...
`outbox_emails.provider_message_id` records the identifier the mail provider returned for the send. It is written on the successful-send path only, in the same statement that stamps the row sent, so a row that never sent and a row parked for manual review both carry none. It is what joins a later bounce or complaint to the message that caused it, and it makes no claim about retry safety: the provider send carries no idempotency token, which is why an unknowable outcome parks for an administrator rather than retrying.
...
#### Work queue
Admin task queue with `queue_category` and `task_type`. Active `task_type` values include `member_contact_request` (member-submitted IFPA-admin contact requests under `queue_category='membership'`; see `M_Contact_IFPA_Admin` and `A_Resolve_Contact_IFPA_Admin_Request`), `media_flag_review` (an item members have reported, under `queue_category='media'`; the card points at the takedown queue that owns the decision, and closes with it; see `M_Flag_Media` and `A_Moderate_Media`), `needs_organizer` (an event whose last organizer deleted their account, under `queue_category='events'`; the card points at the organizer-reassignment screen that owns the fix), and under `queue_category='payments'` the money events an administrator must see: `charge_dispute_review`, `partial_refund_review`, `unattributed_refund`, `payout_failed`, `membership_overcharge_review`, `recurring_donation_charge_declined`, `recurring_donation_paused`, `refund_failed_review`, and `reconciliation_discrepancy`. Each task type is declared once in the application's task-type table, which carries its label, its category, the entity types it may point at, its urgency, how its evidence renders, and the actions an administrator may take on it; the enqueue path refuses a type with no declaration, so no row can exist that the queue has no way to close. Admin notifications are routed by urgency (US §1 Global Behaviors): a task type classified urgent (a security, data-integrity, or administrative-continuity event needing same-day action) emails the admin-alerts mailing list immediately on enqueue; a routine task type surfaces on the work-queue dashboard and in a periodic per-administrator digest of the open routine items. An administrator claims an item (`claimed_by_member_id`, `claimed_at`) to drop it from the other administrators' digests; an item left open and unclaimed past the stale threshold escalates once with a single email to admin-alerts. Every such notification contains task type and entity ID only (no sensitive data). For `member_contact_request` rows the full member message is held in the purgeable `detail_text` column (with a short `reason_text` summary preview); account erasure and the deceased contact scrub redact both, so member free text stays off the append-only audit ledger.

`status` carries `open`, `resolved`, and `dismissed`. An item an administrator acted on is `resolved`; one they read and ruled settled without acting is `dismissed`, which is what the internal-review close path writes. A fourth parked value was considered for an item waiting on a member's answer and rejected: eight statements select `status = 'open'`, among them the de-duplication probe that stops a second item being raised for the same member, and both close paths. A parked item would therefore admit a duplicate and then refuse to be closed. An item waiting on a member stays `open` and carries the wait on the message row instead. An administrator setting an item aside records that beside the status too, in `parked_at`, `parked_by_member_id` and `park_reason`, for the same reason a parked status was rejected. A parked item leaves the working list, the digests and the escalation sweep by those columns rather than by its status, and returns when the member answers a question on it or an administrator clears the park.

#### Member messages
`member_messages` carries an administrator's direct question to one member and that member's structured answer. Every message hangs off the work-queue item that raised the question, so the queue row stays the single record of the matter and the answer returns to the administrator where they were already looking; the originating `work_queue_items` id is therefore required, not optional, and there is no free-standing composer. One row per message: recipient member, sending administrator, originating work-queue item, subject, the message body, the expected answer kind (acknowledge, or confirm and correct a date of birth), status, the structured outcome, the member's optional note, and the sent and answered timestamps. The item's own status is untouched by a question: it stays `open` while it waits, and an administrator still decides when it closes.

The subject, the body and the member's note are owner-and-admin-private content held in purgeable columns: all three are cleared by account erasure and by the deceased contact scrub, each appending an `erasure_log` row, matching `birth_date` handling. The subject is included because an administrator writes it about a member, the same reason the outbound-mail scrub replaces a stored subject. The audit ledger records that a message was sent and answered, carrying the expected answer kind and the body length but never the body, so free text stays off the append-only ledger. Message content never travels by email: the member receives a content-free nudge to their verified login address and reads the message in the application, which is what lets the channel carry a date of birth at all.
...
- **`payment.*`**: `checkout_started`, `donation_checkout_started`, `succeeded`, `refunded`, `canceled`, `partially_refunded`, `attempt_declined` (a card the bank refused; deliberately not named `*_failed`, which is reserved for platform faults that raise the operator alarm), `payout_rejected`, `queue_item_resolved`, `receipt_sent` (the platform sent a member their receipt; the per-recipient outbound copy is deleted once it ages past the outbound-copy retention window, so this is the durable record that a gift was acknowledged), `refund_not_returned` (a refund the provider could not return to the card, or reversed; named like `payout_rejected` rather than `*_failed`, which is reserved for platform faults that raise the operator alarm; the payment row keeps its status and the case is put in front of an administrator), `compliance_anonymize_failed`, `reconciliation_issue_raised`, `reconciliation_issue_resolved`, `recurring_donation_activated`, `recurring_donation_updated`, `recurring_donation_canceled`, `recurring_cancel_requested`, `recurring_charge_succeeded`, `recurring_charge_declined`, `recurring_charge_amount_updated`, `recurring_charge_zero_amount` (a renewal invoice that collected nothing, acknowledged and not booked), `recurring_donation_checkout_abandoned` (a recurring-gift checkout the member started and never completed), `dispute_opened`, `dispute_updated`, `dispute_closed`, `dispute_funds_withdrawn` and `dispute_funds_reinstated` (the card-dispute stages the payment provider reports, the last being the money returning when a dispute is won).
- **`active_player.*`**: `grant`, `extend` (a grant that lengthens an existing period rather than opening a new one), `expire`, `end`, `vouch_noop`, `club_join_noop`, `attendance_noop`, `admin_correction` (an administrator correcting an expiry from the member record, the one write allowed to move an expiry earlier, which is how a standing granted in error is taken back).
...
- **`roster.*`**: `list`, `summary`.
- **`member.*`**: `profile_updated`, `search`, `search_rate_limited`, `pii_purged`, `deceased_pii_scrubbed`, `pii_erasure_failed`, `name_corrected` (an administrator correcting a member's recorded legal name and display name from the member record; the metadata carries each changed name before and after, because a correction made on someone else's behalf is reviewable and reversible only from a trail that says what the name actually was, and the surface that reads it is admin-only), `slug_corrected` (an administrator moving a member's profile URL, recorded with the old and the new value; the metadata also records whether the member's uploader tag moved with them and how many media rows followed it, because that is what keeps their uploads and galleries resolving), `deceased_marked` and `deceased_reverted` (an administrator recording that a member has died, and removing a marking made in error inside the grace period; the metadata records whether a linked historical record followed and how many future event registrations were withdrawn, and the same pair carries the affirmative-only flag on an unlinked historical record, where `entity_type` is `historical_person`), `data_exported` (a member downloading a copy of their own data; the metadata records how many sections the file carried and whether any was truncated, never the contents), `profile_corrected` (an administrator correcting a member's profile on their behalf, carrying the mandatory reason and each changed field's value before and after; it is distinct from `profile_updated`, which is the member editing their own record and records field names alone, because the exception that records values is for a correction made on somebody else's behalf).
- **`admin.*`**: `role_granted`, `role_revoked` (steady-state admin-role grant and revoke via A_Manage_Admin_Role), `bootstrap_grant` (production single-shot SSM-token first-admin), `dev_register_allowlist_grant` (dev/staging register-allowlist admin bootstrap), `loss_alert_raised` and `loss_alert_dismissed` (the administrator-loss recruitment alert raised and closed via SYS_Detect_Admin_Loss), `loss_alert_failed` (a raise the daily sweep could not complete), `bootstrap_token_delete_failed` (the first-admin grant committed but its single-shot token could not be deleted, so an operator must remove the parameter by hand).
- **`admin.club_cleanup.*`**: enumerated rather than left open, so the inventory can be checked rather than merely read. Club-level resolutions `demote_inactive`, `archive`, `dismiss`, `park`, `promote`; candidate resolutions `candidate_park`, `candidate_dismiss`, `candidate_demote`, `candidate_archive`, `candidate_confirm_junk`, `candidate_promote_dormant`; plus `contact_members` (a leader-recruitment mailing to a club's members) and `delist_residue` (unconfirmed legacy affiliations moved to former-only). The club and candidate action names are interpolated into the value at the call site, so they are not literals the convention gate can see; this list is what a reader and a reviewer check against.
- **`audit.*`**: `viewed`, `exported` (an admin reading or exporting the audit log, recorded so access to the ledger is itself accountable; excluded from the default browse to avoid self-noise).
- **`config.*`**: `updated` (an administrator changing a runtime-tunable value from the System Parameters screen, under the `system` category, with the old and the new value and the reason given in the metadata), `price_scheduled` (a membership price added to a product's schedule, under the `pricing` category as the pricing section of this document requires, with the old and the new amount in cents and the date the new price starts).
- **`email.*`**: `sns_subscription_pending` (a mail-feedback notification-topic subscription awaiting out-of-band operator confirmation), `bounce_recorded` and `complaint_recorded` (a bounce or complaint the mail provider reported against a member's address), `outbox_retention_cleanup_failed` (the daily pass could not delete the per-recipient outbound copies that had aged past their retention window, so recipient addresses are still held), `unsubscribed_one_click` (a member withdrew one mailing-list subscription through the one-click unsubscribe header on a bulk message, acting from their mail client without a session), `dead_letter_reviewed` (an administrator looked at a message the sender gave up on, one in a terminal failure state that nothing else in the platform can move, and recorded with a required note that it is settled; it asserts nothing about delivery and the message keeps its status).
- **`mailing_list.*`**: `created`, `updated` and `archived` (an administrator creating, editing, or archiving a mailing list; archiving preserves every subscription and every past send and only stops the list being offered), `subscription_adjusted` (the exceptional change an administrator makes to one member's subscription on that member's behalf, most often releasing an address the mail provider marked bounced; carries the mandatory reason and names the member by id), `broadcast_sent` (a composed message handed to the send path for a list, recording the subject, the archive row it was written to, and how many copies were queued and suppressed; the message itself lives on the archive row, not in the ledger), `subscribed` and `unsubscribed` (a member's own choice on their email-preferences screen, recorded only when a row actually moves; the one-click header withdrawal is recorded as `email.unsubscribed_one_click` instead, because the two are made through different authorities).
- **`email_template.*`**: `updated` (an admin editing a registered outbound template through the template editor).
- **`event.*`**: `organizer_assigned` and `organizer_removed` (an administrator putting a member in charge of an event or taking them off it, each carrying the mandatory reason and recording which member and which role; the pair exists because an event whose only organizer leaves is otherwise silently unrunnable, and the assignment is also what closes the queue item that said so).
- **`freestyle.*`**: `trick.updated`, `trick.published`; `trick_alias.created`, `trick_alias.updated`, `trick_alias.deleted`; `trick_modifier_link.created`, `trick_modifier_link.deleted`; `trick_source.created`, `trick_source_link.created`, `trick_source_link.deleted`; `record.created`, `record.updated`; `consecutive_record.created`, `consecutive_record.updated`, `consecutive_record.deleted`; `trick_tip.edited`, `trick_tip.hidden`, `trick_tip.restored`, `trick_tip.remapped`, `trick_tip.reordered`; `adjudication_notation.authored`, `adjudication.reconciled` (curation of the freestyle corpus, moderation of member-submitted tips, authoring a movement notation onto an Emerging Vocabulary ruling, creating a canonical trick from that authored draft, and recording that a ruling became a trick that already existed, per A_Edit_Freestyle_Trick and its siblings).
- **`net_qc.*`**: `candidate_approved`, `candidate_rejected` (the internal net-results quality-control tooling accepting or refusing a candidate row).
- **`alarm.*`**: `raised`, `cleared`, `acknowledged` (platform alarm state changes ingested from the notification feed, and an admin's acknowledgment via `A_Acknowledge_Alarm`), `subscription_pending` (a notification-topic subscription awaiting out-of-band operator confirmation).
- **`media.*`**: `member_uploaded`, `curated_uploaded`, `member_edited`, `curated_edited`, `member_deleted`, `curated_deleted`, `curated_url_reference_added`, `curated_url_reference_edited`, `curated_url_reference_deleted`, `member_gallery_created`, `curated_gallery_created`, `member_gallery_updated`, `curated_gallery_updated`, `member_gallery_deleted`, `curated_gallery_deleted`, `flagged`, `deleted`, `flag_resolved`, `flag_cleared`, `member_gallery_moderated` (an administrator acting on a member's own named gallery: clearing an abusive name or description, or setting its ordering and tag sets; it carries the mandatory reason and each changed value before and after, and is distinct from `member_gallery_updated`, which is the owner editing their own gallery, because the surface that reads the ledger is looking for what was done on somebody else's behalf), `tag_retired` (an administrator taking an abusive freeform hashtag out of circulation, which detaches it from every media item and gallery criterion that named it; the metadata records the tag as stored and as displayed and how many rows were detached). A takedown is one action however it was reached, so `deleted` covers both a decided report and an administrator removing an item from a member's record with no report in front of them; its metadata carries `flagsResolved`, which is zero in the second case, and `isAvatar`, because a profile picture and a gallery photo are both photos and the ledger has to tell them apart.
- **`testkit.*`**: `persona_seed`, `persona_switch`, `persona_login`, `persona_refresh` (dev/staging persona-harness audit markers).

Reserved names for designed surfaces that do not emit yet (account lifecycle, voting and recognition) follow the same convention when they land: `event.needs_organizer_raised`, `event.needs_organizer_resolved`, `vote.cast`, `vote.eligibility_snapshot_taken`, `vote.ballot_tallied`, `hof.nomination_submitted`, `hof.affidavit_submitted`.
...
#### Alarms
`system_alarm_events` tracks infrastructure and operational alarms. `acknowledgment_note` is set alongside `acknowledged_at` when an admin acknowledges an alarm. `sns_alarm_events` claims each inbound alarm-notification message identifier so a redelivery is processed exactly once, parallel to `ses_events` for SES feedback and `stripe_events` for payment webhooks.
...
`recurring_donation_subscriptions.provider_livemode` mirrors the payments column: set from the provider's own flag when `customer.subscription.created` confirms the subscription, null on the opening row and on rows written before the column existed. The nightly reconciliation compares only rows whose recorded mode matches the loaded credential's, since a live key cannot list test-mode objects; rows of the other mode are set aside and counted on the run, and rows whose mode was never recorded are compared.
...
`bap_inducted_year` (`INTEGER`, nullable) is its Big Add Posse counterpart, for members with the permanent `is_bap` badge. Both are written by the administrator honour grant alongside the badge, and both are cleared with their badge when a grant made in error is taken back. The Big Add Posse year needs a column of its own because the archival induction year lives on `historical_persons`, which a member inducted today need not be linked to.
...
`family_name` and `given_names` (both `TEXT`, nullable) record the member's legal name as two parts, and `real_name` holds them assembled with the given names first. The family name is the private matching anchor every claim path gates on, and deriving it by taking the last word of one string is wrong for a member with two surnames, a name particle, or a family name written first, so it is recorded rather than inferred. `family_name` is required at registration and `given_names` is not: the family name is the anchor every claim path matches on, so it is the part that must always be there. A member whose legal name is a single word, which is ordinary in much of the world, records that one name as their family name and leaves the given names empty; requiring both parts is what would refuse those members. There is deliberately no `CHECK` enforcing either column, because the PII purge clears both and a table-level rule would then reject a purged row. `real_name` is kept because the whole-name matcher and the curated `name_variants` and `given_name_variants` tables are keyed on whole names, and because `legacy_members` and `historical_persons` carry a single name string and are never split; the surname on that side of a comparison is still derived.
...
**Deletion timestamps.** `deleted_at` and `deleted_by` are set by the member's own deletion request, alongside `deletion_requested_at` and `deletion_grace_expires_at`. The first two are what every member-facing view filters on; the second two are for the administrator's record, which displays when the request was made and when the grace period ends. The stored expiry is a snapshot of what the member was told, not the deadline the platform acts on: the purge scan recomputes its own cutoff from `deleted_at` and the current `member_cleanup_grace_days`, so an administrator changing that parameter moves the real deadline and the stored value goes stale by design. All four are cleared by nothing; the platform offers no restore, and the row proceeds to the anonymising purge.
...
- Legacy tier-state evidence; derived fields recording the account's standing at cutover, read by the claim-time tier grant (USER_STORIES `M_Claim_Legacy_Account`): `legacy_ever_paid_tier2`, `legacy_ever_paid_tier1_lifetime`, and `legacy_tier1_annual_active_at_cutover` (INTEGER 0/1, paid history), populated by the loader. Board / Tier 3 standing carries no legacy tier-state evidence: it is an administrator-set flag on the live member row, granted through the admin HoF / BAP / Board status flow.
...
A leadership row outlives the member's account, so that historical leadership stays attributable. The table therefore answers who has ever led a club, and the `club_leaders_current` view answers who leads it now: rows whose member is neither soft-deleted nor deceased. Every read meaning current leadership selects from the view, including the needs-leader predicate, the public club page's leadership block, and the co-leader count the cap is enforced against. Reads that must find a row in order to delete it use the table.
...
Both tables use `ON DELETE CASCADE` on `media_id`: when media is hard-deleted, its flags and tags are automatically removed. Nothing cascades the other way: `tag_id` carries no cascade, so retiring an abusive tag deletes its `media_tags` rows explicitly, alongside the gallery criteria and exclude rows that named it.

`UNIQUE(media_id, reporter_member_id)` prevents duplicate flags from the same reporter on the same item.

`reason_code` is a closed set (`illegal_or_harassing`, `infringes_rights`, `impersonation`, `false_information`, `spam`, `other`), each value a conduct clause of the published terms of use. `reason_text` holds the reporter's optional description, required only on `other`; it is member free text, so account erasure and the deceased scrub clear it while the report row survives.
...
This is recomputable data; the application owns recomputation cadence and may rebuild from source tables at any time. A background job upserts stats rows. `distinct_member_count` drives the "community tag" threshold: tags used by at least 2 distinct members appear in the All Tags index on the public `/media/browse` landing. Popular-tag discovery on the media browse surface treats a tag as public when it is a community tag or appears on curator/system-published content (so the single-uploader curated catalog surfaces), ranked by usage; a single ordinary member's personal tags stay excluded.
...
| `reconciliation_summary_interval_days` | `30` | Cadence (days) for the reconciliation digest, sent to the IFPA treasurer contact address |
...
- `is_primary`: designates the member's primary club among current affiliations. The first current club is primary (`is_primary = 1`); the second is secondary (`is_primary = 0`). At most one primary per member, enforced by `ux_member_club_affiliations_one_primary` (partial unique index on `member_id WHERE is_primary = 1 AND is_current = 1`). When leaving the primary club leaves exactly one current affiliation, that affiliation is promoted to primary in the same transaction, so a member holding any current affiliation always holds exactly one primary. A member holding two changes which is primary through the explicit swap, never automatically.
...
- **Completion grants membership**: onboarding is complete when all three rows are `completed`, and that transition is what grants member authorization. While any row is `pending` the account is pending: every member capability routes the registrant to the lowest-position outstanding task, which is also the only task they may answer, and the account has no profile page and does not appear in member search.
...
| `official_ifpa_roster_current` | `members_active` + `member_membership_status_current` | Operational Official IFPA Roster surface. Filters `is_official_roster_member = 1 AND is_deceased = 0`. Not public; Tier 2 and above, gated at the route, and served on screen only. |
| `club_leaders_current` | `club_leaders` + `members` | Leadership a club can rely on: rows whose member is neither soft-deleted nor deceased. The table keeps every row, so history stays attributable; this view is what every "who leads it now" read selects from. |
...
```
→ incomplete (the row written when checkout opens)
incomplete → active | past_due (on customer.subscription.created, taking the status the provider reports)
incomplete → canceled (on checkout.session.expired)
active → past_due (on invoice.payment_failed)
past_due → active (on invoice.paid or invoice.payment_succeeded after failure; the provider raises both for one settlement, and the first to arrive books it)
...
**Update membership pricing by scheduling a new price through `adminSystemParametersService`**, which inserts a new row into `system_config` with the chosen `effective_start_at` and the acting administrator's `changed_by_member_id`, and appends the matching `audit_entries` row in the same transaction. Pricing keys are `tier1_price_cents` and `tier2_price_cents`. Values are integer cents. `system_config` is append-only; never UPDATE existing rows. Verify seeded rows by `config_key` before making changes.
...
The application must reject any delete request targeting a `tags` row where `is_standard = 1`; the retirement statement carries that refusal in its own `WHERE` clause rather than trusting a caller to check. No `tags` row is deleted at all, whatever its kind: an abusive freeform tag is retired in place, and the surviving row is what holds its normalized form against recreation. Permanent reservation of standard-tag normalized forms (and therefore their redirect and identity semantics) rests on that, and on the application-layer delete discipline that keeps it true.
...
- **`ses_events`**; SES bounce/complaint feed idempotency store, parallel to `stripe_events`. Uses `message_id TEXT PRIMARY KEY` (the inbound SNS MessageId) rather than a surrogate UUID; the feedback handler claims it (INSERT OR IGNORE) inside the feedback transaction so a redelivered notification is processed exactly once. `recipient_count` carries how many addresses that one notification named, because the provider reports per notification and not per address; the admin health view sums it rather than counting rows, which would undercount a multi-recipient bounce. `mail_message_id` carries the provider's identifier for the original message, read from the notification's mail envelope and distinct from the `message_id` primary key, which identifies the report rather than the mail. `outbox_email_id` is the outbox row that identifier resolved to, cleared rather than blocking when retention removes the per-recipient copy, so the notification record outlives the message it is about.
