...
**Adapter Pattern**: Design pattern abstracting external dependencies behind interfaces. Footbag.org uses adapters to isolate infrastructure concerns per DD §1.9: `MediaStorageAdapter` (S3 in staging/production; `LocalMediaStorageAdapter` in dev), `SesAdapter` (`LiveSesAdapter` sends via AWS SES on armed production; `StubSesAdapter` captures messages in memory for dev, test, staging, and dark production), `PaymentAdapter` (Stripe), `JwtSigningAdapter` (`createKmsJwtAdapter` uses AWS KMS Sign/GetPublicKey in production with RS256; `createLocalJwtAdapter` uses a file-based RSA keypair in dev/test with the same RS256 algorithm), `SecretsAdapter` (`LiveSecretsAdapter` reads SSM SecureString parameters in staging and production; `StubSecretsAdapter` is an in-memory map used by tests and by dev), `SafeBrowsingAdapter`, `CaptchaAdapter` (Cloudflare Turnstile in production; stub on dev and staging), `HttpReachabilityAdapter`, `ImageProcessingAdapter`, `VideoTranscodingAdapter`, and `BallotEncryptionAdapter` (KMS envelope encryption in production; local-keyed in dev/test). Enables testing without external services and dev/prod parity by configuration swap.
...
**Health Endpoint**: Lightweight HTTP endpoints reporting system status. Footbag.org exposes two endpoints: /health/live (liveness, confirms the process is running, no external calls) and /health/ready (readiness, validates SQLite connectivity and container memory pressure; returns 503 above the memory threshold to trigger CloudFront maintenance). Backup, SES, KMS, Stripe, and S3 health are surfaced via alarms, not via this endpoint.
...
**Legacy username**: The product-facing term for the old footbag.org account identifier stored in the `legacy_user_id` column. Use it consistently in UI copy, error messages, and documentation regardless of the column name, so a member claiming an old account sees the words they knew the account by rather than a database field name.
...
**Stripe Subscriptions**: Recurring billing feature automatically charging members monthly or annually. Footbag.org uses subscriptions for recurring annual donations only (membership tiers are lifetime and do not renew). Member-initiated cancellation stops future charges; the webhook on `customer.subscription.deleted` transitions the local subscription record to canceled.
