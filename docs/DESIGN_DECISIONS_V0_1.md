...
Single CloudFront distribution fronts all content with different cache behaviors: public cacheable HTML/public GET content from the Lightsail origin uses short caching (default: 5 minutes) with a cache policy configured with Minimum TTL = 0 so origin Cache-Control headers can disable caching when required; authenticated/personalized HTML and API responses from the Lightsail origin use the AWS managed `CachingDisabled` cache policy (TTL 0/0/0); static assets (CSS, JS, images) use aggressive caching with origin S3 static bucket; archive uses 1-year TTL with origin S3 archive bucket and members-only access controls.

Rationale:

- Reduces load on origin server and S3 by serving from edge locations.

- Short caching (default 5 minutes) balances freshness vs performance for public cacheable HTML/public GET content.
...
- Up to a 5-minute lag for updates to cacheable public content may be visible globally unless invalidated sooner.
...
Two-tier rate limiting: in-process middleware (60 requests/min for anonymous, 120 for authenticated) plus AWS WAF at CloudFront for DDoS mitigation. App-side rate limiting state (including IP-based counters for login/reset) is kept in memory only and is not persisted to the database or logs. No sophisticated bot detection or AI-based abuse prevention.
...
- AWS WAF managed rules handle known threat patterns.

- Community scale doesn't justify complex anti-abuse systems.
...
- Manual admin intervention required for coordinated abuse.
...
- Admin tools support temporary IP blocking for severe cases.

- Upload operation caps (application-level, per member): 10 photo uploads per hour, and 5 video link submissions per hour.

- In-process counters of operations per member are memory-only for rate limiting purposes; Any IP blocking is via WAF/IPSet configuration, not app-stored IP history.
