...
`tests/smoke/`, gated behind `RUN_STAGING_SMOKE=1`, excluded from default `npm test`. Runs against real staging AWS (KMS, SES, SSM) via the assumed-role chain configured by `scripts/test-smoke.sh`. The canonical regression gate for staging-AWS adapter parity.

This tier is the only one that reaches AWS at all, and it reaches it read-and-verify only: identity and parameter reads, a KMS signature, a message to Amazon's simulator sink. Every other tier is unauthenticated by construction. The shared setup breaks credential resolution for each worker and everything it spawns, so a test that runs an operator script cannot write to a deployed environment whatever that script would otherwise do, and a convention gate keeps that in place. The rule and its rationale live in `.claude/rules/testing.md`, "Tests never mutate live infrastructure".
...
- Identity, KMS, SES round-trip verification

Does not belong:

- Any write to cloud object storage or a deployed database, in any environment. A test suite is collectible by anyone and runs unattended, so it is the wrong home for something that mutates live infrastructure; that verification belongs in operator tooling a person deliberately runs.
...
Outbound email is a layered surface: content, enqueue, transport drain, human inspection, and live deliverability each fail differently and are tested at the layer that isolates the failure. The platform sends transactional security mail (verification, password, claim) and operational notifications (membership, club, payment, Active Player) through one path: a service composes the message, enqueues an `outbox_emails` row through `communicationService`, and the worker drains the row through the `SesAdapter` seam. `StubSesAdapter` captures messages in memory for development, CI, and staging; `LiveSesAdapter` sends through Amazon SES in production. The `SES_ADAPTER` config invariant in `src/config/env.ts` defaults to `stub` below production and follows the email arming flag in production (armed requires `live`, dark requires `stub`; production boot fails fast on an unset adapter or a flag mismatch), so mail is captured rather than sent everywhere except an armed production.
...
The staging-smoke leg applies to adapters that reach an external service over the network: `JwtSigningAdapter`, `SesAdapter`, and `SecretsAdapter` (real AWS via the assumed-role chain), `SafeBrowsingAdapter` (the real Google Safe Browsing API), and `HttpReachabilityAdapter` (arbitrary member-submitted hosts on the public internet). The two internal docker-network adapters (`ImageProcessingAdapter`, `VideoTranscodingAdapter`) carry boot-config and interface-parity tests but no workstation staging-smoke: their dependency is the internal `image` worker rather than an external surface the dev-against-staging (no-ssh) model can reach, and their failure mode is a noisy first-request error rather than silent IAM or network drift. Their wiring is covered by the interface-parity tests, the compose `image:4000/health` healthcheck that gates stack startup, and the e2e upload path.

`HttpReachabilityAdapter` belongs with the external group rather than the internal one, which is what its outbound probe actually is: it resolves and fetches hostnames a member typed, and it needs no deployed host to exercise, so a workstation runs the same code path a host runs. Its failure mode is also the silent kind the smoke tier exists for. Every other test of it injects a fake fetch and a fake resolver, and outbound HTTPS that a network, a proxy or a resolver refuses is indistinguishable from an adapter that was never wired: the site simply starts telling members their links are wrong. Its smoke probes are chosen to touch nothing incidental — IANA's reserved documentation domain for the reachable case, an RFC 5737 TEST-NET address for the unreachable one, and a loopback address for the SSRF refusal, which is the assertion that matters most because a live adapter that lost its per-hop guard still passes the other two.

`MediaStorageAdapter` carries boot-config and interface-parity tests but no workstation staging-smoke. Its live implementation writes and deletes objects in a real bucket, and no test writes to cloud object storage in any environment, so the put, head, and delete path is proved by the interface-parity tests against an injected S3 client and by the e2e upload round-trip. Verifying the live bucket path is operator tooling, run deliberately, not a suite.
...
The suite reaches live AWS and depends on workstation state a contributor without a deployed-environment role does not hold: the staging runtime AWS profile, an initialized `terraform/staging/` tree, the `footbag-staging` ssh alias, and — for the persona-catalog check — the operator credential file the deploy scripts read. It is not restricted to one tier: the shared dev-and-tester job role grants staging precisely so somebody who develops and tests can run this, and the runtime profile it needs chains from that role. A production target is a different matter, and the boundary is the trust policy rather than a convention: production's runtime role trusts the directly authenticated operator identity and the host source profile, and no human job role at all. Every entry point fails fast with a plain message when the state is absent, naming what writes it, as do the deploy and activation scripts. A contributor with no AWS access at all loses nothing by skipping it: `./run_all_tests.sh --full` deliberately excludes this suite (its summary row shows SKIP), and every other gate runs without AWS.
...
The pipeline validation gates in §8.9 below are canonical for migration and onboarding tests. This section adds the playbook framing for deriving migration tests and the constraints that bind to them. The operational-readiness rules live in GO_LIVE_PLAN.md (private GitHub repo) and are cutover-readiness gates, not test anchors.
...
| G18 | Rate limiting active on identifier lookup, declared-anchor changes, claim confirmations, optional mailbox-link-click round-trip, registration, and password-reset per the `M_Claim_Legacy_Account` user story (token mechanics per DESIGN_DECISIONS "Account Security Tokens"; numeric defaults owned by `docs/USER_STORIES.md`) | Block go-live until limiters engage; declared-anchor enumeration and mailbox abuse otherwise unmitigated |
| G19 | Wizard claim task universally surfaces staged candidates and the declared-anchor prompt to every registrant, per the auto-link matching design in the Legacy Data Migration decision (DESIGN_DECISIONS "Legacy Data Migration") and the `M_Complete_Onboarding_Wizard` user story; all evidence tiers (`declared_anchor_only`, `currently_controls_modern_email_matching_legacy`, `mailbox_control_via_link_click`) exercised at test load. The `mailbox_control_via_link_click` tier is exercised via the G22 round-trip | Members without a stage-1 anchor match never get prompted to declare; legitimate claims are missed |
| G21 | `legacy_user_id` and `legacy_email` (with `legacy_email2` and `legacy_email3`) populated on `legacy_members` where the delivered export provides them. That table is where claim lookup and auto-link candidate matching read the email anchor; the canonical `persons.csv` keeps its email column empty, because it is committed to the public repository and the platform export carries the identity columns nowhere | Claim-lookup falls back to `legacy_member_id` only; auto-link candidate coverage drops because the email anchor is missing |
...
| G23 | Multi-anchor candidate matching covers verified modern email, declared old emails, declared former surname, and current real-name surname, per the auto-link matching design in the Legacy Data Migration decision (DESIGN_DECISIONS "Legacy Data Migration"); seeded `name_variants` table drives first-name variant resolution | Candidate coverage drops to email-anchor-only and admin-help-request volume rises |
| G24 | `OperationsPlatformService` batch auto-link SYS job exercised against the test load, whose personas wait at the wizard's claim step; the job stages candidates (no live-table mutation, no notification emails); one `system_job_runs` row recorded per run; audit-emission coverage verified at test load (one `audit_entries` row per `legacy.auto_link_candidate_staged` event). Idempotent: rerun produces no duplicate candidates | If audit-emission coverage is incomplete, fix the audit emission path before declaring G24 PASS; the staged-card path otherwise reaches launch proved by unit coverage alone, and a defect in it first shows on a member's own claim |
...
- Origin-pin CSRF coverage: every state-changing route refuses a foreign Origin and a forged Referer, while a same-origin request reaches the handler. The perimeter's own exemptions, the signature-authenticated webhook receivers, the one-click unsubscribe endpoint whose caller is a mail client, and the shared-secret internal mounts, are excluded from the sweep and read from the middleware that enforces them, so the probe holds no second copy of that list to drift against.
...
**M_Donate** and **M_View_Payment_History** (dims 1, 3, 12, 13, 15). Authenticated; a donation is recorded distinctly from membership; amount validation; a recurring donation writes its row before the redirect and is promoted, put past due, restored and canceled only by the provider's events, with out-of-order and redelivered events refused as stale; a member cancellation sets the provider flag and the local row moves only on the deletion event; history shows shaped labels, the donation note, and the recurring rows distinct from renewals.

**SYS_Process_One_Time_Payments** and **SYS_Process_Recurring_Donations** (dims 1, 7, 12, 13, 15). Full and partial refunds, every dispute stage, rejected payouts and unattributed refunds each write their audit row and work item in the same transaction as the idempotency claim, proven by fault injection between the claim and the side effect; a settled amount or currency that disagrees with the local record raises the mismatch at once; a zero-amount renewal invoice is acknowledged, not booked; the subscribed event set, the dispatcher and the activation script are pinned to each other.

**SYS_Reconcile_Payments_Nightly** and the **A_Reconcile_Payments** digest (dims 10, 12, 13). Each pass against a mixed ledger: missing on either side, amount and currency mismatch, refund distinguished from mismatch, canceled locally but settled at the provider, stranded recurring checkouts keyed per row, duplicates in quick succession; rows of the other provider mode set aside and counted; overlapping runs raise one issue; resolution is audited and a still-present discrepancy is raised again; the digest goes to the treasurer contact address at the configured cadence, nil report included, and the Financial Reports view is admin-only and read-only.

**A_View_Stripe_Config_And_Payments** and the payments pause switch (dims 1, 5, 12, 14). Admin-only; the health view reads the credential mode from the running process and shows no key material; webhook silence and failure states; totals count live rows only and state what was set aside; the pause flag refuses new checkouts before any provider call while a signed webhook still processes; the operator script appends every flip with its reason and has no application write path.
...
2. **Adapter parity, three legs (§7.2).** Every adapter in `src/adapters/` has a boot-config test (`tests/unit/env-config.test.ts`) and an interface-parity test (`tests/integration/adapter-parity.test.ts`). Every external-service adapter whose live probe is side-effect-free (JWT-KMS, SES, Secrets-SSM, SafeBrowsing, HttpReachability) also has a staging-smoke leg (`tests/smoke/`, including `staging-readiness.test.ts` for the KMS-sign and SES-send round-trips). The internal docker-network adapters (ImageProcessing, VideoTranscoding) carry only the first two legs, as do the adapters whose live call would mutate a deployed system (MediaStorage-S3, Payment). Pass: the matrix has no missing required leg.
