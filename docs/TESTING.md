...
- *Content (unit).* Each email's subject and body come from an exported builder, a pure function from typed inputs to `{subject, bodyText}`. Unit tests import the builder and assert the interpolated parts (recipient name, expiry date, tier label, reason text) and the handling of unicode and empty optional fields. A test imports the builder rather than restating body copy as a literal, per §15.2.
- *Enqueue (integration).* For each email, a test asserts the triggering service method enqueues one `outbox_emails` row with the correct recipient, idempotency key, and builder-produced content; that guard and no-op paths enqueue nothing; and that a member with no deliverable address (absent login email, deceased, soft-deleted, or purged) is skipped without raising.
...
- *Human inspection (development and staging).* Captured mail is readable without a real inbox. Email-gated pages render the simulated-email card (§16.4), and the `/dev/outbox` viewer lists every captured message for notifications that have no host page. The in-process stub and these viewers serve as the catch-all inbox; they need no extra process and run identically in CI.
...
13. **Email catalog (§5.9).** Every promised email has an exported content builder with a unit test, a per-email enqueue test, and an entry in the catalog firing sweep; the shared drain, retry, dead-letter, and idempotency mechanics are covered once over `communicationService`. Pass: the firing sweep enumerates the catalog and every entry has its builder and enqueue test, or the gap is a tracked `IMPLEMENTATION_PLAN.md` item.
