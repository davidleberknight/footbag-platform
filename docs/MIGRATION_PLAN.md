...
It opens with three short sections for coordination with the legacy-site webmaster: a summary of the migration, the open questions that need the webmaster's input, and a recap of the decisions already made. The numbered sections that follow carry the gates, the operational states, the sequencing, and the operational-readiness detail.
...
The migration is a single clean go-live with no parallel systems. At cutover the `www`/apex DNS points at the new platform (`www` at its CloudFront distribution; the apex at a small platform-run redirector that 301-redirects to `www`) and the legacy site goes dark; the legacy server is kept only as a cold, restorable backup for the rollback window plus a dispute-lookup period, then retired. There is no live legacy site afterward. Rollback within the monitored window is a DNS revert to the legacy backup (the zone runs a low TTL). Outbound mail becomes the platform's, sent via SES (the webmaster amends the `footbag.org` SPF and adds the SES DKIM records); legacy mail retires at cutover, with inbound role addresses moving to Google Workspace, provisioned before legacy mail is withdrawn. At the final-export moment the legacy member system is frozen read-only permanently (one-way — it never takes member writes again); the export is taken from that frozen state, so the import is one clean snapshot with no delta reconciliation. Legacy then serves only as the cold read-only backup and is later retired. Functionality not in the first release is built natively afterward, complete by Worlds 2027; legacy is never relied upon. Historical content lives read-only at `archive.footbag.org` (distinct from the legacy backup).
...
Quick status (reconciled to the clean-cutover reframe): **Settled**: 1, 8, 9, 12, 16 (clean DNS cutover; SES outbound + Google inbound; legacy mail retires; impersonation handled in-platform). **Moot under the clean cutover**: 19. **Confirmed against the delivered dump, re-confirm on the final export**: 10, 11, 17. **Still open (webmaster/Julie coordination)**: 2, 3, 4, 5, 6, 7, 13, 14, 15, 18, 20, 21, 22, 23.

Of the still-open items, three are the schedule's long poles and are asked for first: the committee and board table delivery (§28), the mailbox and mailing-list inventory (questions 6-7), and the registrar and provider confirmation (question 2). Each gates a cutover-critical step — starting tiers, the mail switchover, and the DNS switch — so answering them early is what sets the go-live calendar.
...
**1. Front-door architecture: clean DNS cutover. Settled.** At cutover the `www`/apex DNS points at the new platform and the legacy site goes dark; no reverse proxy on the legacy server (so no legacy TLS cert). Cutover is a DNS switch; rollback is a DNS revert to the legacy backup (low TTL). A smoke test proves the path before the flip.

**2. DNS registrar/provider, bind9 end-state, and apex handling. Still open — now go-live-relevant.** Because the clean cutover points the apex at the new platform and bind9 cannot serve ALIAS/ANAME records (a plain CNAME at the apex is illegal per RFC 1034), the apex is served by a small platform-run redirector with a static IP that 301-redirects `footbag.org` to `www.footbag.org`, with `www` as a CNAME to CloudFront; `www` is the canonical host. What remains for the webmaster: confirm the registrar and the authoritative provider, confirm whether keeping bind9 authoritative is a requirement only through go-live or durable (that ruling decides whether the zone later moves to Route 53, where the apex becomes a free ALIAS record and the redirector retires), and confirm the reverse-DNS update path if the sending IP changes. IFPA owns `footbag.org`, the only domain the cutover touches. Separately and at low priority (not a cutover dependency): confirm whether IFPA still owns `footbag.sport` and `footbag.us` and decide whether to hold them defensively (to block a look-alike footbag/IFPA site, or to redirect mistyped traffic to `footbag.org`) or let them lapse.

**3. Legacy subdomains. Still open — per-subdomain disposition needed.** The clean cutover moves only the apex and `www`, so every other `*.footbag.org` subdomain keeps its existing records on the webmaster's zone until dispositioned. The webmaster rules per subdomain — stay-on-legacy, retire, or snapshot-and-archive — against the §19 item 16 inventory (known candidates: `sites.footbag.org`, the Majordomo list host, the `/reference/` MediaWiki). Any subdomain that stays live receives the platform's widened `.footbag.org` session cookie, so the §29.15 cookie analysis is re-verified against the ruling. The only subdomain the platform itself uses is `archive.footbag.org` (platform-controlled, CloudFront, HTTPS, per the Legacy Archive decision in DESIGN_DECISIONS, §6.4). The webmaster's bind9 zone (already provided) is the reference for any DNS cleanup at the later Route 53 migration.
...
**5. TLS on retained subdomains. Still open — follows question 3.** The session cookie widens to `.footbag.org` to reach the platform-controlled `archive.footbag.org` (CloudFront/HTTPS per the Legacy Archive decision in DESIGN_DECISIONS, §6.4). If question 3 retires every legacy subdomain, there is no retained-subdomain leak exposure and this is moot. If any legacy subdomain stays live, it also receives the widened session cookie, so its TLS posture and trust level must be assessed before the cookie widening ships (see §29.15, §29.16). The new platform's own TLS is served by CloudFront/ACM.
...
**8. Email cutover and the inbound-receiver. Settled.** The platform sends outbound via SES from go-live (the webmaster amends SPF + adds SES DKIM); legacy mail retires at cutover. Inbound role addresses (`admin@`, `info@`, …) move to Google Workspace: the `footbag.org` MX points at Google, and every active address is provisioned there before legacy mail is withdrawn. The per-address and per-list inventory (questions 6-7, with Julie) supplies the provisioning facts.
...
**13.** Front-door architecture is a clean DNS cutover: at cutover `www`/apex point at the new platform and the legacy site goes dark. No reverse proxy on the legacy server, so no legacy TLS certificate is needed. A pre-cutover smoke test proves the path before the flip. `www` is a CNAME to the CloudFront distribution; the apex is served by a small platform-run redirector with a static IP that 301-redirects to `www` (bind9 cannot serve apex ALIAS/ANAME records).

**14.** Cutover is the webmaster switching the `www`/apex DNS to the new platform, gated on the smoke test; rollback within the 48-hour window is a DNS revert to the legacy backup (low TTL), which serves read-only (the member system stays frozen); a database snapshot is always taken just before the flip.

**15.** The platform owns outbound mail via SES from go-live (the webmaster amends the `footbag.org` SPF and adds SES DKIM); legacy mail retires at cutover. Inbound `@footbag.org` role addresses move to Google Workspace (question 8, settled): the MX points at Google and every active address is provisioned there before legacy mail is withdrawn.

**16.** The webmaster handles DNS at first; DNS hosting may move to Route 53 at a later, optional, post-stability handover — the bind9 end-state ruling in open question 2 decides. Registrar and provider details are open question 2.
...
1. **Front-door architecture (settled)**: a clean DNS cutover. At cutover the `www`/apex DNS points at the new platform and the legacy site goes dark; there is no reverse proxy on the legacy server (so no legacy TLS certificate). The distribution carries `footbag.org` / `www.footbag.org` as alternate domain names with the ACM certificate as ownership proof; `www` is a CNAME to CloudFront and the apex is an A record to the platform-run redirector that 301-redirects to `www` (bind9 cannot serve apex ALIAS/ANAME records). A pre-cutover smoke test proves the path before the flip. Cutover is the webmaster's DNS switch; rollback is a DNS revert to the legacy backup (the zone runs a low TTL). Client IPs reach CloudFront directly (no proxy-forwarded-header indirection at the apex).
...
15. **Zone authority and bind9 end-state (now cutover-relevant)**: the webmaster, a DNS expert, holds and operates the zone through cutover. Under the clean cutover the apex moves to the new platform at go-live: `www` is a CNAME to CloudFront and the apex is an A record to the platform-run redirector (bind9 cannot serve apex ALIAS/ANAME records). Confirm the registrar, the authoritative provider, the reverse-DNS update path if the sending IP changes, and whether keeping bind9 authoritative is a requirement only through go-live or durable. That ruling decides the end state: if the zone later migrates to Route 53 (deferred and optional, never a cutover prerequisite), the apex becomes a Route 53 ALIAS record, the redirector retires, and DNS authority leaves the webmaster's infrastructure, fully releasing him.

16. **Legacy subdomain inventory**: enumerate every `*.footbag.org` subdomain that must continue resolving at the legacy host through cutover and beyond. `archive.footbag.org` is reserved for the new platform per §29.15. Full inventory needed, from the webmaster, not the repo (the mirror does not represent DNS or server config). A known candidate to confirm and disposition is the Majordomo list host `list.footbag.org` (with its frozen read-only listserv archive, 1994-1999; the webmaster confirms whether a separate `lists.footbag.org` name also exists). The operator-only `sites.footbag.org` proxies a set of WordPress sites (years of Worlds event sites) that must keep working (front-matter question 22), and a live MediaWiki at the `/reference/` path is a retained service whose database is not yet delivered (front-matter question 23). Any private operator-only subdomain must be flagged private and not-for-publication. Media reachable only by direct `video.`/`photo.` file paths is handled under §29.15 (archive completeness), not retained as a subdomain.

17. **Records-actor (AGREED: the webmaster)**: the webmaster applies maintainer-supplied records to the zone. The records are: the ACM validation CNAMEs for both certificates (permanent; they also drive renewals), the SES DKIM CNAMEs (permanent), the custom MAIL FROM subdomain records (`mail.footbag.org` MX and SPF TXT, required for DMARC alignment), the SPF amendment authorizing the SES and Google senders, the DMARC record with its report mailbox, the Google MX record, the `www` record pointing at CloudFront and the apex record pointing at the apex redirector (the cutover itself), and the `archive.footbag.org` record pointing at the archive distribution. Per-record timing and verification commands are the §29.12 hand-applied record inventory.
...
24. **Email cutover coordination**: confirm the SPF amendment and SES DKIM records are applied and one real end-to-end SES send is verified; and that every active `@footbag.org` address is provisioned on Google Workspace and the MX repointed to Google before legacy mail is withdrawn, so no inbound is lost. The Google provisioning and MX repoint run as a discrete step ahead of the web cutover (adding senders to SPF is additive and harmless to the still-running legacy sender); legacy outbound retires at the web cutover.
...
**Settled:** item 1 (clean DNS cutover); item 17 (the webmaster is the records-actor); item 6 (test export delivered, validation running); item 2 (legacy stack known: PHP/MariaDB/apache2/bind9/sendmail); item 3 (member count and activity present in the dump); item 23 (mail server is sendmail).
...
**Then, in order:** provision the Google mailboxes and repoint the MX, apply the SPF amendment + SES DKIM, and verify one real SES send (item 24); answer item 26 (groups/committees disposition, with Julie); answer items 18-19 (DNS-switch coordination, secondary contact); answer item 33 (banned policy); agree the legacy-retirement window (item 31); freeze the legacy member system permanently read-only (items 13 and 29); produce the member export from that frozen state (item 14); run the smoke test on a test subdomain; switch the `www`/apex DNS to the new platform.
...
2. **Mirror member extraction** into `historical_persons` (~1,700 club-only members; loaders built, the extraction code tracked in `IMPLEMENTATION_PLAN.md`). The field mapping and conflict policy are the Legacy Data Migration decision in DESIGN_DECISIONS (§6.5) and the legacy_data pipeline (`legacy_data/CLAUDE.md`).
...
| ID | Criterion | Section | Blocks |
|---|---|---|---|
| EX1 | `footbag.org` domain owned by IFPA; clean DNS cutover to the new platform arranged and verified with the webmaster (`www` CNAME → CloudFront; apex A record → the apex redirector, 301 to `www`) | §23 Phase 4 prereqs; §29.12 | State 3 → State 4 |
...
| WM12 | Legacy retirement milestones agreed and recorded (stable post-cutover operation, rollback + dispute window elapsed; the optional Route 53 zone migration is its own milestone if ruled), with a review cadence | §19 item 31 | State 3 → State 4 |
| WM13 | `footbag.org` zone registrar and authoritative-DNS provider identified; the optional Route 53 zone migration, if ruled, planned with a fresh zone snapshot (relevant at the post-stability DNS-handover milestone, not at cutover) | §19 item 15; §29.12 | DNS-handover milestone (post-cutover) |
| WM14 | Records-actor agreed (the webmaster); maintainer-supplied records applied on schedule per the §29.12 hand-applied record inventory: ACM validation CNAMEs (both certificates) and SES DKIM CNAMEs early; the MAIL FROM subdomain records, apex SPF, DMARC, and the Google MX on email day; the `archive.footbag.org` record before cutover; `www`/apex at T-0 | §19 item 17 | State 3 → State 4 |
...
| OR15 | Smoke test green before the switch: `curl --resolve` against a current CloudFront edge IP returns the `www` page over the `footbag.org` certificate (verified via the test subdomain), and the apex redirector answers `https://footbag.org/` with a 301 to `https://www.footbag.org/`; first run at State 3, re-run green on cutover day | §29.12 | State 3 → State 4 |
...
| OR23 | Post-launch close-watch period completed before legacy retirement and the DNS handover: the 14-day close-watch window passes with no unresolved high-severity issues and error/latency within target | §29.18 | DNS-handover milestone (post-cutover) |
| OR24 | DNS preflight passed: the CAA check on the webmaster's zone allows Amazon's certificate authorities (or no CAA exists) before ACM issuance; at T-48h the 60-second apex/`www` TTL is observed from the webmaster's authoritative nameservers, and the prior TTL is recorded and has expired before T-0 | §29.12 | State 3 → State 4 |
...
- Email cutover complete per §29.12a: SPF amended to authorize SES, SES DKIM applied, one real SES send verified; Google Workspace inbound provisioned and the MX repointed; legacy mail retires at cutover
...
- SES production-access ticket filed and approved (filed at State 2 completion; first response within 24h, resolution can take days; see Phase 4 prerequisites)
...
**Post-flip path B, catastrophic failures (rare, narrowly defined):** DNS revert (the webmaster reverts the `www`/apex DNS to the legacy backup, which serves read-only — the member system stays frozen; the zone runs a low TTL) plus DB restore from the pre-flip snapshot. The revert converges over minutes, not seconds: clients and resolvers that cached the new records lag by up to their cached TTL. Catastrophic is one of:
...
### Front-door architecture (settled)

The design is a clean DNS cutover: at cutover the `www`/apex DNS points at the new platform and the legacy site goes dark (§19.1, §29.12). The legacy server is not in the live path; it is kept as a cold, restorable backup for the rollback + dispute window, then retired. Trade-offs: the apex needs its own mechanism — the platform-run redirector 301-ing to `www`, since bind9 cannot serve apex ALIAS/ANAME records — and rollback is a DNS revert (mitigated by a low TTL). In exchange: no reverse proxy or TLS certificate on the legacy box, no critical-path legacy dependency in the live path, and CloudFront sees client IPs directly.

**CloudFront technical constraints (verified):** the distribution carries `footbag.org` and `www.footbag.org` as alternate domain names with a matching ACM certificate in us-east-1; adding the alternate domain names requires only the certificate, not a DNS repoint. No intermediary ever fronts the distribution: anything terminating TLS for these names would have to present SNI and Host matching the alternate domain names, or CloudFront rejects the request with HTTP 421. A plain CNAME at the apex is illegal per RFC 1034 and bind9 has no ALIAS/ANAME type, which is why the apex uses the redirector; if the zone later migrates to Route 53 (optional), the apex becomes a Route 53 ALIAS and the redirector retires.
...
Under the clean cutover the apex moves to the new platform at go-live: the webmaster switches the `www`/apex DNS to the new platform (`www` a CNAME to CloudFront; the apex an A record to the platform's apex redirector, which 301-redirects to `www`) and the legacy site goes dark. Both the ACM certificate issuance procedure (`docs/DEVOPS_GUIDE.md` §9.2.1) and the DNS cutover procedure (`docs/DEVOPS_GUIDE.md` §6.7) apply at cutover.
...
- **Apex/DNS records ready**: `www` is a CNAME to the distribution's `cloudfront.net` endpoint and the apex is an A record to the platform's apex redirector (301 to `www`), which is provisioned, TLS-serving, and smoke-tested; a low TTL is set in advance so the switch and any revert propagate fast.
...
- **Smoke test green**: `curl --resolve www.footbag.org:443:<current edge IP> https://www.footbag.org/` returns the expected page with the `footbag.org` certificate, verified against a test subdomain pointed at the distribution; and the apex redirector returns 301 `https://footbag.org/` → `https://www.footbag.org/` over a valid certificate. First run at State 3; re-run green on cutover day before the switch.
...
**Hand-applied record inventory.** The records-actor's complete list (§19 item 17): the maintainer supplies exact values, the webmaster confirms each with `dig` against his authoritative servers.
...
- `www.footbag.org` CNAME → the production distribution's `cloudfront.net` endpoint — applied at T-0.
- Apex `footbag.org` A record → the apex redirector's static IP — applied at T-0.
...
**Zone migration to Route 53 (deferred and optional, §19 items 15 and 31):** if the bind9 end-state ruling (§19 item 15) sends the zone to Route 53, the migration happens once stable post-cutover operation is confirmed and the cold backup is retired: the apex and `www` become Route 53 ALIAS records to the distribution, the apex redirector retires, and the registrar's NS records delegate to Route 53. The generic DNS cutover procedure (`docs/DEVOPS_GUIDE.md` §6.7) governs this, with a fresh zone snapshot first so MX, DKIM, SPF, DMARC, and validation CNAMEs copy faithfully. Migrating fully releases the webmaster; if bind9 authority is durable, the redirector and the hand-applied records remain the standing arrangement.

### 29.12a Legacy host after cutover (cold backup)

After cutover the legacy host is **not** in the live path for the apex or `www`. It is kept as a cold, restorable read-only backup for the §27 rollback window plus a dispute-lookup period, then retired (§19 item 31). There is no parallel front door for the main site; whether any legacy subdomain stays live is the open §19 item 16 disposition.
...
2. Historical content is served read-only from the platform-controlled `archive.footbag.org` (its own CloudFront distribution over HTTPS, Route 53 DNS, signed-cookie auth per DD §6.4, Legacy Archive), not from live legacy subdomains. The session cookie widens to `Domain=.footbag.org` per DD §3.2 (JWT sessions) and §6.4 so the archive receives it; the only intended host receiving it is the platform-controlled archive over HTTPS; if the §19 item 16 ruling keeps any legacy subdomain live, that host also receives the widened cookie and the exposure is re-assessed before the widening ships.
...
- **Inbound email impact:** `@footbag.org` inbound is on Google Workspace (per §28), independent of the legacy host. `@ifpa.footbag.org` on llic.net is independent too.
...
Rollback: if Google inbound fails verification, revert the MX to the legacy mail server until it is fixed; the web cutover is independent and unaffected. Gate: EX7.
