...
**16.** DNS hosting moves to Route 53, under IFPA-controlled access, before go-live — advance preparation (required by design intent: no hard dependency on legacy tech after go-live). IFPA is the registrant: the primary maintainer, as an IFPA officer and the domain technical contact, reclaims the registrar account under IFPA control and points the domain at Route 53; no registrar action is asked of the legacy webmaster, whose bind9 service retires once the move completes and nothing still resolves through it. The move precedes the certificate and mail-authentication work that depends on the zone, so those records are applied on Route 53 by the operator rather than hand-applied on the legacy nameservers.
...
3. **Tier-mapping fields on `legacy_members` (§15.16)**: the tier/payment source fields available on the member record are `MemberIFPATier`, `MemberIFPAExpiration`, `MemberIFPAExpiration2`, `MemberIFPAPrevExp`, `MemberIFPAPaid`, `MemberIFPAPaymentDate`, and `MemberIFPAJoined`, plus `ifpa_memberpayments`; §15.16 states which of them the derivation reads. The `legacy_*` columns are derived, not copied. `MemberIFPAExpiration` and `MemberIFPAExpiration2` together are what the legacy site's own tier function reads: the second carries Tier 2 standing, and the pair distinguishes lifetime standing (the `-1` sentinel) from an annual expiry date. Board-at-cutover standing is precedence 1. The committee tables live in the legacy `groups/` app alongside the 1.2GB `ifpa_group_messages` archive, which is why that module's data is handled separately from the rest of the dump. Their structure: `ifpa_committee_members` links a `CommitteeID` to a member `CommitteeMemberID` with title, admin, and voting flags, and `ifpa_committees` carries a `CommitteeType` and `CommitteeIsOfficial` (groups and committees share this one table). All board logic sits behind the single constant `BOARD_IFPA_TIER_CODES` in `extract_legacy_members.py`, which emits `legacy_was_board_at_cutover` and `legacy_board_underlying_paid_tier`. The constant stays permanently empty, so the tier-code path makes no positive board determination and does not distinguish "unknown" from "not board". The legacy admin PHP confirms `MemberIFPATier` holds only 0, 1, and 2 and does not encode Tier 3, so there is no `MemberIFPATier` board code to set; precedence 1 therefore derives from the committee tables rather than from a tier code. If the board or paid-history derivation proves insufficient, those fields are left unpopulated and their standings do not fire, so affected claims grant on honors alone.
...
**Zone move to Route 53 (go-live preparation):** the zone moves before go-live, early enough that all records are staged directly on Route 53, and resolution is verified against Route 53 before any cutover step depends on it. The primary maintainer (IFPA officer and domain technical contact) is the acting party for the registrar side. The move procedure is AWS_OPERATIONS.md (private GitHub repo), the registrar-reclamation and zone-move section. DNS authority sits under IFPA-controlled access from the move onward.
...
2. Historical content is served read-only from the platform-controlled `archive.footbag.org` (its own CloudFront distribution over HTTPS, Route 53 DNS, signed-cookie auth per DD §6.4, Legacy Archive), not from live legacy subdomains. The archive's three CloudFront signed cookies carry `Domain=.footbag.org` per DD §6.4 so the archive edge receives them, while `footbag_session` stays host-only per DD §3.2 (JWT sessions); the signed cookies carry `Secure`, so only a `.footbag.org` name answering over HTTPS could receive them, and no legacy subdomain stays live, so the platform archive is the only other host that ever does.
...
1. **Inbound mailbox inventory.** A candidate enumeration is derivable in part by replaying the legacy alias logic. `members/admin/livealiases.sh` is the authoritative composition: it runs three generators in sequence — `members/admin/dumpaliases.php` for the per-member alias-to-forward map plus the `announce`/`members` lists, `ifpa/admin/createmailinglists.php` for the committee and group lists, and `pushadminaliases.php` for the role addresses (`admin@`, `webmaster@`, `registration@`, and so on). The first two are in the delivered clone; the third and its `admin/` module are not, so the role-address half of the inventory is not derivable from the clone and must be obtained another way. What the dump does not settle is which addresses are actively used versus dead or spam-only, and any apex aliases hand-configured in sendmail outside the codebase. The inventory is the transition-day safety gate (§28): every active address must be provisioned on Google before legacy delivery is withdrawn.
