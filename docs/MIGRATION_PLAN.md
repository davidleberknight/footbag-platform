...
**6. A list of footbag.org mailboxes and aliases (real mailboxes versus forwarding). Still open.** Every address still in use must exist on Google before mail is repointed there, or its incoming mail is silently lost. Recommendation: the webmaster lists every footbag.org address, noting which are real mailboxes versus forwarding and which are actually used, and every active one is created on Google before the mail switchover. Partial inventory confirmed (Julie's discovery plus a live-DNS check, July 2026): the hand-built apex aliases julie@, ifpa-treasurer@, and sanctioning@ are live (forward targets not yet known), and the apex SPF record authorizes the legacy mail host, one further unexplained sender, and Google (concrete host names and the sender IP recorded in DEVOPS_GUIDE.md, private GitHub repo), so at least one sender already sends @footbag.org mail through Google.
...
**22. The WordPress event sites behind sites.footbag.org. Settled: crawl everything into the member-visible archive.** The legacy host serves a set of WordPress sites (years of Worlds event sites) under the operator-only sites.footbag.org name; a live-DNS check (July 2026) shows that name is served from a second legacy machine (host name recorded in DEVOPS_GUIDE.md, private GitHub repo), which joins the shutdown checklist. The www crawl already captured the event sites where they are cross-published as www.footbag.org paths (worlds94 through worlds2017); the remaining vhost-only delta is seeded and crawled into the same mirror, whose hostname rewriting makes the content ordinary member-visible archive pages, so the vhost name never surfaces and is simply retired. Per-site re-host or consolidate decisions stay open, with the archive as the default. Nothing stays on a live legacy server.
...
15. **Zone authority: the move to Route 53 (go-live preparation)**: the webmaster holds and operates the zone today; before go-live the zone moves to Route 53 under IFPA-controlled access (front-matter question 2, decision 16). Because IFPA is the registrant, Julie, as IFPA's representative and the domain technical contact, reclaims the registrar account under IFPA control; no registrar or zone action is asked of Steve. The zone moves to Route 53 early so that every record is staged directly on Route 53 and nothing is hand-applied on the legacy nameservers. After the move the operator manages the records, and `www` and the bare domain are served as alias records to the platform's CloudFront distribution at the switch, with no separate redirect server needed (Route 53 serves a bare-domain alias; bind9 could not). The legacy bind9 service goes dark at the cutover.
