...
Where each question stands under the single coordinated go-live: **Settled or required by design intent**: 1, 8, 12, 16 (the clean DNS switch on Route 53 and the end of legacy mail at go-live are required by the no-legacy-dependency design intent; the write-freeze-and-export action and in-platform impersonation handling are settled). **No longer applies**: 5 (no legacy subdomain stays live, so there is nothing left to monitor). **Reopened, needs verification**: 9 (the Google Workspace account facts must be confirmed rather than assumed), 19 (evidence shows a forum existed on the legacy server after all, so its fate must be confirmed). **Confirmed against the delivered data, to be checked again on the final export**: 10, 11, 17. **Still open**: 2, 3, 4, 6, 7, 13, 14, 15, 18, 20, 21, 22, 23 — now owned by Julie's discovery and the operator (registrar reclamation, the zone move, the email and list inventories, and the subdomain and tournament dispositions), except the final member export (14) and the write-freeze that precedes it, which remain Steve's.

Four things set the go-live calendar and are handled first, and all four are now Julie's or the operator's, not the webmaster's: the committee and board membership table (detailed later in this plan, §28), the list of mailboxes and mailing lists (questions 6-7), Julie's reclamation of the registrar and the operator's zone move to Route 53 (question 2), and Julie's discovery of which groups and email addresses people still actually use (her own section below). Each one blocks a step of the cutover — starting membership tiers, switching the mail, switching the DNS, and standing up the replacement groups and lists — so handling them early is what sets the schedule.
...
**4. Reachability through the go-live window.** With the zone on Route 53, the go-live switch and any emergency DNS change are operator-made, so an incident does not depend on reaching one person. What still needs Steve reachable, agreed in advance: the write-freeze and the final member export happen on his systems just before the switch. The legacy host power-down after go-live verification is his, on his own schedule and off the critical path. Recommendation: agree the go-live window with Steve ahead of time and keep a direct channel open for the write-freeze and the export.
...
**9. Who owns and administers the Google Workspace account. Reopened — the facts must be verified, not assumed.** The intent stands: the IFPA board owns the Workspace, administered by the IFPA secretary together with a platform maintainer. But the account's actual state is unverified, so before it can carry the organization's mail it must be confirmed: which Workspace edition it is, who the legal and billing owner is, which domains it covers, who the current super administrators are, what the recovery contacts are, who controls domain verification, and whether a nonprofit conversion or other account change is needed. Julie has had a Workspace account since 2009. Super-admin control follows domain ownership, not any prior administrator: with IFPA controlling the domain's DNS (once the zone is on Route 53), IFPA regains super-admin control through Google's domain-verification admin recovery — the "Request User Promotion" flow promotes Julie's existing active account to super admin after IFPA adds Google's TXT/CNAME record to that domain, independent of any prior admin's password, 2FA, or cooperation. Steve is not required, provided Julie's account is an active user in that Workspace instance and IFPA controls the DNS of the domain it is verified against (both confirmed first); only if no IFPA-affiliated person holds any active user account there would Steve's cooperation or Google's slower support-assisted recovery be needed. Julie then verifies the facts above, and she and the board approve the future administration and recovery model. Before go-live, at least two IFPA-authorized administrators must have tested, recoverable access. No one is promised a permanent administrative role until governance approves it.
...
**19. Forums. Settled: archive.** The delivered repository contains no forum application code or data, but it proves a phpBB forum existed on the host: a character-set migration tool in the legacy code names a phpBB forum database and its tables, and a saved shell history from the production server shows the forum database being opened and a neighbouring forum configuration file being read. Any surviving forum content is captured into the read-only archive with the rest of the legacy site; no webmaster investigation is needed, and nothing stays live.
...
**23. The /reference/ MediaWiki instance. Still open.** A MediaWiki (version 1.6.8, with a custom `FootbagAuth.php` single sign-on) runs at the /reference/ path, its source on the legacy host and its database not yet delivered. Because that engine version is far out of security support, it is not re-hosted: its content is captured into the read-only archive with the rest of the legacy site (through the `www.footbag.org` crawl of the `/reference/` path), like the forum. No webmaster investigation is needed; nothing stays on a live legacy server.
...
Steve, the legacy-site webmaster (contact at `brat@footbag.org`, the Canonical Email Addresses decision in DESIGN_DECISIONS, §5.5), is the current operator of the live legacy site. The front-matter open-questions table carries the live status of the webmaster items below; this section holds the contract-grade detail. The webmaster's remaining deliverable is the legacy member data (the final frozen export); the legacy DNS, mail, and server-config facts have already been delivered or are now owned by IFPA and the operator. Julie, the IFPA secretary and IFPA representative, holds the governance answers (membership policy, committees, records, rules currency), leads the community-requirements discovery, and reclaims the registrar as the domain technical contact. The written design in these canonical docs is the baseline; proposed changes arrive as specific doc-revision requests, and the maintainer keeps the canonical docs.

The webmaster is not asked to produce club data; that comes from the mirror pipeline (§20). The long-term operator pattern for coordinating with any external DNS/mail upstream is documented in the maintainers' private operations guide, "External DNS/mail upstream coordination runbook"; this section applies that pattern to the webmaster's specific contract.
...
15. **Zone authority: the move to Route 53 (go-live preparation)**: the webmaster holds and operates the zone today; before go-live the zone moves to Route 53 under IFPA-controlled access (front-matter question 2, decision 16). Because IFPA is the registrant, Julie, as IFPA's representative and the domain technical contact, reclaims the registrar account through the registrar's owner-verification and makes (or has the operator make) the name-server change; no registrar or zone action is asked of Steve. The zone moves to Route 53 early so that every record is staged directly on Route 53 and nothing is hand-applied on the legacy nameservers. After the move the operator manages the records, and `www` and the bare domain are served as alias records to the platform's CloudFront distribution at the switch, with no separate redirect server needed (Route 53 serves a bare-domain alias; bind9 could not). The legacy bind9 service goes dark at the cutover.
...
17. **Records-actor (the operator, on Route 53)**: the zone moves to Route 53 early (item 15), so the operator applies every record there — the ACM validation CNAMEs for both certificates (early and permanent; they also drive renewals), the SES DKIM CNAMEs, the custom MAIL FROM records (`mail.footbag.org` MX and SPF TXT, needed for DMARC alignment), the SPF amendment authorizing the SES and Google senders, the DMARC record with its report mailbox, the Google MX when the mail step runs, and the go-live switch itself (`www` and the bare domain to CloudFront, and `archive.footbag.org` to the archive distribution). No records are hand-applied on the legacy nameservers. Per-record timing and verification commands are the §29.12 record inventory.
...
19. **Reachability protocol**: DNS is operator-made on Route 53, so no delegate with standing DNS access is needed. What is agreed instead: the go-live window, and reaching Steve through it for the write-freeze and the final member export (front-matter question 4). His host power-down follows on his own schedule afterward.
...
27. **Tournament in a box**: see §28 "Tournament in a box". The tool is rebuilt natively after go-live; its requirements come from the existing AngularJS frontend and the Worlds organizers (Julie confirms which functions they use and what data they need), not from a webmaster write-up. The legacy software itself is never carried forward, and nothing is asked of Steve here.
...
31. **Legacy retirement milestones**: there is no long-lived parallel role and no standing legacy server after go-live. The operator's DNS cutover takes the legacy site dark; after go-live verification Steve powers down the remaining legacy production services on his own schedule, leaving only the encrypted recovery and reference artifacts (item 30). This teardown is off the go-live critical path. Anything needed later is built natively (complete by Worlds 2027), never kept running on legacy.
...
| PC7 | Production-first-admin SSM-token route lands per DD §2.9 | DD §2.9; the maintainers' private operations guide, "Production first-admin bootstrap" | State 3 → State 4 |
| PC9 | TRUST_PROXY hop count verified against the live chain (CloudFront → nginx = 2 in both staging and production under the clean cutover; no legacy front-door hop): env value matches, and `req.ip` resolves to the real client under a spoofed `X-Forwarded-For` probe | the maintainers' private operations guide, "Configuration, Secrets, and Key Management" | State 3 → State 4 |
...
**No automated rollback** is provided after the DNS cutover. Path B is operator-driven via the runbook in the maintainers' private operations guide.
...
2. Which functions the organizers actually use, how often, and what data they need after an event — Julie confirms with the Worlds organizers.
...
Allocation (decided): not in v1. Tournament-in-a-box is completely replaced by a native tool built after go-live, in time for Worlds 2027; it is not kept on a live legacy host. The native version is scoped from the existing frontend and backend plus Julie's confirmation with the Worlds organizers; no write-up or registration export is requested from Steve unless that confirmation establishes a specific need. Worlds 2026 runs on the legacy registration software (Decision 21), which then retires.
...
Gate: host-side SQLite backup producer runs on a schedule, ships to S3, and emits `BackupAgeMinutes`; a full restore drill has been rehearsed end-to-end on staging; a production-data restore drill has been completed against a copy of production data with recovery time meeting the RPO/RTO targets in the maintainers' private operations guide, "Recovery objectives by scenario"; the backup-age CloudWatch alarm (`enable_backup_alarm = true`) is enabled and has emitted a non-alarm state; the cross-region DR bucket has Object Lock configured per the maintainers' private operations guide, "Nightly cross-region DR sync" (Object Lock can only be enabled at bucket creation, so retrofitting requires bucket recreation). Recovery targets: 5–10 min RPO, ~5 min RTO per the maintainers' private operations guide, "Recovery objectives by scenario". Procedure: `docs/DEV_ONBOARDING.md` §7.4 (setup); the maintainers' private operations guide, "Backup, Restore, and Disaster Recovery" (runbook).
...
- Snapshot is written to the cross-region DR bucket (S3 Object Lock per the maintainers' private operations guide, "Nightly cross-region DR sync") under a path distinct from the routine backup stream, so the snapshot is not aged out by the routine retention policy.
...
- A dry-run restore against staging has been completed end-to-end within the past 7 days, using the same restore procedure that will be invoked for path B. The dry-run runbook is written into the maintainers' private operations guide before the drill and includes the steps to re-point the app at the restored DB. Serial dependency: the runbook must exist before the dry-run, and the dry-run must succeed before cutover; schedule both with weeks of lead time, not days.
...
Under the clean cutover the apex and `www` move to the new platform at go-live: the operator switches both records on Route 53 to the CloudFront distribution and the legacy site goes dark. Both the ACM certificate issuance procedure (the maintainers' private operations guide, "ACM certificate for footbag.org runbook") and the DNS cutover procedure (the maintainers' private operations guide, "DNS cutover sequence runbook") apply at cutover.
...
- **TTL verified low**: at T-48h the webmaster's drop of the apex/`www` TTL to 60 seconds is observed from his authoritative nameservers, the prior TTL is recorded, and the prior TTL has fully expired before T-0 (the runbook is the maintainers' private operations guide, "DNS cutover sequence runbook").
...
**Record inventory.** The complete list (§19 item 17). The zone moves to Route 53 early, so Terraform owns the records and the operator applies each on Route 53, confirming with `dig`; nothing is hand-applied on the legacy nameservers.
...
**Zone move to Route 53 (go-live preparation, §19 item 15):** the zone moves before go-live, early enough that all records are staged directly on Route 53. A zone snapshot is taken first so MX, DKIM, SPF, DMARC, and validation CNAMEs copy faithfully into Route 53; Julie (IFPA representative and domain technical contact) reclaims the registrar and the name-server change points the domain at Route 53; resolution is verified against Route 53 before any cutover step depends on it; and the apex and `www` are served as Route 53 ALIAS records at the switch. The generic DNS cutover procedure (the maintainers' private operations guide, "DNS cutover sequence runbook") governs the move. The legacy bind9 service goes dark at the cutover, and DNS authority sits under IFPA-controlled access from the move onward.

### 29.12a Legacy host after cutover (shutdown)

After cutover the legacy host serves nothing. The operator's DNS cutover is what darkens the site; after go-live is verified Steve powers down the remaining legacy services on his own schedule, off the critical path (§19 item 31). Recovery relies on the platform's own snapshot and the encrypted artifacts (§27, §19 item 30), not on a standing legacy server. No legacy subdomain stays live (§19 item 16).
...
**Email cutover step.** The SES sending setup is verified before cutover (SPF amendment + SES DKIM) and the inbound role-address receiver is settled (§19 item 24), gated by the confirmed inventory (§19 item 21) and per-list dispositions (§19 item 26). Skeleton (detailed operator click-paths live in the maintainers' private operations guide once the step is executed):
...
Usage discovery (Julie's community-requirements discovery supplies the facts; disposition of any IFPA-owned list function or data is an IFPA governance decision, not a webmaster or maintainer one):
...
Procedure: Terraform provisioning and end-to-end test runbook live in the maintainers' private operations guide.
...
1. Operator secret hygiene: every runbook that writes a secret to SSM or a host file uses the `file://`/temp-file (or stdin) transport, and pasted-key steps suppress shell history, so no live secret reaches process argv or shell history (the maintainers' private operations guide, "Stripe key and webhook-secret rotation runbook", "Safe Browsing API key rotation runbook", "Production first-admin bootstrap", and "Turnstile key rotation runbook"; DEV_ONBOARDING §8.10/§8.15/§8.16).
2. The workstation runtime AWS profile for the production role carries `mfa_serial`, so a stolen at-rest `~/.aws/credentials` cannot assume the role without the operator's MFA device (the profile follows the chained-AssumeRole pattern in the maintainers' private operations guide, "Operator-workstation staging readiness smoke test", which documents the staging profile where `mfa_serial` is intentionally omitted; the production profile adds it).
3. Source-profile and CloudWatch-publisher access keys are on a verified 90-day rotation cadence and named in the quarterly access review (the maintainers' private operations guide, "Source-profile access-key rotation" and "Quarterly routine tasks"); reconciles with §29.4.
4. Break-glass has contemporaneous second-operator notification, a CloudTrail/CloudWatch alarm on privileged-role AssumeRole, and bounded auto-revocation of any console-granted temporary access (the maintainers' private operations guide, "Break-glass access").
5. SES feedback webhook auth reviewed: the `SES_FEEDBACK_WEBHOOK_KEY` query-string transport is accepted only with CloudWatch access-log read narrowed to System Administrators, or replaced by a request-body HMAC (the maintainers' private operations guide, "SES feedback loop activation").
