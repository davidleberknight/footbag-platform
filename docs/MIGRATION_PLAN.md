...
**Email sequencing:** the platform's SES sending is verified before cutover (the apex sender policy replaced rather than extended, SES DKIM applied), and Google inbound is provisioned with the MX repointed as a discrete step ahead of the web cutover, so no inbound is lost. Contact addresses printed on the new site (`admin@`, per §29.8) resolve on Google.

**What the new platform already has (built and working):** outbound transactional email via SES (verification, password reset, claim links, receipts, reminders); the outbox drain that carries every send, including the paced release of bulk mail, which fills each pass with transactional rows first and gives bulk only a capped remainder, stops the bulk stream by itself when the recent bounce or complaint rate is at or above threshold, and offers an operator stop scoped to bulk alone; the mailing-list tables and eight seeded lists, with the one-click unsubscribe path; Terraform for SES domain identity, DKIM, SPF, DMARC (the Terraform writes these records into Route 53; with the early zone move, the operator applies them directly on Route 53, and nothing is hand-applied on the legacy zone). Bounce and complaint handling is built and reads from a queue the worker polls, and standing that queue up per environment is a go-live gate.

**What is not built, despite the tables existing:** the group-roster audience is not implemented, so a group-backed list is refused at the composer until the groups build supplies a roster. Everything else here has landed: administrators create, edit and archive mailing lists and compose sends to them, organizer members compose to the community announce list, and every send writes a broadcast archive row, so the paced release described above can now carry a real member-facing send.
...
6. Restore live `mailto:admin@footbag.org` in `/legal`: swap the `.contact-pending` span used in Privacy, Terms, and Copyright contact lines for a live `mailto:admin@footbag.org` once the mailbox behind it is genuinely reachable. The trigger is the email cutover, meaning Google inbound provisioned and the address tested end to end from outside, not the outbound sender cutover: the platform accepts no inbound mail, so verifying that it can send says nothing about whether anyone can write to `admin@`. A named reader with an agreed answering expectation is part of the same precondition, since a live link to an unmonitored mailbox is worse than the inert notice it replaces. Template-only change; no service or DB work.
