...
- (`guard-find-exec.sh`) hard-denies `find` command-execution predicates (`-exec`, `-execdir`, `-ok`, `-okdir`), including quote- and backslash-obfuscated spellings such as `find . '-exec' …`;
- (`guard-leading-cd.sh`) hard-denies a command that begins with `cd` or `pushd` (including just inside an opening subshell paren), so a leading directory change is rewritten away instead of tripping the built-in `cd`-plus-redirect prompt;
- (`guard-shell-loop.sh`) hard-denies a command carrying a shell loop (`while`/`for`/`until` … `do`) at command position, so a hand-rolled loop is rewritten as simple statically-analyzable commands or the Grep/Read tools instead of tripping the built-in prompt for a command it cannot statically analyze; a loop keyword that is only an argument, inside quotes, or unaccompanied by a `do` (`grep -w for`, `echo "a; while b"`) is not matched, and a loop inside a committed script is invisible here because the command names the script, not the loop;
- (`guard-process-substitution.sh`) hard-denies a command carrying process substitution (`<(…)` or `>(…)`), so a comparison written as `diff <(…) <(…)` is rewritten as the Read tool or as two plain commands writing to stdout, instead of tripping the built-in prompt for a command the analyzer cannot statically decompose; the match is the bare two-character sequence, so a quoted occurrence is denied too, because a guard that parsed shell quoting to tell those apart is how guards acquire bypasses and a false deny costs only a rewrite;
