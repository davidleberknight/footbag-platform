...
Rules are the workhorse of the harness: nearly every line of code Claude writes here is governed by one. `.claude/rules/` holds a convention file per application layer: controllers, services, views, templates, the database layer, database write safety, adapters, tests, code comments, secret handling in scripts, and the Terraform trees — plus `private-repo.md` for the authorized-only private operations checkout. The production service units are deliberately outside that set: they are short, they are reinstalled from the release tree by every deploy, and their contract is stated in the design decisions and the private operations reference rather than in a path-scoped rule. Each carries a `paths:` glob, so it attaches exactly when a file in its layer is opened and costs no context before that.
...
**The load-bearing distinction:** canonical documents describe design intent and are timeless; the private tracker is the one place implementation status lives. A deviation, a current-versus-target gap, a completion note, or a dated status line belongs in a tracker issue, never in a canonical document. The same rule reaches backwards as well as forwards: a ruling that has been reversed is deleted rather than marked superseded, because a reader who meets both the old position and the new one has to work out which of them binds. `.claude/rules/doc-governance.md` enforces both, preventing the failure mode where documentation rots into stale status, or into a debate with itself, that the agent then trusts. Keeping status out of the repository entirely also keeps it out of always-loaded context: the tracker is a pay-per-read source, fetched only when a task needs scope or deviation state.
...
**Deny beats ask when a rewrite exists.** A guard has three outcomes, and choosing between them is a policy, not a per-hook quirk. *Allow* when the command is provably inert. *Ask* when it genuinely writes or reaches something, because that is the human's decision. *Deny* when the command is not statically analysable but an equivalent analysable form exists — a leading `cd`, a shell loop, process substitution, a redirect target written as a variable. A denial there costs only a rewrite, while an ask costs a human interruption on work that was never theirs to adjudicate. This is what keeps read-only research from ever reaching a prompt: it either proves itself inert, or it is sent back to be written so it can. Never add an ask for a command the agent could simply express differently.
...
- (`guard-db-destructive.sh`) guards destructive database commands;
- (`guard-dangerous-git.sh`) guards any git command carrying a command-running or file-writing flag (`--upload-pack`, `--receive-pack`, `--exec-path`, `--open-files-in-pager` run a command; `--output` writes a file), so a statically-allowed `git fetch` cannot shell-exec via `--upload-pack` unprompted. It also names the work-discarding verbs (`reset --hard`, `clean -f`, `checkout --`, `restore`, `branch -D`), which the git-mutation block above now denies outright, so those never reach a prompt; the ask stays as the layer that would still hold if that block were ever narrowed;
- (`guard-find-exec.sh`) hard-denies (deny-beats-ask, above) `find` command-execution predicates (`-exec`, `-execdir`, `-ok`, `-okdir`), including quote- and backslash-obfuscated spellings such as `find . '-exec' …`;
- (`guard-leading-cd.sh`) hard-denies (deny-beats-ask, above) a command that begins with `cd` or `pushd` (including just inside an opening subshell paren), so a leading directory change is rewritten away instead of tripping the built-in `cd`-plus-redirect prompt;
- (`guard-shell-loop.sh`) hard-denies (deny-beats-ask, above) a command carrying a shell loop (`while`/`for`/`until` … `do`) at command position, so a hand-rolled loop is rewritten as simple statically-analyzable commands or the Grep/Read tools instead of tripping the built-in prompt for a command it cannot statically analyze; a loop keyword that is only an argument, inside quotes, or unaccompanied by a `do` (`grep -w for`, `echo "a; while b"`) is not matched, and a loop inside a committed script is invisible here because the command names the script, not the loop;
- (`guard-interpreter-writes.sh`) hard-denies (deny-beats-ask, above) a command that hands an interpreter an inline program which writes a file: a heredoc, or a `-c` / `-e` / `--eval` program, calling `open(…, 'w')`, `writeFileSync`, `write_text`, `File.write`, `shutil.move`, Perl's `open` with a `>` mode, or a similar write. The rule it enforces is that files are edited through the Edit and Write tools, where a change gets a diff preview and review, and an interpreter fed its program inline writes without any shell redirect for the redirect guard to see. All three conditions must hold, so a read-only inline program (parsing JSON, computing line numbers) is untouched, and a committed script under `scripts/` is invisible here because the command names the script rather than the program. The interpreter name is matched with an optional version suffix and the flag with or without a following space, because `python3.11` and `node -erequire(…)` are the same construct as their tidier spellings and a guard that misses them is a guard in name only;
- (`guard-process-substitution.sh`) hard-denies (deny-beats-ask, above) a command carrying process substitution (`<(…)` or `>(…)`), so a comparison written as `diff <(…) <(…)` is rewritten as the Read tool or as two plain commands writing to stdout, instead of tripping the built-in prompt for a command the analyzer cannot statically decompose; the match is the bare two-character sequence, so a quoted occurrence is denied too, because a guard that parsed shell quoting to tell those apart is how guards acquire bypasses and a false deny costs only a rewrite;
...
- (`guard-readonly-bash.sh`) asks on any output redirection to a real file (`> file`, `>> file`), whatever read-only-looking head precedes it, because a static allow for that head (`cat`, `egrep`, `date`, a read-only `git` subcommand) would otherwise auto-approve the write; the check is deliberately head-agnostic (it strips the exempt targets and asks if any redirect remains) so it cannot drift out of sync with the allow list, and spacing is irrelevant (`>out` gates exactly as `> out`). The discard device (`/dev/null`) and the session scratchpad directory (`/tmp/claude-*/…/scratchpad/`) are exempt; a bare `/tmp/claude-*` path outside the scratchpad still gates. A target written as a shell variable is hard-denied rather than asked (deny-beats-ask, above): the guard cannot prove where `"$VAR/file"` lands, and an unprovable destination is a command to rewrite, not a decision to put to the human. Spelled literally it gates on its own merits;
- (`guard-rm.sh`) auto-approves an `rm` that deletes only files under the session scratchpad directory, and asks for every other `rm` (so scratchpad cleanup does not prompt while real deletions still gate);
- (`guard-full-suite-vitest.sh`) asks to confirm a full-suite `vitest run` invoked directly, which would bypass the smoke/e2e/dev excludes baked into `npm test` and pull in the live-AWS and local-stack tiers that run only on explicit instruction;
- (`allow-readonly-bash.sh`) auto-approves genuinely read-only Bash, with deny and ask decisions from the guards always beating this allow — and the approver itself refusing any write- or exec-capable form rather than relying on a neighbour guard to catch it.

A Stop hook (`guard-question-quality.sh`) blocks low-quality questions to the human.
...
**Fail mode is chosen per hook:** security gates fail closed and deny on doubt. The read-only auto-approver only ever grants: it emits allow for a command it can prove read-only and otherwise stays silent so the normal prompt decides. How it earns that proof:

- **Segment and vet every head.** It splits the command into segments and checks each head against a read-only allowlist, stripping control keywords, grouping, and negation so a command hidden behind them (`( rm x )`, `! rm x`) is still checked, and stripping read-only wrappers (`timeout`, `nice`, `command -v`) so the wrapped command is what gets vetted. Command-runner heads that execute their argument (`env`, `command CMD`) are not treated as read-only, and a head is rejected when an exec or write flag rides mid-arguments where a prefix rule cannot see it (`git --output`, `git ls-remote --upload-pack`, `rg --pre`, `sed -i`, a second file operand).
- **Refuse a read-only-looking head that can still act.** `find` carrying any action predicate (`-exec`/`-delete`/`-fprint`, matched after stripping quotes and backslashes so `find . '-exec' …` cannot slip through); `sort -o`, and `sort --compress-program`, which runs an external program on sort's temp-file spill; `date -s`; `hostname <name>`; a mutating `git reflog` subverb (`expire`/`delete`, gated because `reflog`'s read-only `show`/`list` forms keep it on the read-only git list); `git fsck --lost-found` (the one `fsck` form that writes, into `.git/lost-found`, gated so plain read-only `fsck` stays approvable); a `sed` that writes or executes. It drops `printenv`, which would dump the whole environment, and it defers on bash 5.2 function substitution (`${ cmd; }`).
- **Admit a few richer tools only in provably inert forms.** Read-only `git` and `gh` by an allow-list of read-only subcommands, accepting `git -C` only for a literal in-project target — a relative path (the working directory is the repo root) or an absolute path at or under the project directory, the gitignored companion-checkout symlinks included by their bare in-project name (the form the root `CLAUDE.md` prefers over a leading `cd`; a `-C` target given as a shell variable, a `..`/`~` path, or an out-of-tree real path cannot be proven in-project, so the approver stays silent and the command prompts). `sqlite3` only as an inline query on a read-only open (the `-readonly` flag, or a `file:...?mode=ro` URI that names no writable or alternate mode) with no shell-exec or file-write dot-command, no `ATTACH`, no write statement, and no value-setting `PRAGMA` — each refused by the approver itself rather than trusting the engine or a sibling guard. `curl` only as a loopback health probe that discards its body (`-o /dev/null`, with no mutating method, upload, file write, or off-box redirect via `-x`/`--resolve`/`-L`). `unzip` only in a read-only mode (`-p` stream to stdout, `-l`/`-v` list, `-t` test, `-Z` zipinfo) with no `-d` extract target. A redirect to `/dev/null` or the session scratchpad directory (`/tmp/claude-*/…/scratchpad/`, the same area `guard-rm.sh` allows deletes in) is treated as writing nothing.
- **Resolve substitutions fail-closed.** A command substitution is resolved with a quote-aware, paren-balanced scan (so a quoted `count(*)` never moves the boundary), innermost first, accepted only when its inner is a simple read-only command or a read-only `sqlite3` query and passes the same per-head write/exec-flag vetting used at top level — one shared check, so `$(sed -i …)` or `$(git ls-remote --upload-pack=…)` is refused exactly as its top-level form is, rather than head-vetted only. An input process substitution `<(...)` resolves the same way, except its inner may be a read-only pipeline: the inner is split on its unquoted separators and pipes and every piece must be a simple read-only command. An output `>(...)` writes and is never accepted. A nested read-only substitution resolves one layer per pass. The approver stays silent on a non-read-only inner, arithmetic, a subshell, backticks, a write redirect, or an unquoted separator (a newline included) inside the inner, and it fails closed on any ANSI-C `$'…'` quoting whose escape rules would desync the quote scan.
- **Track quoting exactly.** When it neutralizes separators inside quotes it honors backslash escaping, so a backslash-escaped quote cannot smuggle a live command past the segment split, while a backslash-escaped separator (literal text to bash wherever it sits) is neutralized rather than splitting a phantom segment. Quoted strings are tracked across newlines — an inline SQL query or a `node -e` script spanning lines is one argument, not several commands — and both this approver and the redirect guard read a literal `>` inside quoted argument text (an SQL `<>` comparison, a JavaScript arrow function) as text, not as a write redirect.
- **Stay safe alone.** Because deny and ask always beat allow, a write that slips past it is still caught by its guard — and because the approver also runs inside subagents (where, on some client versions, a sibling guard may not fire), it is kept conservative enough to be safe as a lone approver.
...
- Wildcard allows are limited to read-only command heads (the first word of a command) plus a short list of deliberate development conveniences: `mkdir`, `kill` and `pkill` for dev-server lifecycle, `git fetch`, `npm run build`, the local test tiers that touch nothing outside the repo (`npm test`, and `npm run test:unit` / `test:integration` / `test:coverage` / `test:strong-hash`), the two direct tool invocations behind those same tiers (`npx tsc -p tsconfig.json --noEmit` as one exact spelling, and `npx vitest run ...` in its run form only, so watch-mode `npx vitest` still prompts and a full-suite run still meets the full-suite guard), and a short reviewed list of repo scripts (the harness self-check, the hook-fixture suite, the generated-content check, and the e2e port-reclaim helper). The private ops repo's read-only tracker viewer carries a rule of the same shape, but is not reliably auto-approved: the companion checkout resolves outside the project tree, so the product's own path check still prompts, and no portable repo rule can cover a per-developer path. Tracker reads therefore go through the read-only `gh` forms, which the approver grants on their own merits. The tiers that reach outside the repo are deliberately not auto-approved and so prompt: the live-AWS `npm run test:smoke`, the browser end-to-end `npm run test:e2e*`, the heavy pentest tier, and the everything-run `npm run test:all`. Destructive, history-rewriting, package-installing, and service-touching commands sit in ask or deny.
...
- Network reads route through domain-scoped `WebFetch(domain:...)` allows and WebSearch rather than an allowed `curl`. The one auto-approved `curl` is a loopback health probe that discards its body (`curl -o /dev/null http://localhost/…`); every content-returning or off-box `curl` prompts, because content is fetched through WebFetch, not an auto-approved `curl`. `sqlite3` is auto-approved only for an inline read-only query on a read-only open (`-readonly`, or the `file:...?mode=ro` URI), through the approver hook; and read-only `gh` subcommands auto-approve on their own merits. Four `gh` write forms also auto-approve, and only these: editing, commenting on, closing, and reopening an issue, each allowed by an exact prefix that includes the private-tracker repo flag, so the rule matches only when the command is written `gh issue edit -R "$FOOTBAG_PRIVATE_REPO" …` and never covers another repository. That is a deliberate trade: an issue edit is reviewable and reversible, the human's decision to change a card is already the sign-off (the `tracker-ops` skill's one-ask-per-batch rule), and a prompt per card taught nothing but click-through. Filing a new card still prompts, because scope creep is the risk there; deleting an issue or a repository is denied outright; every other `gh` write form still prompts.
...
- `.claude/settings.local.json` — **machine-local, per-developer.** Gitignored, so it is never shared and is absent in CI. It is where clicking "always allow" at a prompt silently writes a rule, so it drifts toward over-permission by construction and is kept nearly empty in practice. It legitimately holds exactly one kind of project-specific content: the `env` block carrying machine-local values that committed text must never name, such as the `FOOTBAG_PRIVATE_REPO` slug of the maintainers' private repository (the hidden-reference rule). A permission rule may stay here only when it names an absolute machine path. Such a rule cannot be committed, since an absolute home path is wrong on every other machine, and moving it to the user-global file below would widen it to every project you open, which is the opposite of least privilege. Anything else is click residue: a shareable, platform-independent rule is promoted to the committed `settings.json`, including a reviewed repo-script allow, and the rest is deleted. Naming a path is not a licence to do anything, so a path-scoped rule still clears the same mutation-capable-head screen the committed allow list faces. The harness self-check enforces both halves: it fails on any rule that names no absolute path, and on any rule carrying a destructive command head however tightly scoped, so a stray always-allow click surfaces on the next self-check run instead of festering; the `env` block is never scanned. Claude never writes this file itself: a `permissions.ask` guard on `Edit` of `.claude/settings.local.json` in the committed `settings.json` routes any such edit through the human, and a Claude-initiated `Write` of the file is banned outright by root `CLAUDE.md` regardless of permission rules, because the always-allow click, not Claude, is its only intended writer.
- `~/.claude/settings.json` — **user-global, per-machine, every project.** The home for a machine-specific rule that genuinely applies across every project: the session scratchpad directory (so the AI's own temp/scratch files never prompt; a Bash redirect that writes into that directory is additionally exempted by the read-only approver and the redirect guard, and an `rm` confined to that directory is auto-approved by `guard-rm.sh`) and the memory-store path. These cannot be committed — an absolute home path is wrong for every other machine, and the team-portability rule forbids machine-specific paths in `.claude/`. A machine-specific rule that matters only inside one project belongs in that project's local file above instead, so it is not widened to every repo you open.
...
- the machine-local `settings.local.json`, when present, carries only path-scoped permission rules, none of them with a destructive command head;
