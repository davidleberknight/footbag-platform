...
- The repository assumes PreToolUse and Stop hooks do not fire for a subagent: Anthropic does not document hook behavior inside subagents and it has varied across versions, so the design never relies on it. User-scope settings likewise are not relied on there.
- A subagent's safety therefore rests on the two things it can count on: inherited permission rules and the restrictive tool allowlist in its definition. That is why neither `auditor` nor `researcher` carries Edit or Write; auditor's Bash and researcher's web access are bounded by the inherited permission rules.
...
**Authoring consequence:** every guard a static permission rule can express goes in `settings.json`, because subagents inherit it. Hooks are reserved for the positional cases (a mutating flag later in the argument list) that a static rule cannot express, and are treated as main-session protection only.

**Official grounding:** the documented subagent permission-inheritance behavior. The undocumented status of subagent hook behavior is exactly why the design refuses to depend on it.
...
- Prompts are reduced in only three safe ways: the read-only-Bash auto-approver, narrowly scoped exact allows, and per-developer OS sandboxing that is never committed. The `fewer-permission-prompts` skill prunes an overgrown local allow-list.
