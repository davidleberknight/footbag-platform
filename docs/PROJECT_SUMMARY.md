...
**Migration Plan:** the migration's public architecture reference, covering legacy data migration design (streams, claim flow, auto-link, merge rules, club bootstrap, name model, competition history), operational-readiness gate detail (backup, observability, edge security, IAM, email ops, maintenance jobs, secrets rotation, pre-cutover reverts), and the pipeline validation gates that govern the data load. The go-live gate index, the cutover state machine, the launch sequencing, and the stakeholder coordination live in GO_LIVE_PLAN.md (private GitHub repo). The canonical design docs above describe the long-term design; this one describes how the project transitions from where it is today to production launch.
