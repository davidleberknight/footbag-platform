...
- `getMediaHubPage() -> {PageViewModel<MediaHubContent>}` — public; lists FH-owned named-gallery URL bookmarks at `/media`; filters `members.is_system = 1` on `member_galleries.owner_member_id`; per-gallery item count is computed by tag-AND match against `member_gallery_tags` minus items carrying any tag in `member_gallery_exclude_tags`; criteria-tag display values rendered as pills
- `getNamedGalleryPage(galleryId, rawPage) -> {PageViewModel<NamedGalleryContent>}` — public; backs `/media/<gallery_id>`; reads the `member_galleries` row plus its `member_gallery_tags` and `member_gallery_exclude_tags` sets, then runs a tag-AND query so an item appears iff it carries every criteria tag and no exclude tag (per `docs/USER_STORIES.md` §V_View_Gallery); filters `moderation_status = 'active'`, `is_avatar = 0`; throws `NotFoundError` for unknown or non-FH-owned galleries (anti-enumeration); item ordering follows `member_galleries.sort_order` (`upload_desc` default, `upload_asc`, `caption_asc`); video tiles render via the canonical `VideoMedia` shape produced by `expandVideoFromMediaItem` (`videoPlatform` + `videoId` + `videoUrl` + `videoEmbedUrl` + `thumbnailUrl` + `videoTitle`), consumed by `partials/video-facade.hbs`
- `listOwnedGalleries() -> {CuratorGallerySummary[]}` — admin; backs `GET /admin/curator/galleries`; returns every FH-owned named gallery with name, description, sort_order, criteria-tag set, exclude-tag set, and item count (tag-AND match minus excludes)
- `getGalleryForEdit(galleryId) -> {CuratorGalleryEditView}` — admin; backs `GET /admin/curator/galleries/:id/edit`; throws `NotFoundError` for unknown or non-FH-owned galleries
- `updateGallery({adminMemberId, galleryId, updates}) -> {void}` — admin; backs `POST /admin/curator/galleries/:id/edit`; single transaction UPDATE of `member_galleries` metadata plus DELETE-then-INSERT of `member_gallery_tags` and `member_gallery_exclude_tags`; validates name (required, max 150 chars), description (max 1000), sort_order (`upload_desc` / `upload_asc` / `caption_asc`), criteria tags (non-empty, valid pattern, no duplicates), exclude tags (valid pattern, no duplicates, no overlap with criteria); throws `ValidationError` on bad input, `NotFoundError` on unknown gallery; new tags are auto-created in `tags` (freeform; no `#curated` restriction since galleries can use it as criteria)

**Authz:** Upload/submit: Tier 1+. Edit tags/delete own: owner. Flag: Tier 1+. Admin delete: admin. Gallery editing (`listOwnedGalleries`/`getGalleryForEdit`/`updateGallery`): admin. Gallery viewing: public.
...
**Persistence Touchpoints:** `media_items`, `media_tags`, `tags`, `audit_entries`, `members` (read for system-member resolution); `/curated/{category}/*.meta.json` filesystem (URL-reference sidecars; source of truth, see DD §1.13).
