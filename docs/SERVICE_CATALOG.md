...
Owns account entry and authentication flows (registration, email verification, credential verification, password change and reset, legacy archive passthrough JWT) and the legacy-migration claim surface (legacy-account claim via two-step emailed-token flow with email-equality fast path, direct historical-person claim, auto-link classification, and the unified link-history wizard view-model). Does not own member profile CRUD, historical-person reads, tier calculation (delegates to `MembershipTieringService`), data exports, session-cookie HTTP glue (controllers set and clear cookies; the service returns a signed JWT string), club lifecycle, or club-leader promotion beyond bootstrap confirmation.
...
Unified linking surface: every post-verify funnel branch (high/medium classifier confidence, low confidence, no match) lands on one page that mixes legacy_members + historical_persons + back-linked "both" cases into a single candidate list with neutral provenance labels, plus a manual-id input that tries both tables in sequence. High/medium classifier output appears as a one-click "This is me" card at the top; low-confidence registrants see a banner plus the same candidate list; no-match registrants see an empty list plus the manual-id input. A "Your clubs (coming soon)" placeholder previews the future `M_Review_Legacy_Club_Data_During_Claim` flow. The standalone `/history/claim` and `/history/auto-link` GET routes are 301 redirects to this page; their POST endpoints stay as form action targets. Profile-edit's CTA points here whenever either link is missing.

Method roster: `registerMember`, `verifyEmailByToken`, `resendVerifyEmail`, `verifyMemberCredentials`, `attemptLogin`, `changePassword`, `requestPasswordReset`, `completePasswordReset`, `generateLegacyArchiveAccess`, `restoreAccount`, `lookupLegacyAccount`, `claimLegacyAccount`, `initiateLegacyClaim`, `peekLegacyClaim`, `consumeAndClaimLegacy`, `lookupHistoricalPersonForClaim`, `claimHistoricalPerson`, `getAutoLinkClassificationForMember`, `getLinkHistoryView`, `manualLegacyClaimRecovery`, `confirmBootstrapLeadership`, `resolveBootstrapLeadership`.
