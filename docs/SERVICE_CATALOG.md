...
Persistence: `club_viability_signals` (read + write for detail page signals), `club_cleanup_resolutions` (owned), `candidate_cleanup_resolutions` (owned), `club_cleanup_claims` (owned), `clubs` (status write on resolution), `legacy_person_club_affiliations` (residue read + de-list write), `club_leaders` (read), `member_club_affiliations` (read), `club_bootstrap_leaders` (read), `legacy_club_candidates` (read + classification/lifecycle cleanup writes), `audit_entries` (append).
