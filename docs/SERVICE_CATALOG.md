...
- `getMediaHubPage() -> {PageViewModel<MediaHubContent>}` — public; lists FH-owned named-gallery URL bookmarks at `/media`; filters `members.is_system = 1` on `member_galleries.owner_member_id`; per-gallery item count is computed by tag-AND match against `member_gallery_tags`; criteria-tag display values rendered as pills
- `getNamedGalleryPage(galleryId, rawPage) -> {PageViewModel<NamedGalleryContent>}` — public; backs `/media/<gallery_id>`; reads the `member_galleries` row + its `member_gallery_tags` set, then runs a tag-AND query so an item appears iff it carries every criteria tag (per `docs/USER_STORIES.md` §V_View_Gallery "Gallery built dynamically based on tag matching"); filters `moderation_status = 'active'`, `is_avatar = 0`; throws `NotFoundError` for unknown or non-FH-owned galleries (anti-enumeration); paginates reverse-chronologically; tile shaping branches on `media_items.video_platform` (`youtube` derives `https://i.ytimg.com/vi/{id}/hqdefault.jpg` and links to `video_url`; `vimeo` uses `thumbnail_url` and links to `video_url`; `s3` keeps the legacy storage-adapter path)

**Authz:** Upload/submit: Tier 1+. Edit tags/delete own: owner. Flag: Tier 1+. Admin delete: admin. Gallery viewing: public.

**Persistence Touchpoints:** `media_items`, `member_galleries`, `member_gallery_tags`, `gallery_external_links`, `media_tags`, `media_flags`, `tags`, `members`, `work_queue_items`, `audit_entries`, `outbox_emails`
