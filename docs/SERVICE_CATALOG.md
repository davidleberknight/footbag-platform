...
**Rate limiting:** rate-limit enforcement (`rateLimitHit` plus `throw new RateLimitedError(...)`) lives in the owning service method, whatever the bucket keying (per-member, per-admin, per-IP, per-target); controllers supply request-derived key inputs as arguments. Controllers map `RateLimitedError` to HTTP 429 with `Retry-After` from `retryAfterSeconds`; the app-level error handler supplies that mapping for any controller that does not re-render a form. Never implement rate limits in a per-route blanket middleware. All bucket sizes and windows come from `system_config_current` keys.
...
| `MemberService` | Member-account page shaping; soft-delete; row-level PII erasure primitives (`purgeAccountPII`: one transaction clearing credentials/contact/links/anchors, HoF/BAP display preserved; `scrubDeceasedMemberPII`: contact-only erasure preserving identity, honors, and links; both append an `erasure_log` row); deceased handling; GDPR export; member search | Login/registration credential check; legacy-claim flow; tier writes; payments | `members_searchable` for search; S3-deletion-before-`deleted_at`; PII purge atomic; deceased and soft-deleted are distinct grace paths; owner-only profile routes | Member controllers; `AdminGovernanceService`; `OperationsPlatformService` (purge) | `src/services/memberService.ts` |
...
| `ClubService` | Club lifecycle (create through archive); flat co-leader management; roster; contactability via co-leaders | Media; payments | SA only (no `deleted_at`); flat equal co-leaders, a member co-leads at most one club, max 5 co-leaders per club; a co-leader may step down even as the last (leaderless tolerated); standard-tag reservation via `HashtagDiscoveryService`; news via `NewsService.emitNewsItem` only | Member controllers; `AdminGovernanceService`; `MemberService` (deceased) | `src/services/clubService.ts` |
...
| `SesFeedbackService` | SES bounce/complaint feedback intake: SNS notification parsing, escalation-only `members.email_status` writes (`bounced` from ok; `complained` from ok/bounced; admin-set `suppressed` never overwritten), flips the member's currently-subscribed `mailing_list_subscriptions` rows to `bounced`/`complained`, per-recipient audit rows | Transport auth (shared-secret query key checked by the IPC controller); SNS subscription confirmation (recorded for the operator, never auto-fetched) | Permanent bounces only (transient bounces change nothing); every processed notification appends `email.bounce_recorded` / `email.complaint_recorded` with a masked address | Public webhook route (`/webhooks/ses-feedback`) | `src/services/sesFeedbackService.ts` |
...
| `AdminClubLeadershipService` | Admin co-leader management: Needs Leader opportunity list (computed fresh on open), add a co-leader from the member base (affiliation created or reactivated), remove a co-leader with mandatory reason, five-co-leader cap with explicit cap-override reason | Member-facing leadership flows (ClubService, onboarding paths 1 and 2); club viability cleanup (ClubCleanupService) | Flat equal co-leaders, a member co-leads at most one club; resolving leadership supersedes the club's provisional `club_bootstrap_leaders` rows; every action audit-logged with before/after and reason | Admin club-leadership routes | `src/services/adminClubLeadershipService.ts` |
...
| `CuratorMediaService` | Admin upload, edit, delete, list of curator-attributed media on behalf of the system member account; named-gallery editing for FH-owned and member-owned `member_galleries`; magic-byte and format validation; ffmpeg curator transcode | Member-attributed uploads | `uploader_member_id` is always the system member id (admin actor recorded only in audit); curator video bytes use `video_platform='s3'` (member video routes reject this as defensive boundary); auto-applies `#curated`; rejects `#by_*` from input on gallery edits (uploader-attribution is system-managed at upload time); FH-owned gallery JSON sidecars at `/curated/galleries/<slug>.json` are the pre-go-live authoring source (the persistent DB is source of truth after go-live, per DD §1.13); member-owned galleries auto-prepend `#by_<owner_slug>` to criteria tags | Admin controllers; `CuratorSeedService`; member gallery controllers (member-owned editing path) | `src/services/curatorMediaService.ts` |
...
Owns club lifecycle (create, edit, activate/deactivate, archive), flat co-leader management, roster management, and contactability (a club is reachable through its co-leaders). Does not own media or payments.

Required patterns: SA only (no `deleted_at` on `clubs`; use `clubs_all` for archived queries); a club's leadership is a flat set of equal co-leaders (`role='co-leader'`), a member co-leads at most one club, max 5 co-leaders per club; a co-leader may step down or leave even as the last one, leaving the club leaderless (tolerated); no club-level contact field, so a club is reachable through its co-leaders' member-visible contact emails; removing another co-leader is an admin action; standard hashtag reserved via `HashtagDiscoveryService.reserveStandardTag()` at creation, permanent and not HD; club display names are not required to be globally unique (the hashtag is the canonical identifier); a club with zero co-leaders surfaces on the single low-priority admin "could use a leader" list; a successful leadership claim returns a club of any status to `'active'` (a new current affiliation does the same for an inactive club); each co-leader's contact email is member-visible by role; co-leader identities on the club detail and country pages are member-visible (the anonymous public receives no co-leader names); news items emitted via `NewsService.emitNewsItem` only.
...
Owns club viability evaluation (`crowdsource_club_viability` predicate with G1-G4 gates per `A_Periodic_Club_Cleanup`), the `leaderless_active_club` and `stale_provisional_leader` predicates, admin club cleanup queue page shaping, the admin-home backlog badge (open-item count plus oldest-item age, computed from the same queue assembly), candidate cleanup actions (demote to dormant, archive, confirm junk, return junk to dormant; guarded writes make a concurrent repeat a no-op), candidate defer (an unpromoted candidate leaves the queue for 30/90/180 days and re-surfaces with a deferred-by annotation), concurrent-admin claim markers (non-blocking claimed-by hints, auto-released on any resolve action, stale after 30 minutes), cleanup resolution tracking (defer/dismiss/demote/archive), de-listing of unconfirmed legacy residue (`legacy_person_club_affiliations` 'pending' rows to 'former_only', per club and cascaded on demote/archive), the candidate-flag queue group (wizard activity answers about unpromoted candidates, grouped per candidate with one vote per member; resolved by terminal dismiss or defer, independently of the candidate's promotable item), and group-level bulk actions (bulk defer across a predicate group or the candidate-flag group with one shared reason and one audit row per item; bulk de-list across all residue clubs, each club in its own transaction). Predicates are evaluated on demand when an admin opens the queue; there is no unattended background process. Does not own club lifecycle (ClubService) or signal collection during onboarding (MemberOnboardingService writes signals).

Required patterns: signal aggregation queries derive S1/S2/S3/L1/O1 from `club_viability_signals`, `legacy_club_candidates`, `club_leaders`, and `member_club_affiliations` at evaluation time. Resolutions tracked per club-plus-predicate in `club_cleanup_resolutions` and per candidate in `candidate_cleanup_resolutions` (UNIQUE constraints); deferred items reappear after expiry. Queue items name the members whose latest activity answer was negative; signal authorship renders only on this admin surface, never publicly. All resolutions audit-logged.

Persistence: `club_viability_signals` (read: club-keyed rows feed the gates, candidate-keyed rows feed the candidate-flag group), `club_cleanup_resolutions` (owned), `candidate_cleanup_resolutions` (owned), `club_cleanup_claims` (owned), `clubs` (status write on resolution), `legacy_person_club_affiliations` (residue read + de-list write), `club_leaders` (read), `member_club_affiliations` (read), `club_bootstrap_leaders` (read), `legacy_club_candidates` (read + classification/lifecycle cleanup writes), `audit_entries` (append).

Side effects: audit append per resolution and per residue de-list; claim markers are deliberately un-audited.
...
Required patterns: contact requests persist as `work_queue_items` rows with `queue_category='membership'`, `task_type='member_contact_request'`, and `entity_type='member'`. Categories are an enumerated TypeScript constant (`display_name_correction`, `profile_url_correction`, `tier_status_question`, `identity_link_issue`, `other`). Decision labels are an enumerated TypeScript constant (`corrected`, `denied`, `duplicate`, `out_of_scope`); no thread or clarification loop in the first cut. Per-member open-count cap of 3 enforced via `workQueue.countOpenForMember`; the 4th submission throws `RateLimitedError`. Message body length capped at 2000 characters; resolution note length capped at 500. The submission audit row records the category and the message length only; raw member-authored text stays out of the append-only ledger, which is exempt from PII purge. The operational summary is `work_queue_items.reason_text` (category label + first 200 chars); the full message body is held in the purgeable `work_queue_items.detail_text` column (cleared by account erasure and the deceased contact scrub). Resolution emails dispatched via `SesAdapter.sendEmail` with the decision-label-keyed subject and a templated reply body (decision plus admin note) that does not echo the member's original message. Categories enum stays in TypeScript until shape stabilizes; DB CHECK constraint deferred.
...
Required patterns: state transitions are optimistic-locked via `UPDATE...WHERE...state=expected` so two simultaneous claimers cannot double-process. `getJobForAdmin` returns `null` for both unknown jobs and jobs owned by another admin so the surface cannot be enumerated. `markSucceeded` requires a non-null `mediaId` (DB CHECK). Boot-time recovery is the only sweep: it resets expired-lease `processing` rows and re-dispatches parked `pending_transcode` rows with `retry_count > 0`. Other transitions are HTTP-event-triggered, except the worker's immediate in-process re-claim after a retry-eligible failure. Idempotent re-dispatches are filtered at `claimForProcessing` (returns `null` if not in `pending_transcode`); duplicate finalize POSTs filtered at `markPendingTranscode` via `ConflictError`.
...
Persistence: `system_job_runs`, `system_alarm_events`, `system_config_current`, `audit_entries`, `reconciliation_issues`, `members` and `erasure_log` (purge-eligibility reads).
