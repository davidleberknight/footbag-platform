...
Task UI shape: each task renders a single page with the task-specific view-model (city (required), region (optional), country (required), DOB (required), competition year, and show-year toggle for `personal_details`; unified legacy-and-HP candidate list with declared-anchor matching for `legacy_claim`; multi-stage club picker for `club_affiliations` with stages 1/2A/2B/wrap-up; simple form for `show_competitive_results`), a `Skip for now` action, and a `Complete and continue` action. Per-card answers persist on form submit; each card has a dedicated submit control (e.g. "Save answers" for `club_affiliations` Stage 1 cards, "Confirm selection" for disambiguation cards). Visual consistency is enforced via the shared wizard layout primitive so all four current tasks render in the same chrome. The `club_affiliations` view-model carries a per-card `confidence` discriminator (`pre_populate` for Stage 2A cards, `onboarding_visible` for Stage 2B cards) so the template renders the stage-appropriate framing and per-card question wording. Stages 2A/2B match at region level (state or province), with same-city candidates first. The wrap-up links to the clubs browse page filtered to the registrant's country for geographic browse and signal submission from club detail pages. Stage 2A's "I'd like to join" path renders an outbound link to the club's join page on the club detail surface; no signal is recorded by the wizard.
