...
| Freestyle glossary | `GET /freestyle/glossary` | `FreestyleService.getGlossaryPage` | Public | `PageViewModel<FreestyleGlossaryContent>`; domain glossary for freestyle terminology rendered from the curated glossary source; §3 ("How Tricks Are Built") carries the compositional-vocabulary terminology inline; the operator-board partial renders on the landing page and `/freestyle/learn` rather than in the glossary, keeping the glossary's scope on terminology and execution detail; §3 also carries an intermediate-operator reference subsection (`id='intermediate-operators'`) rendering ten compound/quantifier/directional operator entries with `id='term-{slug}'` anchors for equivalence-chain deep-linking; §10 (Foundational Tricks) lists eleven irreducible base tricks (clipper, mirage, legover, pickup, illusion, whirl, butterfly, swirl, osis, around-the-world / ATW, orbit); pixie and fairy render in a small set-modifiers subsection below §10 (`id='set-modifiers-tier-1'`) so their `term-{slug}` anchors remain resolvable as set-modifier references rather than as core tricks. | `src/controllers/freestyleController.ts`, `src/views/freestyle/glossary.hbs` |
