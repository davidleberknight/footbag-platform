...
| Freestyle glossary | `GET /freestyle/glossary` | `FreestyleService.getGlossaryPage` | Public | `PageViewModel<FreestyleGlossaryContent>`; domain glossary for freestyle terminology rendered from the curated glossary source. 14-section hierarchical structure (P1 IA refactor): Core Concepts / Contact Surfaces & Delays / Dexterities / Timing & Sets / Core Trick Families / Modifiers & Operators / Jobs & Operational Notation / ADD Accounting / Symbolic Composition / Run Architecture / Family & Topology Concepts / Community Vocabulary / Historical Terms / Sources. CSS-only sticky left-rail sidebar on desktop (≥1024px), horizontal-strip fallback at tablet widths, native vertical stack on mobile. Anchor-preservation forever-rule: all `#term-{slug}` + `#glossary-panel-{term}` + `#connective-panels` + `#traditional-reference` + `#run-quality` anchors stable across the refactor. Beginner / Intermediate / Advanced tier badges retained as soft chips, no longer the primary structural organizer. |
