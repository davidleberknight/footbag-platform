...
| Freestyle observational layer | `GET /freestyle/observational` | `FreestyleService.getObservationalLayerPage` | Public | `PageViewModel<FreestyleObservationalContent>`; observational-layer surface for trick names that appear in external corpora (PassBack, FootbagMoves, Shred Global, Footbag Finland) but have NOT been promoted to canonical curated status. Cards group by external ADD claim (numeric buckets ascending, ADD-unknown bucket last; alphabetical within each bucket). Curator-review state surfaces as a per-card status chip (neutral / accent / muted tones); rejected entries are not rendered. Two-letter source badges (PB / FM / SG / FF / OTHER) carry full source attribution as tooltips. Compact 2-line cards default; a `<details>` expansion surfaces additional readings, formula, curator note, and blockers when present. Strict layer-separation contract: never inlines into canonical surfaces; entries carry no hashtag chips, no trick-detail pages, no media attachments. Sourced from the curator-authored `OBSERVATIONAL_TRICKS` content module; no DB access. Discoverable from `/freestyle/tricks` dictionary landing (Card 5). |
