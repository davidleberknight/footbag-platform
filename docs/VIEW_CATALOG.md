...
| Club detail | `GET /clubs/{clubKey}` (shared `:key`) | `ClubService` (read method per code) | Public | `PageViewModel<ClubDetailContent>`; `clubKey` matches `^club_[a-z0-9_]+$` exactly (no aliasing or fuzzy); leaders section public; `contactEmail` rendered only when `showContact = true` (current leaders shown to authenticated viewers; provisional entries and anonymous viewers never); club contact email and WhatsApp render to authenticated viewers only; service filters non-renderable leaders at read query (not in template); club snapshot section renders pre-shaped vitality strings (known leaders count or `None known yet`; members count or `Unknown`; status label); see §7.9. |
