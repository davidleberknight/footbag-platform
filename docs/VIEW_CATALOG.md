...
| `GET /history/auto-link` | (deprecated) | 301 redirect to the onboarding wizard's legacy_claim task, query string preserved |
...
| `GET /history/claim` | (deprecated) | 301 redirect to the onboarding wizard's legacy_claim task, query string preserved |
...
- State-changing wizard routes (POSTs for `startTask`, `submitTaskResponse`, `skipTask`, `completeTask`) are subject to the Origin-pin middleware per DD §3.3; cross-site POSTs are rejected at 403 before reaching the service.
...
- Skipping a task is a 303 to the next task (or to the dashboard at the end of the catalog) and does not block sign-in.
...
Dashboard task widget: rendered at the member's personal-home view; queries `MemberOnboardingService.getTaskWidget`; shows one row per outstanding task (state `pending` or `skipped`) with a Resume button. The widget is hidden when no outstanding tasks remain.

Exact wizard route paths are an implementation-time decision (candidates: `/register/wizard/<taskType>` mounted post-verify and re-usable as `/members/me/onboarding/<taskType>` from the dashboard). Once paths are pinned, add specific route entries to §5 Public route catalog.
