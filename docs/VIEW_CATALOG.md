...
- Freestyle section: landing wrapper (`.freestyle-landing`), hero with mascot (`.hero-with-mascot`, `.hero-mascot`), tile cards (`.card-tile`, `.format-card`), embedded video wrapper (`.video-embed`), stats strip (`.stats-strip`), trick-dictionary view toggle (`.trick-view-toggle`, `.trick-view-toggle-active`, `.trick-view-toggle-sep`), trick-dictionary coverage summary (`.trick-coverage-summary`, `.trick-coverage-counts`, `.trick-coverage-note`), trick-dictionary ADD-grouped layout (`.trick-add-group`, `.trick-list`, `.trick-row`, `.trick-row--external-placeholder`, `.trick-row-header`, `.trick-name`, `.trick-add-chip`, `.trick-add-chip--unrated`, `.trick-family-chip`, `.trick-category-chip`, `.trick-media-chip`, `.trick-media-chip--tutorial`, `.trick-media-chip--demo`, `.trick-media-chip--missing`, `.trick-media-chip--available` (legacy alias), `.trick-status-badge`, `.trick-notation`, `.trick-notation--missing`, `.trick-description`, `.trick-description--missing`, `.trick-aliases`, `.trick-placeholder-note`), trick-detail pathways cross-link block (`.trick-pathways`, `.trick-pathway-list`, `.trick-pathway`, `.trick-pathway--learn`, `.trick-pathway--watch`, `.trick-pathway--family`, `.trick-pathway--empty`, `.trick-pathway-title`, `.trick-pathway-primary`, `.trick-pathway-secondary`, `.trick-pathway-link`), trick-dictionary sets-grouped layout (`.trick-set-group`, `.trick-set-type`), family-filtered cross-link block (`.related-set-groups`, `.related-set-groups-label`, `.related-set-group-link`, `.related-set-group-count`, `.related-set-group-sep`), trick-detail reference-media split (`.reference-media-subsection`, `.reference-media-subsection--tutorials`, `.reference-media-subsection--demos`, `.reference-media-subheading`).
...
| `GET /history/claim` | (deprecated) | 301 redirect to the link-history wizard, query string preserved |
| `POST /history/claim` | Claim lookup | Legacy account lookup handler (form action target for the wizard's manual-id flow and direct-form POSTs) |
...
| Members welcome | `GET /members` | `MemberService.getMembersWelcomePage` | Public; authenticated and unauthenticated both see the same informational page | `PageViewModel<MemberWelcomeContent>`; `content.tiers[]` iterates the four canonical tier labels (Tier 0 Registered Member, Tier 1 IFPA Member, Tier 2 IFPA Organizer Member, Tier 3 IFPA Director) with the per-tier benefits blurb from `tierBenefitsBlurb`; `content.showJoinCtas` gates the Sign Up and Log In cards (true only when the visitor is unauthenticated). No search affordance: member search is authenticated-only per `M_Search_Members` and lives on the personal home. | `src/controllers/memberController.ts`, `src/views/members/welcome.hbs` |
| Member profile / personal home | `GET /members/:memberKey` | `MemberService.getOwnProfile` (owner, accepts `?q=` for member search) / `MemberService.getPublicProfile` (public HoF/BAP) | Owner OR public HoF/BAP | Owner view (`PageViewModel<OwnProfileContent>`) is the authenticated personal home: composes membership (per `M_View_Tier_Status`: tier badge, benefits blurb, Active Player line for Tier 0 with formatted expiry, underlying-tier text for Tier 3, upgrade-CTA visibility flags), identity (legacy-account and historical-person link state with CTA to the link-history wizard when either link is missing), quick-actions (slug-scoped links to profile-edit, galleries, upload), member search (form posts back to `/members/:memberKey?q=`; results from `searchMembers` reading `members_searchable`; min 2 chars; 20-result cap with `hasMore` and `tooShort` flags; results show display name, country, honor badges only), and coming-soon feature cards for unbuilt M_ surfaces, alongside the existing About / Competitive Results / Account sections. Contact-field visibility via `email_visibility`; non-owner non-HoF/BAP fails closed. Public view (`PageViewModel<PublicProfileContent>`) remains the limited HoF/BAP read-only profile. See §7.1. | `src/controllers/memberController.ts`, `src/views/members/profile.hbs` |
| Member profile edit | `GET /members/:memberKey/edit` | `MemberService.getProfileEditPage` | Owner only | `PageViewModel<ProfileEditContent>`; read-only identity block (name, login email, profile URL); link-history wizard CTA to `/members/:memberKey/link-history` shown when legacy-account or historical-person link is missing, hidden when both are linked; avatar upload inline via multipart form posting to `/members/:memberKey/avatar`; `member_links` max 3 URLs validated as https. | `src/controllers/memberController.ts`, `src/views/members/profile-edit.hbs` |
| Link-history wizard | `GET /members/:memberKey/link-history` | `IdentityAccessService.getLinkHistoryView` | Owner only (slug must equal session member's slug; 404 otherwise for anti-enumeration) | `PageViewModel<LinkHistoryContent>`; one-section page mixing legacy_members + historical_persons + back-linked "both" cases into a single candidate list with neutral provenance labels. Each card carries a `claimMode` discriminator (`auto_link_confirm` | `legacy_claim` | `hp_review_page` | `already_linked`) that drives its action: `auto_link_confirm` POSTs to `/history/auto-link/confirm`; `legacy_claim` POSTs to `/history/claim` with hidden `from=link-wizard` so the response 303-redirects back here with sent state inline; `hp_review_page` deep-links to `/history/<personId>/claim`; `already_linked` renders a read-only badge. Below the candidate list, an "Other ways to find your records" form posts a single identifier to `POST /members/:memberKey/link-history/find` which tries legacy_members then historical_persons. A "Your clubs (coming soon)" placeholder previews the future club-affiliation review flow per `M_Review_Legacy_Club_Data_During_Claim`. Sent-state notice + simulated-email card render inline when `?sent=1` is set; `?nomatch=1` renders an anti-enumeration "didn't match" notice; `?from=register` renders the dashboard skip link; `?reason=low_confidence` renders the low-confidence preamble. "Back to dashboard" and skip links target `/members/:memberKey` (the authenticated personal home, which carries the member-search affordance). The post-verify funnel redirects every classifier branch here ("one place to link"). | `src/controllers/claimController.ts`, `src/views/members/link-history.hbs` |
| Link-history manual-id lookup | `POST /members/:memberKey/link-history/find` | `IdentityAccessService.lookupLegacyAccount` + `legacyClaim.findHistoricalPersonById` | Owner only (404 on slug mismatch) | Form-only endpoint, no view-model. Tries legacy_members first; on `auto_linked` outcome 303-redirects to wizard with `?linked=legacy`; on `enqueued` 303-redirects to wizard with `?sent=1&out=enqueued&since=<n>`; on legacy miss tries historical_persons by id and 303-redirects to `/history/<personId>/claim` if found; otherwise 303-redirects to wizard with `?sent=1&out=no_match&since=<n>&nomatch=1`. Empty identifier 303-redirects to wizard with `?nomatch=1`. Anti-enumeration: legacy-miss-then-HP-match is observably different (different redirect URL) but only fires for unclaimed HPs the user supplied an exact id for, which is the same enumeration surface as `/history/<personId>/claim` already exposes. | `src/controllers/claimController.ts` |
...
| Claim initiation | `GET /history/claim`, `POST /history/claim` | `LegacyMigrationService.initiateAccountClaim` | Authenticated only; redirect to `/login?returnTo=%2Fhistory%2Fclaim` | `PageViewModel<ClaimInitiateContent>`; non-revealing response regardless of lookup outcome; rate-limited per requesting account, per target row, per session/IP. The page-level view-model carries a `skipHref` field rendered when the user arrived via the registration redirect (`?from=register`), and a `lowConfidenceBanner` flag rendered when arrival came via an auto-link drift redirect with `?reason=low_confidence` (the registrant's auto-link classifier returned `low` confidence). The form's POST round-trips the `from` flag via a hidden input so the sent state and error states preserve the skip affordance and banner. The POST handler consumes the service's discriminated outcome: on `auto_linked` (verified `login_email` equals legacy `legacy_email`) it 303-redirects to the member dashboard with the merge already applied; on `enqueued` it renders the sent state with the simulated-email card; on the silent anti-enumeration outcomes (`no_match`, `target_rate_limited`) it renders the same sent banner with an `outcomeNote` shown only in dev (`SES_ADAPTER=stub`) explaining which silent path fired so the operator can distinguish a real send from a no-op. The simulated-email card renders on the sent state in dev and staging-sandbox modes, filtered to `/history/claim/confirm/` URLs and scoped to "this turn" via `sinceIndex` so neither a stale verify-email card nor a prior matching attempt's URL bleeds into the current sent state. See §7.7. | `src/controllers/historyClaimController.ts`, `src/views/history/claim-initiate.hbs` |
...
- `GET /members/:memberKey/link-history`, `POST /members/:memberKey/link-history/find`: auth required, owner-only (slug mismatch returns 404 for anti-enumeration).
