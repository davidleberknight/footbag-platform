...
**Two-surface freestyle model.** Freestyle pages render at two distinct surfaces. The **canonical surface** is the compact symbolic-object primitive documented above — used in dictionary browse, landing Core Tricks, glossary compression-flow, faceted browse views, and search results. The **expanded surface** is the per-trick narrative page at `/freestyle/tricks/:slug` — used for prose descriptions, reference media, execution detail, family relationships, and historical context. Browse cards link compact → expanded. Expanded pages may render the compact symbolic object as a hero element but must not duplicate the browse view as their identity layer. Prose, learning notes, execution walkthroughs, and historical context belong on the expanded surface only; they must not drift onto compact browse cards.
...
| Member account stub | `GET /members/:memberKey/:section` | `MemberService` (per-section) | Owner only | `PageViewModel`; supported sections `media`, `settings`, `password`, `download`, `delete`; unsupported sections fail closed. | `src/controllers/memberController.ts`, `src/views/members/stub.hbs` |
...
| Freestyle landing | `GET /freestyle` | `FreestyleService.getLandingPage` | Public | `PageViewModel<FreestyleLandingContent>`; freestyle-section primitives (`.freestyle-landing`, `.hero-with-mascot`, `.card-tile`, `.page-jump-nav`, `.freestyle-featured-grid`, `.featured-card`, `.video-embed`, `.stats-strip`); renders an under-hero `page-jump-nav` (five anchors: Basic Components, Core Tricks, Featured, Operators, Where next), then the "The Language of Freestyle Footbag" structure (short intro lede + autoplay demo loop), a Basic Components grid (Contact / Set / Dex / Spin / Duck / Delay; Dex carries Direction / Movement type / Support type sub-fields), and a Core Tricks grid of eleven compact symbolic objects (each card: #slug + zero-or-more ≡-readings + optional symbolic notation + ADD; missing rows render `—` with a pending-dictionary-entry footnote). Below: a merged Featured strip (competition-format reference cards and curated demonstration videos rendered as `FreestyleFeaturedItem[]` with `key` / `title` / `caption` / `media` / `tags` in one compact grid; competition-format cards (routine / circle / sick3 / shred30) carry one-line captions and no chips; curated-demonstration cards carry hashtag chip strips shaped via `shapeMediaTagsForBrowse`; empty array collapses the section content with no placeholder cards), the operator-board orientation strip (fourteen Tier-1 operators across set / body / structural tiers; card hierarchy is glyph, name, `INPUT → RESULT` example, short action phrase, icon-only deeplink with `aria-label`; no per-tier intro paragraph), a concise three-link orientation block (`Where to go next`), and the portal-cards grid. | `src/controllers/freestyleController.ts`, `src/views/freestyle/landing.hbs` |
...
| Freestyle glossary | `GET /freestyle/glossary` | `FreestyleService.getGlossaryPage` | Public | `PageViewModel<FreestyleGlossaryContent>`; movement-language primer + traditional reference rendered on the v5 sectional spine. §1 Movement-Language Primer; §2 Contact Surfaces & Delays (foundational / foot / body-region / specialized-variant subsections); §3 Dexterities (Motion Style + Execution Window + Execution Quality clusters); §4 Timing Layers & Sets (inline parabolic-arc SVG `glossary-timing-clock` showing the four-beat clock); §5 Core Trick Structures (`core-tricks-grid` partial with `idPrefix="term-"` preserving the cross-page `term-{slug}` deep-link contract used by `glossaryAnchors.ts`; family-tree visuals for 4 pilot families via `content.familyTrees`); §6 Modifiers & Operators split into **Surface A** "Common Advanced Modifiers" (13 player-facing feel cards in `set` + `body` clusters with a midtime-body sub-cluster note; cards from `content.modifierFeelCards` anchored `id="modifier-{slug}"`) and **Surface B** "Advanced Decomposition & Operator Theory" wrapping the body-modifier reference dl + intermediate-operators dl (`id="intermediate-operators"`, with `OperatorReferenceEntry` rows anchored `id="term-{slug}"`) + execution-mechanics dl (`id="execution-mechanics"`) + set-modifier registry grid (`id="set-modifiers-tier-1"`); §7 Symbolic Notation (thesis + worked examples; Operational Notation lives here as an Advanced Reference subsection `id="operational-notation"` carrying the bracket-flag / sides / directions / sequence-operators / pre-states reference plus the abbreviations subsection split into `trickNames` + `operationalTokens` from `content.abbreviations`); §8 Composition & Decomposition (carries the symbolic-compression worked-example flow `id="symbolic-compression-flow"`; each `≡` reading runs through `shapeNotationDisplay` so operator / side / core-family classification surfaces as role-classified `notation-token` spans rather than plain prose); §9 Movement Topologies (six connective panels from `content.connectivePanels` — paradox / symposium / ducking / spinning / whirl / pixie — at their permanent home with the observational-symbolic-layer badge); §10 Traditional Reference (ADD system + run-quality tiers + competitive formats); §11 Community & Historical Vocabulary (deprecated and folk names). §12 Learning Pathways deferred. Each major section carries a `glossary-tier-badge--{beginner|intermediate|advanced}` chip for orientation. Anchor convention: `term-{slug}` lives on Surface B structural-reference entries (preserving the `glossaryAnchors.ts` deep-link contract used by progression / modifier-family / dictionary inbound links); `modifier-{slug}` is a parallel namespace on Surface A pedagogical feel cards. Editorial discipline: individual and sprint-numbered references stripped from user-visible prose; verb discipline per `exploration/red-consolidation/SEMANTIC_COMPRESSION_DOCTRINE.md` §3 locution levels (Level-1 "is" for expert-ruled decompositions; Level-3 "structurally expandable to" for doctrine-pending readings). | `src/controllers/freestyleController.ts`, `src/views/freestyle/glossary.hbs` |
