...
| Freestyle glossary | `GET /freestyle/glossary` | `FreestyleService.getGlossaryPage` | Public | `PageViewModel<FreestyleGlossaryContent>`; domain glossary for freestyle terminology rendered from the curated glossary source; §3 ("How Tricks Are Built") embeds the operator-board partial as the visual orientation layer for the compositional vocabulary defined inline, plus an intermediate-operator reference subsection (`id='intermediate-operators'`) rendering nine compound/quantifier operator entries with `id='term-{slug}'` anchors for equivalence-chain deep-linking. | `src/controllers/freestyleController.ts`, `src/views/freestyle/glossary.hbs` |
