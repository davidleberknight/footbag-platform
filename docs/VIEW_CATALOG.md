...
| Freestyle landing | `GET /freestyle` | `FreestyleService.getLandingPage` | Public | `PageViewModel<FreestyleLandingContent>`; two-band beginner-first landing. Hero (page title, a short movement-first subtitle, mascot icon). A "What is Freestyle?" section carries the intro paragraphs and an optional curator-owned demo video. A "Start Here" band groups the beginner cards (Watch & Learn, Trick Dictionary, Glossary, The Story of Freestyle); a "Go Deeper" band groups the reference and archive cards (Trick Records, Competition, Scoring & Combos, Operators & Modifiers, Insights, Observed Tricks). A "Featured" video strip (competition formats plus curated demonstrations) follows, and a lower enrichment band renders last: a 12-cell foundational-atom mosaic (one labelled cell per canonical core atom, the set bound to `CORE_TRICKS`), each cell loading a curated loop through the media adapter by the `mosaic-<slug>.mp4` `source_filename` convention and falling back to a labelled empty tile when none exists. Standard `.card` + `.btn-outline` throughout. |
