...
| Freestyle glossary | `GET /freestyle/glossary` | `FreestyleService.getGlossaryPage` | Public | `PageViewModel<FreestyleGlossaryContent>`; domain glossary for freestyle terminology rendered from the curated glossary source; §3 ("How Tricks Are Built") carries the compositional-vocabulary terminology inline (the operator-board partial renders on the landing page and `/freestyle/learn` rather than here); §3 also carries: an intermediate-operator reference subsection (`id='intermediate-operators'`) rendering ten compound/quantifier/directional operator entries with `id='term-{slug}'` anchors for equivalence-chain deep-linking, a torque/mobius symbolic-compression worked example (`id='symbolic-compression-flow'`) rendered with the compact-symbolic-object pattern; each `≡` reading runs through `shapeNotationDisplay` so operator / side / core-family classification surfaces as role-classified `notation-token` spans rather than plain prose, and an Execution Mechanics subsection (`id='execution-mechanics'`) collecting several PassBack-adapted execution-mechanics concepts; §8 is "Symbolic Compression" — the section's lead is the maintainer-mandated thesis sentence "The language evolves by compressing recurring compositional structures into shorter readable symbolic forms." cross-linked back to the §3 worked example; §9 (Operational Notation) opens with a semantic-vs-operational layer-contrast table so the layer boundary is unmistakable; §10 (Foundational Tricks) renders the eleven irreducible base tricks (clipper, mirage, legover, pickup, illusion, whirl, butterfly, swirl, osis, around-the-world, orbit) as a registry-style core-tricks-grid (shared partial with the landing's Core Tricks section; `idPrefix="term-"` preserves the cross-page `term-{slug}` deep-link contract used by `glossaryAnchors.ts`); a set-modifiers subsection below §10 (`id='set-modifiers-tier-1'`) renders a registry grid of nine entries (pixie, fairy, stepping as Tier-1 with glyph chips, plus atomic, quantum, blurry, nuclear, barraging, furious as intermediate entries with `≡` decomposition lines), each carrying a resolvable `term-{slug}` anchor; §13 connective panels carry a positioning note marking them as transitional educational/dictionary bridges pending future relocation. | `src/controllers/freestyleController.ts`, `src/views/freestyle/glossary.hbs` |
