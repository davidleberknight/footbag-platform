...
| Freestyle landing | `GET /freestyle` | `FreestyleService.getLandingPage` | Public | `PageViewModel<FreestyleLandingContent>`; portal landing surface. Hero ("Freestyle Footbag" + portal-framing subtitle + mascot icon), short portal-framing lede paragraph, two-anchor jump nav (#featured + #reference-shelf). Portal-card grid in user-centered order: Tutorials & Learning → Trick Dictionary → Glossary → World Records → Competition → History/ADD/Combo → Insights → Operators & Modifiers (demoted to last and tagged as Advanced Reference). Featured strip carries 4 competition-format cards + 4 curated demonstrations. The Freestyle Reference Shelf below Featured groups six collapsed `<details>` panels (Basic Components, Core Tricks, Operators & Modifiers, ADD & Scoring Basics, Notation Basics, Learning Path), each gated on its own slug-tagged body class; the three legacy standalone sections (Basic Components / Operator Board / Core Tricks) now live inside their respective shelf panels. The operators-modifiers panel additionally surfaces the `/freestyle/operators` + `/freestyle/sets` cross-link block. Get Started tiles render at the bottom. The legacy `.freestyle-language-intro` + `.freestyle-basic-components` + `.freestyle-core-tricks` standalone-section wrappers are retired. Test-pinned: Tier-4 accounting patterns remain absent from the operator board + core-tricks grid (4-tier rendering hierarchy contract preserved). |
