...
**Typography.** One type system across all public pages, including every freestyle surface: body text uses the `--font-body` token (Inter, Helvetica Neue, Arial, sans-serif); code, hashtags, and notation use the `--font-mono` token (ui-monospace, SFMono-Regular, Menlo, Consolas, "Liberation Mono", monospace). Every `font-family` declaration in `src/public/css/style.css` references one of these tokens; no rule names a typeface directly, and no section introduces its own typeface, serif accent, or alternative sans stack.

**Design tokens are required.** Colors, radii, and shadows in CSS rules come from the `:root` tokens. A new visual value enters as a named token first; rules never carry one-off hex literals for values the token set already expresses. Media queries use the canonical breakpoints 480px (phone) and 768px (mid-width); no other breakpoint values.
