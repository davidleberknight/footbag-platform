...
| Onboarding wizard task page | `GET /register/wizard/:taskType` | `MemberOnboardingService` + per-task delegated services | Authenticated only; redirect to `/login?returnTo=...` on unauthenticated visit | `PageViewModel<TaskContent>` keyed by task type. `personal_details`: location, DOB, competition year, and show-year toggle. `legacy_claim`: unified candidate list with declared-anchor matching per §7.7. `club_affiliations`: stage-aware view-model (stages 1/2A/2B/3A/wrap-up). `show_competitive_results`: simple form. Each page renders a `Skip for now` POST and a `Back to dashboard` link to the member's profile. Unknown `:taskType` returns 404. Idempotent `startTaskList` runs on first GET so all four task rows exist. See §7.6a. |
...
| Freestyle landing | `GET /freestyle` | `FreestyleService.getLandingPage` | Public | `PageViewModel<FreestyleLandingContent>`; two-band beginner-first landing. Hero (page title, a short movement-first subtitle, mascot icon). A "What is Freestyle?" section carries the intro paragraphs and an optional curator-owned demo video. A "Start Here" band groups the beginner cards (Watch & Learn, Trick Dictionary, Glossary, The Story of Freestyle); a "Go Deeper" band groups the reference and archive cards (Trick Records, Competition, Scoring & Combos, Operators & Modifiers, Insights, Observed Tricks). A "Featured" video strip (competition formats plus curated demonstrations) renders last. Standard `.card` + `.btn-outline` throughout. |
...
| Freestyle set-notation reference | `GET /freestyle/sets/reference` | `FreestyleService.getMovesPage` | Public | `PageViewModel<FreestyleMovesContent>`; flat reference table sourced from the Holden compilation (legacy `/freestyle/moves` 301-redirects here). |
...
Task UI shape: each task renders a single page with the task-specific view-model (city (required), region (optional), country (required), DOB (required), competition year, and show-year toggle for `personal_details`; unified legacy-and-HP candidate list with declared-anchor matching for `legacy_claim`; multi-stage club picker for `club_affiliations` with stages 1/2A/2B/wrap-up; simple form for `show_competitive_results`), a `Skip for now` action, and a `Complete and continue` action. Per-card actions persist immediately; there is no separate save action. Visual consistency is enforced via the shared wizard layout primitive so all four current tasks render in the same chrome. The `club_affiliations` view-model carries a per-card `confidence` discriminator (`pre_populate` for Stage 2A cards, `onboarding_visible` for Stage 2B cards) so the template renders the stage-appropriate framing and per-card question wording. Stages 2A/2B match at region level (state or province), with same-city candidates first. The wrap-up links to the clubs browse page filtered to the registrant's country for geographic browse and signal submission from club detail pages. Stage 2A's "I'd like to join" path renders an outbound link to the club's join page on the club detail surface; no signal is recorded by the wizard.
