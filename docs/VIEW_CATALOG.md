...
- Freestyle section: landing wrapper (`.freestyle-landing`), hero with mascot (`.hero-with-mascot`, `.hero-mascot`), tile cards (`.card-tile`, `.format-card`), embedded video wrapper (`.video-embed`), stats strip (`.stats-strip`), trick-dictionary view toggle (`.trick-view-toggle`, `.trick-view-toggle-active`, `.trick-view-toggle-sep`), trick-dictionary ADD-grouped layout (`.trick-add-group`, `.trick-list`, `.trick-row`, `.trick-row--external-placeholder`, `.trick-row-header`, `.trick-name`, `.trick-add-chip`, `.trick-add-chip--unrated`, `.trick-family-chip`, `.trick-category-chip`, `.dict-trick-row-media`, `.dict-card-media-chip`, `.dict-card-media-chip--tutorial`, `.dict-card-media-chip--demo`, `.dict-card-media-chip--record`, `.trick-status-badge`, `.trick-notation`, `.trick-notation--missing`, `.trick-description`, `.trick-description--missing`, `.trick-aliases`, `.trick-placeholder-note`), trick-detail pathways cross-link block (`.trick-pathways`, `.trick-pathway-list`, `.trick-pathway`, `.trick-pathway--learn`, `.trick-pathway--watch`, `.trick-pathway--family`, `.trick-pathway--empty`, `.trick-pathway-title`, `.trick-pathway-primary`, `.trick-pathway-secondary`, `.trick-pathway-link`), trick-dictionary sets-grouped layout (`.trick-set-group`, `.trick-set-type`), family-filtered cross-link block (`.related-set-groups`, `.related-set-groups-label`, `.related-set-group-link`, `.related-set-group-count`, `.related-set-group-sep`), trick-detail reference-media split (`.reference-media-subsection`, `.reference-media-subsection--tutorials`, `.reference-media-subsection--demos`, `.reference-media-subheading`), trick-detail related-trick reason label (`.related-trick-reason`), trick-detail modifier-memberships block (`.trick-modifiers`, `.trick-modifier-list`, `.trick-modifier-item`, `.trick-modifier-head`, `.trick-modifier-name`, `.trick-modifier-cluster`, `.trick-modifier-axis`, `.trick-modifier-gloss`), trick-detail additional-family chips (`.trick-also-family`, `.trick-also-family-chip`, `.trick-also-family-chip--minor`, `.trick-family-minor-tag`, `.trick-hero-meta-chip-family--minor`), By-family minor-lineage band (`.trick-minor-lineage-band`, `.trick-minor-lineage-intro`, `.trick-minor-lineage-list`, `.trick-minor-lineage-count`), trick-detail relationship groups (`.related-group`, `.related-group-label`, `.relationship-why`), trick-detail quantity ladder (`.trick-quantity-ladder`, `.ladder-steps`, `.ladder-step`, `.ladder-step-current`, `.ladder-step-here`, `.ladder-step-add`, `.ladder-step-missing`, `.ladder-step-missing-label`), trick-dictionary ADD jump-nav (`.add-jump`, `.add-jump-label`), set-encyclopedia card status pill (`.sets-encyclopedia-card-status`, `.sets-encyclopedia-card-status--platform-tracked`, `.sets-encyclopedia-card-status--community-cited`, `.sets-encyclopedia-card-status--under-review`), operator-board partial (`.operator-board`, `.operator-board-heading`, `.operator-board-lede`, `.operator-tier`, `.operator-tier-set`, `.operator-tier-body`, `.operator-tier-structural`, `.operator-tier-eyebrow`, `.operator-tier-title`, `.operator-tier-intro`, `.operator-grid`, `.operator-card`, `.operator-glyph`, `.operator-name`, `.operator-action`, `.operator-example`, `.operator-example-input`, `.operator-example-arrow`, `.operator-example-result`, `.operator-card-deeplink`), intermediate-operator reference (`.glossary-intermediate-operators`, `.glossary-operator-pending-flag`, `.glossary-operator-meaning`, `.glossary-operator-decomposition`, `.glossary-operator-decomposition-label`, `.glossary-operator-pending-note`, `.glossary-operator-examples`, `.glossary-operator-lineage`), operator-board landing footer link (`.operator-board-footer-link`), operators-index rows (`.operator-index-axis`, `.operator-subfamily-label`, `.operator-meta-chip`, `.operator-meta-chip--add`, `.operator-status-pill`, `.operator-status-pill--teaching`, `.operator-status-pill--platform-tracked`, `.operator-status-pill--doctrine-pending`, `.operator-row-descriptor`, `.operator-row-links`, `.operator-row-link`), modifier-stub detail page (`.modifier-stub-meta`, `.modifier-stub-what`, `.modifier-stub-notation`, `.modifier-stub-decomposition`, `.modifier-stub-decomp-label`, `.modifier-stub-examples`, `.modifier-stub-trick-list`, `.modifier-stub-trick-add`, `.modifier-stub-related`). The freestyle surfaces carry an extended semantic color palette in `:root` (role-hue-tone token names such as `--pending-umber`, `--historical-gold`, `--set-azure`); the tokens are enumerated in `src/public/css/style.css`, not duplicated here.
...
| Onboarding wizard task page | `GET /register/wizard/:taskType` | `MemberOnboardingService` + per-task delegated services | Authenticated only; redirect to `/login?returnTo=...` on unauthenticated visit | `PageViewModel<TaskContent>` keyed by task type. `personal_details`: location, DOB, gender (defaults to undisclosed), competition year, and show-year toggle. `legacy_claim`: unified candidate list with declared-anchor matching per §7.7. `club_affiliations`: own-affiliation cards (Stage 1A/1B) plus the find-or-create wrap-up landing. `show_competitive_results`: simple form. Each page renders a `Skip for now` POST and a `Back to dashboard` link to the member's profile. Unknown `:taskType` returns 404. Idempotent `startTaskList` runs on first GET so all four task rows exist. See §7.6a. |
...
| Club detail | `GET /clubs/{clubKey}` (shared `:key`) | `ClubService` (read method per code) | Public | `PageViewModel<ClubDetailContent>`; `clubKey` matches `^club_[a-z0-9_]+$` exactly (no aliasing or fuzzy); leaders section member-visible (renders to authenticated viewers only; anonymous HTML carries no leader names); `contactEmail` rendered only when `showContact = true` (current leaders shown to authenticated viewers; provisional entries and anonymous viewers never); service filters non-renderable leaders at read query (not in template); one auth-gated club-members list carries confirmed and 'pending' entries together (member-profile link for claimed search-visible accounts, historical-person link otherwise, per-entry unconfirmed status note on 'pending' rows); the page carries no at-a-glance card and no feedback or signal affordances; see §7.9. |
| Media hub | `GET /media` | `MediaGalleryService.getMediaHubPage` | Public | `PageViewModel<MediaHubContent>`; six equal-size media-collection cards in one grid, the browse-by-hashtag card leading with a distinct green accent and linking to `/media/browse` (then Freestyle Tutorials & Demos linking to `/media/freestyle-tutorials`, Freestyle Records, Net, Sideline, Related Sports). Coming-soon cards differ only by a footer label, not lighter weight. Member-created named galleries (member-owned, non-default) preserved in a grid below the primary cards. |
...
| Freestyle operators reference | `GET /freestyle/operators` | `FreestyleService.getOperatorsPage` | Public | `PageViewModel<FreestyleOperatorsContent>`; compact, browseable index of the modifier vocabulary grouped by the four movement-system axes (`content.indexAxes`, grouping from `src/content/freestyleOperatorIndex.ts`), rendered in the shared `dict-trick-row` row idiom: identity (name + `#hashtag`), a type chip + ADD-weight chip (`operator-meta-chip`), a status pill (`operator-status-pill--teaching` / `--platform-tracked` / `--doctrine-pending`), and View-details / Browse-tricks click-throughs. The SET notation line renders only for set-structured modifiers (those with a canonical-set formula); body modifiers carry no notation. Name + ADD come from the `freestyle_trick_modifiers` table; status is derived (a hand-authored teaching page, else a curator-pending decomposition, else platform-tracked). Below the index, a short cross-cutting tail (operator-decomposition concept, alpine, cross-body) is the only reference content; per-modifier depth (decomposition, worked examples, structural definitions) lives on the detail/stub pages, and the full per-modifier reference plus its load-bearing `#term-{slug}` anchors stay in the glossary's Modifiers & Operators section (the anchor home that semantic tokens link to). |
...
| Freestyle modifier detail | `GET /freestyle/modifier/:slug` | `FreestyleService.getModifierDetail` | Public | `PageViewModel<ModifierFamilyPageContent>` or `PageViewModel<ModifierStubContent>`; resolves every known modifier. `getModifierDetail` returns a discriminated `{kind}`: a modifier with a hand-authored teaching page renders the rich `modifier-family` template (mechanical lead, common confusions, progression, same-idea-other-bases, related modifiers); any other known modifier (present in the operator index, the `freestyle_trick_modifiers` table, or the operator reference) renders the data-driven `modifier-stub` template (what-it-is, type chip + ADD weight, set-only notation, decomposition + worked examples from the operator reference, common tricks from `freestyle_trick_modifier_links` lowest-ADD-first, related axis-sibling modifiers). An unknown slug throws `NotFoundError` and renders 404. Observational symbolic-grammar layer. |
...
2. **Authenticated non-owner viewing a HoF/BAP member**. `MemberService.getPublicProfile` returns `PageViewModel<PublicProfileContent>` with approved non-PII fields only: display name, country, honor badges, bio, optional `member_links`, plus gender when the member has opted in to gender visibility (signed-in viewers only; only Male / Female render). No edit links. Contact PII is opt-in per field and reaches authenticated members only: email shows when `email_visibility = 'members'`, and phone and WhatsApp each show when their own `phone_visible` / `whatsapp_visible` flag is set. `email_visibility` is `'private'` or `'members'` only; contact fields are never exposed to an unauthenticated visitor.
...
- Results show display name, country, and honor badges (HoF, BAP, board), plus gender when the member has opted in to gender visibility. No email, no city, no other member-internal data.
...
`showContact` is role-and-viewer based: true only for a current (claimed or assigned) leader rendered to an authenticated viewer. Provisional entries and anonymous viewers always map to false.
...
Unauthenticated visitors viewing the club detail page see neither the leaders section nor the club member roster; a login prompt or equivalent bounded call-to-action appears in place of the visible roster. Authenticated visitors see one club-members list: confirmed entries link to the live member profile when the person has claimed a search-visible account (otherwise to the historical-person page), and 'pending' legacy affiliations render only with a per-entry unconfirmed status note, never presented as confirmed membership. A member's gender appears beside their roster entry when they have opted in to gender visibility.
