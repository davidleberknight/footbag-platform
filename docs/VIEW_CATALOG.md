...
| Wizard year submit | `POST /register/wizard/first_competition_year/submit` | `MemberOnboardingService.submitTaskResponse` → `MemberService.setFirstCompetitionYear` | Authenticated only | Writes `members.first_competition_year` (or NULL on empty); validates the value is a whole number between 1972 and the current year; completes the task and 303-redirects to the next task per §8.4. Invalid year returns 422 with the form re-rendered and an inline error. |
...
| Club detail | `GET /clubs/{clubKey}` (shared `:key`) | `ClubService` (read method per code) | Public | `PageViewModel<ClubDetailContent>`; `clubKey` matches `^club_[a-z0-9_]+$` exactly (no aliasing or fuzzy); leaders section public; `contactEmail` rendered only when `showContact = true`; service filters non-renderable leaders at read query (not in template); club snapshot section renders pre-shaped vitality strings (known leaders count or `None known yet`; members count or `Unknown`; status label); see §7.9. |
| Media hub | `GET /media` | `MediaGalleryService.getMediaHubPage` | Public | `PageViewModel<MediaHubContent>`; lists every named-gallery URL bookmark (FH-owned and member-owned); FH first then alphabetical within each cohort; `byMember` chip lifted from `#by_<slug>` criterion when it resolves to active member (auth-gated profile link); `criteriaTags[]` and `excludeTags[]` chips; per-card `owner` shape (`displayName`, `slug`, `isSystem`); FH-owned card owner rendered as plain text, member-owned as plain text inside card (no nested anchor). |
...
| Freestyle landing | `GET /freestyle` | `FreestyleService.getLandingPage` | Public | `PageViewModel<FreestyleLandingContent>`; portal landing surface. Hero ("Freestyle Footbag" + portal-framing subtitle + mascot icon), short portal-framing lede paragraph, two-anchor jump nav (#featured + #reference-shelf). Portal-card grid in user-centered order: Tutorials & Learning → Trick Dictionary → Glossary → World Records → Competition → History/ADD/Combo → Insights → Operators & Modifiers (demoted to last with a single one-sentence framing line; the prior "Advanced Reference" sub-tag was retired in the second-pass cleanup so the card carries no loud badge). Featured strip carries 4 competition-format cards + 4 curated demonstrations. The Reference Shelf below Featured groups six collapsed `<details>` panels (Basic Components, Core Tricks, Operators & Modifiers, ADD & Scoring Basics, Notation Basics, Learning Path), each gated on its own slug-tagged body class. The Basic Components and Core Tricks panels embed their respective grids; the Operators & Modifiers panel deliberately does NOT embed the operator-board encyclopedia (that lives at /freestyle/operators), instead surfacing a lightweight preview: a one-sentence framing summary, three non-interactive example chips (pixie, spinning, paradox), and a single CTA "Open full operator reference →" pointing at /freestyle/operators. Get Started tiles render at the bottom. The legacy `.freestyle-language-intro` + `.freestyle-basic-components` + `.freestyle-core-tricks` standalone-section wrappers are retired, and the landing-only `.operator-board-footer-link` / `.freestyle-portal-card-advanced-tag` classes are deleted. Test-pinned: Tier-4 accounting patterns remain absent from the landing core-tricks grid (4-tier rendering hierarchy contract preserved); the embedded operator-board on /freestyle is also absent (the canonical board renders on /freestyle/operators and /freestyle/learn). |
| Freestyle records | `GET /freestyle/records` | `FreestyleService.getRecordsPage` | Public | `PageViewModel<FreestyleRecordsContent>`; world records grouped by record type. |
...
| Freestyle ADD analysis | `GET /freestyle/add-analysis` | `FreestyleService.getAddAnalysisPage` | Public | `PageViewModel<AddAnalysisContent>`; pure curator content (no DB access); §1 component-contribution table; §2 worked examples with explicit ADD derivation lines; §3 discrepancy case studies; §4 interpretation notes; cross-links to dictionary + glossary + history. |
| Freestyle combo analysis | `GET /freestyle/combo-analysis` | `FreestyleService.getComboAnalysisPage` | Public | `PageViewModel<ComboAnalysisContent>`; pure curator content (no DB access); operates at the sequence level above the trick dictionary, parallel to ADD analysis at the trick-decomposition level; 8 sections: philosophy, run-quality terminology, sequence architecture, difficulty architecture, worked combo examples, transition topology, caveats, further reading. |
| Freestyle observational layer | `GET /freestyle/observational` | `FreestyleService.getObservationalLayerPage` | Public | `PageViewModel<FreestyleObservationalContent>`; observational-layer surface for trick names that appear in external corpora (PassBack, FootbagMoves, Shred Global, Footbag Finland) but have NOT been promoted to canonical curated status. Cards render as a flat alphabetical list; NO grouping by claimed ADD, because observed tricks are not canonical resolved tricks and the claimed numeric value is often a relative modifier delta, not a canonical difficulty class. Per-card external-claim label is source-attributed (e.g. `PB claim: 4`, `FM claim: 5`) and omitted entirely when the source publishes no numeric claim. Curator-review state surfaces as a per-card status chip (neutral / accent / muted tones); rejected entries are not rendered. Two-letter source badges (PB / FM / SG / FF / OTHER) carry full source attribution as tooltips. Compact 2-line cards default; a `<details>` expansion surfaces additional readings, formula, curator note, and blockers when present. Strict layer-separation contract: never inlines into canonical surfaces; entries carry no hashtag chips, no trick-detail pages, no media attachments. Sourced from the curator-authored `OBSERVATIONAL_TRICKS` content module; no DB access. Below the cards, a `Tracked vocabulary` section lists documented-but-unpublished trick names grouped by documenting source (FootbagMoves / footbag.org / PassBack), not by ADD claim, surfacing the wider tracked corpus as staged for review rather than omitted; sourced from the generated `freestyleTrackedNames.ts` content module. Discoverable from `/freestyle/tricks` dictionary landing (Card 5). |
