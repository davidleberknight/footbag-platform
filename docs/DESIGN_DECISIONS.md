...
The version column on mutable domain tables counts the updates a row has received, incremented on every UPDATE. Counting them is what tells a repeated event on a reused row apart from the original: the club membership notification key is built from the affiliation row's id and its version, so a member who leaves a club and rejoins is sent the second email rather than having it suppressed by the outbox's permanent idempotency index. Concurrent writers are handled by the transaction model above, and a transition that must not be applied twice carries its own guard as a predicate in the statement's WHERE clause, the way the media job worker claims a row by requiring the state it expects to find. Where two people edit one record the later write stands, and the audit ledger is what an overwritten value is recovered from, carrying each changed field's before and after value wherever somebody edited a shared record or another person's. Append-only and ledger tables omit it because a trigger pair blocks every update, so there is no later change for it to count. Junction tables omit it because the application inserts and deletes their rows rather than editing them. Imported and reference catalogues omit it along with the actor columns, because their rows record what a source supplied and carry that provenance in their own source columns; an administrator curating such a catalogue is editing a record of external material, and the audit ledger holds the account of who edited it.
...
- Normative defaults. Defaults for required env vars live in `src/config/env.ts` (or are required explicit in production). Defaults for `system_config` keys are defined in USER_STORIES "Configurable Parameters" and must be seeded into the database during initial database creation.
...
Temporary internal tooling lives under a dedicated subtree at `src/internal-<purpose>/**` with a matching view tree at `src/views/internal-<purpose>/**`, kept separate from the permanent product surface in `src/services/**`, `src/controllers/**`, and `src/views/**`. Permanent role-gated admin tooling is product code and lives in the ordinary trees under the established `admin` naming, separated from member-facing surfaces by the admin authorization gate rather than by file location. The separation is orthogonal to the dev/staging/production adapter parity model defined in §1.9 and §5.3.
...
- Every file in such a subtree carries a retirement banner naming the subsystem it dies with, so the scope is mechanically greppable at retirement time rather than reconstructed by hand, and the banner is what proves the scope empty after removal.
...
Layer-boundary rules and data invariants that can be checked mechanically are enforced at merge by a convention gate (`scripts/ci/assert_conventions.sh` and its delegated checkers), not left to human review. Every check declares what it reads, and one that cannot find it reports that it did not run rather than passing over an empty scope; only a tree that declares itself a fixture may skip, which is what lets the rules themselves be proved red and green inside throwaway repositories. The gate covers template discipline (no branching on raw domain enums, no multi-variable URL assembly, no inline data-island serialization), controller discipline (no direct SQL execution, no direct cookie emission outside the cookie helpers), data-layer discipline (positional SQL parameters only, the canonical UTC timestamp form, presence of every documented append-only trigger, the standard metadata columns on every mutable domain table or a declared family and reason for a table that carries fewer, and an UPDATE that stamps every metadata column its table carries), service discipline (failure audit rows route through the operational-error helper), test discipline (a test seeds rows through the shared factories rather than a hand-rolled INSERT, exempting a table the suite itself creates as a fixture, and, per statement, one whose subject is the database refusing a row; no committed unconditional skip in either the TypeScript or the Python suites, a conditional one being the sanctioned form (and a capability skip additionally refusing to stay silent where the verdict is taken); no row identified by ordering on a timestamp; no assertion decided by an unfrozen clock or unseeded randomness, unless its bound is derived from a budget the code under test declares and says so; no per-test timeout equal to the configured default, which changes nothing and reads as though the case had needed it; temp paths carrying the swept prefix; synchronous spawns carrying the shared bound; and the setup's isolation of AWS credentials and of the rest of the machine left intact, no spawn handed an environment built from scratch in place of the isolated one), audit-log discipline (every `action_type` is a lowercase dotted `domain.event`, and every such literal appears in the data model's action_type catalogue, so the inventory that calls itself authoritative cannot drift from the code), supply-chain discipline (every GitHub Actions reference pinned to a commit SHA), artifact discipline (no tracked archive and no Terraform state in any form, detected by magic bytes wherever the file sits and whatever it is called), migration discipline (a migration file is additive: it drops nothing, renames nothing, edits the schema table not at all, and writes no rows, unless the statement itself declares what it is doing, `-- CONTRACTION:` for a schema removal and `-- DATA CHANGE:` for one that touches rows), comment hygiene (no doc-path references or delivery-epoch labels in code or template comments), deploy-image discipline (every runtime read resolving under the repository root has a matching Dockerfile COPY, every runtime image normalizes the modes of the repository paths it copies, and no copied path is left unreadable by the image's non-root account), and the visual-token and template restrictions of §4.8. Checks that require judgment, or that range over a growing unenumerable set such as service file-header JSDoc accuracy and completeness, are not gated; they are carried by the adversarial bug-hunt review.
...
Error Handling Pattern: Services catch SqliteError, inspect error.code property, handle known error codes (SQLITE_CONSTRAINT_UNIQUE → meaningful business error, SQLITE_BUSY → temporary-unavailable service error, rendered as 503, SQLITE_FULL → critical alert), propagate unknown errors to controller layer. Controllers log all database errors with safe context only: query name, operation, correlation ID, and redacted parameter summaries (never raw PII, secrets, tokens, emails, or payment details).
...
Member personal data: retained for a configurable grace period (Administrator-configurable default 90 days, parameter key: member_cleanup_grace_days) after soft delete, then purged from primary storage. Purge sets credential and contact fields (email, phone, passwordHash) to NULL. For non-nullable identity/location columns retained for referential integrity, the application overwrites values with anonymized placeholders (not original data). Exception: members with HoF or BAP flags keep the record the honor publishes after purge: slug, `displayName`, `bio`, country, both archival identity links, and their claim on the legacy account. Credential and contact fields are still nulled, the legal name is still replaced with a placeholder, and other required retained identity/location fields are anonymized as needed. The member row is retained as an anonymized record for referential integrity and audit history.
...
- Every bucket, without exception, carries a bucket policy denying any request where `aws:SecureTransport` is false. Encryption at rest and encryption in transit are separate obligations and this is the second one: the framework names the bucket policy as the mechanism, and a posture where every client happens to use TLS is not the same as one where plaintext is refused. It costs nothing, because every access path is already TLS, and `scripts/ci/check_bucket_baseline.sh` fails a tree where a bucket lacks it, alongside its encryption configuration and its public access block. One consequence is load-bearing rather than incidental: CloudFront reaches an origin-access-control bucket over HTTPS only while `signing_behavior` is `always`, so downgrading that setting on any distribution would start taking 403s from S3.
- The Terraform state bucket is governed by this decision too. It holds no application data, but it is the account's densest concentration of secret material: state records resource attributes in plaintext, so any secret a provider generates or reads back lands in it. It takes SSE-S3, the transit deny, versioning and a public access block like every other bucket. Two stronger controls were considered and declined. A customer-managed key would add a second gate, but the operator identity that reads state holds full administrative access today, so the same principal would hold the bucket and the key — the identical reasoning that declines customer keys above, and the answer changes once that identity is scoped down. Object-level access logging was declined because nothing in this account consumes such a log, and a log with no reader is a cost and a false assurance rather than a control; the answer changes when a detection or query pipeline exists to read it.

Trade-offs:

- There is no per-object access record on the application-data buckets: S3 server access logging is enabled on none of them and CloudTrail records management events only, so a read of an object by a permitted principal is logged nowhere. Incident response therefore rests on IAM and bucket-policy scoping, on CloudFront access logs for the viewer read path only, and on CloudTrail management events; the per-decrypt audit trail and operator key-disable containment that a customer-managed KMS key provides are forgone, and so is any object-level record that would substitute for them.
...
- The steady-state path is the lifetime path: an admin selects a member, supplies a reason, and the system writes `is_admin=1`, the role's Tier 2 invariant where the target holds less, and an `audit_entries` row with `actor_type='admin'` recording the tier movement.
...
- Dev and staging bootstrap is an email allowlist matched at registration time. Source: an operator-local file in the maintainers' private operations checkout, reached through the canonical repo-root symlink because it carries maintainer email addresses, parsed by the deploy pipeline into the `FOOTBAG_DEV_INITIAL_ADMIN_EMAILS` env var written to `/srv/footbag/env` on the target host. The env-config layer fails fast at boot if the var is set on a production host, and the deploy pipeline refuses to write the value to a production host. The runtime mechanism lives in `src/dev-bootstrap/runtime.ts`.
...
An authenticated session is represented by a JSON Web Token (JWT) issued at login and at completion of the email-verify, password-reset, and password-change flows. The token travels in the `__Host-footbag_session` cookie. Cookie attributes set at issue: `HttpOnly`, `SameSite=Lax`, `Secure`, `Path=/`, host-only scope (no `Domain` attribute, so the cookie returns only to the host that issued it; the archive's own signed cookies carry the parent-domain scope co-residency needs, per §6.4), `Max-Age` equal to the configured session lifetime (86400 seconds by default, expressed in seconds at the wire; the Express helper passes milliseconds). The name and every one of those attributes are unconditional, on every transport and in every environment, and no other cookie name is ever read. The `__Host-` name prefix is enforced by the browser rather than by the server: a cookie so named is discarded unless it carries `Secure`, carries no `Domain` attribute, and has `Path=/`. That is the defense host-only scope cannot supply on its own, for two reasons: cookie scope is evaluated against a request's destination host rather than against whoever set the cookie, and the `Cookie` request header carries only names and values, so the application cannot distinguish a cookie it set from one another host planted. Without the prefix, any other `footbag.org` name, including one still served by another operator, could set a parent-domain cookie of the same name that shadows the real session and silently substitutes its own identity for the member's. Deriving either the name or `Secure` from the request, the environment, or `NODE_ENV` would be fail-open, since losing that signal would silently restore the shadowing surface; browsers exempt `localhost` from the HTTPS requirement for `Secure` cookies, so the unconditional form holds in development as well. No per-session server-side state is maintained; the JWT's RS256 signature, produced by a non-exportable AWS KMS key in production (or a local PEM keypair in dev and test) per §3.5, authenticates the credential on every request.
...
|---|---|---|---|---|
| `__Host-footbag_session` | `issueSessionCookie` in `src/lib/sessionCookie.ts` | RS256 signature; private key in AWS KMS (production) or a local PEM (dev and test), per §3.5 | 24 hours by default, administrator-configurable via `jwt_expiry_hours` | JWT claims: `sub` (member id), `passwordVersion`, `role` hint, standard `iat`/`exp` (§3.4) |
...
- `Secure` is always set on the session cookie, and is never conditioned on `req.secure`, the `X-Forwarded-Proto` header, `NODE_ENV`, or anything else. The cookie's `__Host-` name requires it, so a conditional flag would mean a conditional session: on any request where the condition failed, the browser would refuse the cookie outright. Browsers exempt `localhost` from the HTTPS requirement for `Secure` cookies, which is what lets the same unconditional attribute set serve local development. The archive's three CloudFront signed cookies do derive `Secure` from the request, since they are presented to a different host whose transport this request does not determine.
...
2. Read the `__Host-footbag_session` cookie, and no other name (§3.2). Absent: return without setting `req.user`; the request proceeds as unauthenticated.
...
- `Origin`-header pinning closes the same-site subdomain surface that `SameSite=Lax` does not cover. Cookie scope is evaluated against a request's destination host, not the page that initiated it, so the session cookie reaches the platform on a request made from any `footbag.org` subdomain whether or not it carries a `Domain` attribute; and sibling subdomains are same-site, since a site is the registrable domain, so `SameSite=Lax` does not isolate same-site subdomain POSTs. A static malicious form served from `archive.footbag.org`, or a future XSS in legacy archive content, could otherwise issue authenticated POSTs to `footbag.org`. The middleware rejects with 403 any state-changing request whose `Origin` header does not match the canonical `PUBLIC_BASE_URL` origin exactly. The `Origin` header is a forbidden-header-name in the CORS specification: no client-side script can override it via `fetch()` or `XMLHttpRequest`. Browsers send `Origin` on every cross-origin state-changing request and on same-origin state-changing requests as well.
...
`SameSite=Lax` alone, with no Origin pinning. Rejected: `SameSite=Lax` does not isolate same-site subdomain POSTs, because sibling subdomains share the registrable domain and are therefore same-site; the session cookie's host-only scope does not change this, since a cookie is sent on the strength of a request's destination rather than its initiating page. A malicious form, future XSS in legacy archive content, or a compromised third-party widget on the archive subdomain would otherwise issue authenticated POSTs to `footbag.org`. Origin pinning closes that subdomain CSRF surface.
...
The session JWT (§3.2) has a 24-hour lifetime by default (`exp = iat + 86400` seconds), administrator-configurable through the `jwt_expiry_hours` parameter and read at every issue, so a change reaches the next sign-in without a redeploy. The rationale below is for that default. The signing key is an AWS KMS asymmetric RSA-2048 key in production and a local PEM keypair in dev and test (§3.5). The JOSE header carries `alg: RS256` and `kid` (the active signing-key identifier) so the verifier can resolve which key signed each token across rotation overlap windows. The payload carries `sub` (member id), `passwordVersion`, `role`, and the standard `iat` and `exp` claims. The platform does not issue refresh tokens; instead, the authorization middleware re-issues the session JWT in place when the existing token is within 6 hours of expiry, replacing the cookie with a freshly-signed token carrying the configured lifetime on the same response that performed the check. The re-issue window is fixed, so a configured lifetime at or below six hours re-issues on every authenticated request rather than once per session. A token allowed to expire without an intervening authenticated request is not renewed; the next request lands unauthenticated and the user must authenticate again. The session cookie's transport attributes are defined in §3.2 and not restated here.
...
- A rotation replaces the signing key outright: the new key signs from the moment it is installed, and a token naming the prior key is refused immediately, so every active session ends and members sign in again. The prior key is disabled in KMS once the rotation is recorded.
...
- The `kid` header names the signing key that produced a token, so a token outliving its key is refused rather than verified against the wrong one. Verification accepts one key at a time: a token whose `kid` is not the current one is rejected, in both the KMS-backed adapter and its local counterpart. Rotation is therefore a flag day by design — every active session ends the moment the key changes, and members sign in again. That is the ruled trade rather than an omission. Rotation here is a scheduled operator action on a runbook, the signing key is asymmetric and so unsupported by KMS automatic rotation in any case, and in the case that matters most, a suspected key compromise, ending every session at once is the wanted behaviour rather than one to work around. Holding several public keys through an overlap window would remove the logout at the cost of a two-phase rotation and a verification path that must decide which of several keys to trust; that was considered and declined.
...
- The signing key is rotated for cause, never on a clock: there is no cadence, no interval and nothing to fall behind on. Any reason is valid — a suspected exposure, an operator leaving, a key that has been somewhere it should not — and time passing is not one. Rotation timing is therefore not alarmed and could not usefully be: an alarm on a date would need a key-age metric no part of the system publishes, which would either sit permanently in insufficient-data or report a figure nothing maintains. The control is the documented rotation procedure and the reason that invokes it, not a monitor. What IS alarmed is failure rather than schedule: a key that stops working takes authentication down, and the KMS error alarm catches that.
...
- Lightsail has no EC2 instance profile. Parameter Store reads require activating an AWS runtime credential path (long-lived IAM-user keys in `/root/.aws/credentials`, root-owned and mode 0640 inside a 0750 directory owned by a dedicated credential group, used as a source profile to assume the runtime role, or SSM Hybrid Activation); on EC2 an instance profile handles this automatically. The group read is what lets the unprivileged web and worker containers, which are joined to that group, resolve the profile without the file being handed to the service account outright; the deploy mounts the directory into them read-only. The consequence is stated in the threat model below rather than left to be inferred from the mode. The host env file `/srv/footbag/env` carries only the runtime profile name (`AWS_PROFILE`) and other non-secret config, never the access keys themselves. The Lightsail credential-path cost is accepted for secrets where rotation-without-redeploy or multi-consumer access justifies it.
...
Threat Model Clarification: Parameter Store does not protect against an attacker who gains shell access inside the production container while usable runtime AWS credentials are available to that container. An attacker in the container can call SSM GetParameter for any SecureString values allowed to the runtime assumed role. The exposure is the long-lived source-profile key itself, not only a temporary session: the web and worker containers mount the credential directory read-only and are joined to its group, so a process inside either one reads the access key rather than merely holding an assumed-role session that expires. The key survives a container restart and is invalidated by rotation alone. What bounds it is the source user's own policy, which permits nothing but assuming the two runtime roles, so the key confers no more than the session it is used to obtain. The image worker deliberately carries no such mount. For secrets that must remain non-exportable even under container compromise, the system uses KMS/HSM-backed keys (non-exportable asymmetric signing keys) and IAM separation (normal web runtime cannot decrypt ballots).
...
`SESSION_SECRET` is held in Parameter Store as a SecureString and mirrored into the host env file `/srv/footbag/env` (root:root 0600) on every deploy. Terraform owns the parameter's existence and its KMS reference, never its value: the resource carries a placeholder under `ignore_changes`, because a Terraform-generated random value is written into state in plaintext by construction, so Terraform owning the value would mean the state file held a live secret. `scripts/provision-ssm-secret.sh` is the only writer; it generates the value, checks the destination before writing, keeps the value off the command line and out of shell history, and shreds its own copy afterwards. Rotation is a fresh store followed by any deploy, which invalidates every active session; a hand edit on the host is reverted by the next deploy. The value is generated fresh per environment and never reused across staging and production. The placeholder is refused by the deploy's own shape gate and by the application's boot check, so a deploy before the store step fails loudly rather than serving with an unset secret. The application and the deploy script both reject any value containing the literal placeholder substring `changeme` or shorter than 32 characters. Because recovery from a lost value is a fresh store rather than a restore, no copy is held in the credential vault. Rotation runbook: DEVOPS_GUIDE.md (private GitHub repo), "SESSION_SECRET rotation runbook".
...
- `scripts/verify-host-env.sh` warns (advisory, non-fatal) when the value is missing or non-integer.
...
- A leaked secret bypasses the gate until rotated. Mitigation: the secret is one of three access controls at the perimeter, alongside the Lightsail CloudFront-prefix-list firewall and the trust-proxy named-range trust set. TLS on the edge-to-origin hop is the fourth layer and is what makes them defence in depth rather than three answers to one question: those three decide who may connect, and only TLS decides who may read. Until the hop is encrypted the belt-and-suspenders claim holds for reachability and not for confidentiality.
- Origin TLS protects future traffic only, and this matters for the secret above. The hop carries session cookies, payment webhook bodies and their signatures, and the origin-verify secret itself in clear until it is encrypted, so anything observed before that flip is already compromised and encrypting afterwards does not un-compromise it. Rotating the origin-verify secret is therefore part of the origin-TLS cutover rather than a separate task, and the runbook sequences it as the last step of first issuance.
...
- A route's guard is required whether or not the interface currently offers a path that reaches it. Reachability is a property of today's templates and routes, so a missing server-side check is a defect on an authenticated surface as much as an anonymous one, and is never closed on the grounds that no form submits the malformed value today.
...
The UI intentionally avoids certain flashy or high-complexity design and interaction patterns, in order to keep the system simple, accessible, and volunteer-friendly. Specifically, the following are out of scope given the technology constraints: No SPA frameworks (React, Vue, Angular, etc.) or client-side routers. No infinite scroll that lacks a straightforward paginated fallback. No parallax scrolling effects or scroll-based animation frameworks. No auto-playing background videos or audio on core pages. Page media defaults to click-to-play with a poster resting state and an accessible play/pause toggle. The only autoplay exception is at most one intentional demo clip per page; background video and autoplaying audio remain prohibited. No UI flows that rely solely on hover, drag-and-drop, or complex gestures without keyboard/desktop-friendly alternatives. No heavily customized JS-only form controls that break standard keyboard/screen-reader behavior. No dependence on heavyweight front-end build tools beyond what is required to bundle TypeScript and CSS.
...
- Construct the URLs the response itself needs: redirect targets, and the `formAction` and `cancelHref` of a form the controller renders. The hrefs inside a page view model are composed by the page-shaping service that returns it, so the URL space is known in both layers and each owns the part it emits.
...
- Compose page-models beyond trivial glue. Trivial glue is: assigning the response's own URL fields (`formAction`, `cancelHref`, a redirect target) onto a service result, shallow-spreading a service result into a `PageViewModel<TContent>`, and selecting between N render variants on a single discriminant. An href a page displays comes from the builder, not from the controller. Anything else (iterating service rows through a per-row transform, deriving a label from a domain code, merging two service results, filtering tags or fields based on a domain rule) is page-model composition and belongs in the service or in a service-owned page-model builder.
...
- Bounce state transitions: hard bounces (SES permanent-failure type) auto-flag the `MailingListSubscription` as `bounced` and block further sends to that list for that member. Soft bounces (SES transient-failure type) are ignored: they self-heal, and the provider's own account-level suppression list already stops sending to an address that keeps failing, so the platform keeps no per-address soft-bounce count. Complaints auto-flag immediately and block all lists for that member, except a `not-spam` feedback type, which reports the opposite and is ignored. A complaint flag is terminal for automated mail; an administrator corrects a wrong one through the admin member-management surface, and the `suppressed` state is reserved for an explicit administrator block rather than written by the feedback path.

- Bounce and complaint idempotency: inbound SNS messages carry a `messageId`; the feed handler claims processed `messageId` values in a `ses_events` table with `messageId` as primary key, inside the same transaction as the status write. A redelivery leaves the record exactly as it stands. This is not an optimisation: the queue promises at-least-once delivery, so the same notification arriving twice is ordinary rather than exceptional. Parallel to the `stripe_events` idempotency in §6.1. The same row records the provider's identifier for the original message and the outbox row it resolved to, which is what lets a bounce name the send that produced it; resolving to nothing is ordinary, because the per-recipient outbox copy ages out on the outbox retention window while the notification record keeps the identifier after it.
...
Outbound platform mail is sent by AWS SES (see §5.4); mail a person sends from a Workspace-hosted address goes out through Google Workspace. Inbound receive for all role addresses is handled by Google Workspace, which delivers or forwards each address per its configured mailbox or alias. SES is not used for inbound ingestion.

| Address | Purpose | Direction | Used by |
|---|---|---|---|
| `admin@footbag.org` | Legal, administrative, privacy, copyright, and trademark contact for members and the public. Also one of the five certificate-validation addresses below | Receives | `/legal` page (Privacy, Terms, Copyright sections); operator of record contact |
| `administrator@footbag.org`, `hostmaster@footbag.org`, `postmaster@footbag.org`, `webmaster@footbag.org` | The other four addresses the certificate authority accepts as proof of control over the domain, alongside `admin@`. IFPA holds them because whoever receives one can obtain a publicly trusted certificate for any name under the domain, at issuance and again at renewal; they carry no correspondence role and need no reader. A request for a name beginning `www` is proved at this apex set (§6.13) | Receives | No platform code; held so that no one else holds them |
...
| `ifpa-treasurer@footbag.org` | IFPA treasurer contact for donation, sponsorship, and other financial correspondence; recipient of the platform's periodic reconciliation digest | Receives | Carried over from the legacy site as an apex alias; published donations contact on the legacy site; `PaymentReconciliationService` addresses the digest here, so the mailbox must be read before the first digest is due |
| `noreply@footbag.org` | Transactional sender (account verification, password reset, receipts, system notifications); never monitored, never a reply target. The address in force is the `ses_sender_identity` Terraform output, never a value written down | Sends | `CommunicationService.processSendQueue` via SES |
...
| `sanctioning@footbag.org` | Correspondence about event sanctioning. Organizers apply in the platform rather than by mail, so this address carries questions and follow-up, not applications. Inbound mail forwards to the sitting IFPA Sanctioning Director. The platform's own sanctioning notification does not pass through this address: it is sent to the Sanctioning Director directly | Receives | Carried over from the legacy site as an apex alias, replacing the sanctioning committee's former working contact on the retiring IFPA subdomain. Note that the address with the widest published reach is the one printed in the IFPA rulebook's sanctioning-requirements chapter, which instructs organizers to apply by mail; that instruction is superseded by the in-platform flow and the chapter is the artifact the content work must update |
...
- Platform mail carries no reply path at all: transactional messages, role correspondence and community announcements alike are sent without one, and a reply to any of them is rejected rather than delivered. A rejection tells the sender at once that nobody was listening, which is better than accepting a message into a mailbox no one reads. A member who holds an account reaches an administrator through the in-platform request flow; anyone else uses the contact address published on the legal page.
...
- Each canonical address is a first-class address of `footbag.org` in the Workspace that serves it: it is what a member sees in the From line, and it is the address Calendar invitations and sharing notices are sent from. A Workspace secondary domain satisfies this, because it provisions accounts whose own address is at that domain. A domain alias does not, because an alias routes to an account whose identity stays on another domain, so the From line and the Calendar sender remain that other domain. Which of the two the tenant holds is therefore the requirement, not whether `footbag.org` is the tenant's primary domain. The tenant's own prefix domain is temporary: after go-live every account and group is renamed onto `footbag.org`, `footbag.org` becomes the primary domain, and the prefix domain is removed. No canonical address waits on that change.
...
- The application's IAM grant to SES names the specific actions the application uses (`ses:SendEmail`, `ses:SendRawEmail`), never `ses:*`, and bounds them through a `ses:FromAddress` condition rather than through the resource ARN. The condition enumerates every address the platform may send as: the environment's own sending identity, and the address of any mailing list carrying its own sender, which today means the community announce list. The enumeration replaces the default rather than extending it, so an address left out of it is refused at the send, and the apply refuses a set that omits either. The condition carries the bound because the resource cannot: once domain authentication is enabled the identity becomes the whole domain, at which point a resource-scoped grant would authorise sending as any address at it. The Configuration Set used for sending is named explicitly in the policy.
...
- Google Workspace is configured with one mailbox or forwarding rule per receive address (`admin@`, `brat@`, `directors@`, `dleberknight@`, `dmarc-reports@`, `ifpa-treasurer@`, `ops-alert@`, `sanctioning@`), and with the four further certificate-validation addresses (`administrator@`, `hostmaster@`, `postmaster@`, `webmaster@`). `announce@` and `noreply@` are not among them: both are sending identities, provisioned as groups with no members so replies are rejected. `brat@`, `directors@`, `dleberknight@`, `ifpa-treasurer@`, and `sanctioning@` are carried over from the legacy site and must be live on Google before the apex mail records point at Google, since that is the moment mail stops reaching the legacy host. `dmarc-reports@` is live before the DMARC record publishes, since reports sent to an address that does not exist are discarded and the tightening evidence is lost. The five validation addresses are live from the moment the apex mail records point at Google, and stay held for as long as certificates renew, because the authority re-solicits them at every renewal.
...
- Engaging paid testers through real inbox delivery would put test
  traffic against the production domain's sender reputation, since the
  environments share one AWS account and one sender identity. The
  in-page card avoids that entirely.
...
- Staging Docker compose and scripts/verify-host-env.sh set
...
- One-time payments (membership dues, event registrations, one-time donations): Implemented via Stripe Checkout in payment mode. State transitions are keyed by payment_intent_id. Checkout offers card-backed payment methods only, pinned in the request rather than left to the Stripe Dashboard: fulfilment is keyed to the payment intent's settlement event, so a method that settles days after the buyer completes the checkout has no representable outcome here and would leave the payment pending indefinitely with reconciliation agreeing, since both sides read the same unsettled status. Accepting delayed-settlement methods is a later decision that would require fulfilling on the checkout-session events instead. Checkout sessions are also created with an explicit expiry of an hour or two rather than Stripe's day-long default, and the cancel page expires the session at Stripe outright, because an unresolved membership checkout blocks the member's next purchase attempt until it resolves. The enforced state machine is: pending → succeeded on payment_intent.succeeded; pending → canceled on checkout.session.expired; succeeded → refunded on charge.refunded. A declined attempt does not move the record: while the checkout session is open the buyer may present another card, so payment_intent.payment_failed records the attempt on the transition ledger and leaves the payment pending. Recording the decline as terminal would make the success that follows it unapplicable under the monotonic machine, charging a member who never receives what they paid for. A refund the provider then cannot return to the card, or reverses (refund.failed, or refund.updated carrying a failed or canceled status), cannot move the row back: it is recorded on the payment's money history and raised as an administrator work item, and the money is returned another way at the provider.

- Recurring annual donations: Implemented via Stripe Subscriptions. The platform creates or reuses a Stripe Customer object for each member (stripeCustomerId stored on the member record) and creates a yearly Stripe Subscription via Stripe Checkout in subscription mode. The platform does not manage the billing schedule or retries. Stripe owns the annual renewal cycle, dunning configuration, and retry logic. Local state transitions are driven entirely by incoming webhooks: active on customer.subscription.created; amount and lifecycle changes mirrored on customer.subscription.updated; a new payment record created on invoice.paid or invoice.payment_succeeded, whichever arrives first for a settlement (the provider raises both, and only invoice.paid reports an invoice marked paid out of band), the second booking nothing; local status set to past_due on invoice.payment_failed; local status set to canceled on customer.subscription.deleted. A subscription already recorded as canceled absorbs no further update, because applying one would leave the row carrying a cancellation time while claiming to be live. The Stripe Billing dunning schedule (number of retries, intervals) is configured by a System Administrator in the Stripe Dashboard and is not replicated in application configuration. The end-of-dunning action is not similarly free. Stripe offers three outcomes once retries are exhausted — cancel the subscription, mark it unpaid, or leave it past due — and only cancellation emits customer.subscription.deleted, which is the sole event that moves a local subscription to its terminal state. Under either other outcome the provider stops attempting payment while the local row stays past_due indefinitely: the member's action-dashboard item, which tells them the provider will try again, never clears, and reconciliation raises nothing because both unpaid and past_due map to local past_due and the two sides therefore agree. The Dashboard must be configured to cancel. The subscription linkage on an invoice is read from the invoice's parent subscription details, not from a top-level field, and the Stripe API version is pinned explicitly in the payment adapter with the webhook endpoint's version kept equal to it; payload shapes are version-specific and an unpinned SDK can reshape them under working code.
...
- Payment reconciliation runs as a scheduled nightly job that compares platform records against Stripe and records mismatches; distribution is handled outside the automated system. The comparison covers only local records of the provider mode the loaded credential is in, because a live key cannot list test-mode objects and the reverse; rows of the other mode are set aside and counted on the run rather than raised as discrepancies.
...
- Every CloudFront distribution serving a custom domain name pins viewer minimum TLS protocol to `TLSv1.2_2021` (or higher). A distribution answering only on its default `*.cloudfront.net` name is fixed at `TLSv1` by AWS, which permits no other value with the default CloudFront certificate. The protocol version is set in Terraform; CloudFront viewer settings are not modified via the AWS Console.
...
- CloudFront access logs are enabled and shipped to a dedicated log bucket whose lifecycle matches the retention windows in §1.2. They cover the viewer read path only: a principal reading an object straight from S3, which the runtime role can do on the media bucket, does not appear in them.
- The distribution carries `footbag.org`, `www.footbag.org` and the temporary pre-cutover `preview.footbag.org` as alternate domain names with a matching ACM certificate in `us-east-1` (preview is a SAN from first issuance, because a later addition forces a certificate replacement). Adding an alternate domain name requires only the certificate, not a DNS repoint.
...
- The signed cookies are scoped to `.footbag.org` so one sign-in on the platform covers the archive subdomain without a separate login.
...
- The main app's login and JWT-refresh middleware sets the signed cookies alongside the session JWT. The three signed cookies use `Domain=.footbag.org`, because a cookie without that scope is returned only to the issuing host and would never be presented at the archive; the session JWT stays host-only, since the archive edge authenticates on the signed cookies alone and never reads it.
...
- The encrypted container, the key vault, and the opening credential are never committed to the repository, placed in issues, logs, tests, or AI prompts. Concrete custody details (store, folder, vault entry, access list) are kept out of the public repository entirely. Their home is the maintainers' private repository, which is access-controlled and already holds the vault's entry map, the inventory of sensitive material, and the operations runbooks. A store or folder name recorded there discloses nothing on its own, because the credential that opens the container lives separately; leaving those details in no written place at all is what allows a custody gap to go unnoticed.
...
- `markDeadLetter` writes a NULL `body_text`, unconditionally: the worker classifies a send failure only as throttling, ambiguous, or definitive, so it has no toxicity signal to branch on, and dead-letter is terminal with no resend path behind it. This keeps a live token out of the dead-letter table and stops that table becoming a replay vector when an operator triages the queue. The two manual-review endings deliberately keep their bodies, because that status exists for an operator to read the message and decide whether to resend it.
...
## 6.11 How the `footbag.org` Namespace Is Served

Decision:

The `footbag.org` zone is authoritative on Route 53 under IFPA-controlled access, and Terraform owns every record in it. `www.footbag.org` is the canonical host, served as a Route 53 ALIAS to the production CloudFront distribution; the apex is an ALIAS to the same distribution, which 301-redirects it to `www`. No separate apex redirector exists: an ALIAS at the apex carries no CNAME-at-apex limitation and costs nothing, while a redirector would be an instance to run and to patch. Legacy infrastructure is not in the live path — no reverse proxy or front door on a legacy server, and no legacy TLS certificate — so the proxy chain is CloudFront then nginx, two hops, and CloudFront sees client addresses directly.

Mail routing is independent of the web records in mechanism, and not in requirement. In mechanism: MX is unaffected by address-record changes, and nothing in the web path touches it, so either can be changed without touching the other. In requirement they are not separable at all. The site is not fully functional, and cannot be exercised as this design specifies, unless both are under IFPA control: the zone gates the certificates, the platform's own names, the archive and sender authentication, and the apex mail records gate every address the site publishes to receive on. They are needed together and they are changed separately, in the same cutover: the web records move first and the apex mail records follow in the next change, so the legacy site's mail keeps working, sending and receiving, until the moment the legacy site leaves the live path.

Two mechanisms can withhold the site at the edge and they are not interchangeable. A viewer-request function on the default behaviour answers per hostname, so it can hold one name back while another stays reachable; the Stripe webhook path is exempt inside the function, and the health path needs no exemption because it rides its own ordered behaviour the function never runs on. The `enable_planned_maintenance` origin swap replaces the default behaviour's origin, and that behaviour serves every hostname on the distribution, so it blanks all of them together. The swap is therefore the right mechanism for the automatic 5xx origin-failure fallback and for a deliberate maintenance window, and the wrong one wherever a single hostname must stay reachable.

Recovery is fix-forward or a platform restore. No legacy server is kept running as a rollback target, and post-go-live legacy retention is limited to encrypted, non-public artifacts under IFPA-controlled access.

Rationale:

- Implements the Legacy Independence at Go-Live decision (§6.12): DNS is one of the functions that may not remain dependent on legacy infrastructure.
- Keeping legacy infrastructure out of the live path entirely means no legacy TLS certificate to maintain, no critical-path dependency on legacy hardware, and CloudFront seeing client IPs directly — the proxy chain stays CloudFront then nginx, two hops, with no third front-door hop at any milestone.
- Holding the zone under IFPA-controlled access removes the last infrastructure dependency on the legacy operator: the apex is served as a free ALIAS record (no CNAME-at-apex limitation, no Anycast fee, no redirector instance to run), and any emergency DNS change is operator-executed rather than resting on one person's availability.
- The simplest recovery is platform-side: fix-forward or a restore; when a DNS change is needed, a low-TTL zone converges in minutes.
- Mail is decoupled from the web records in mechanism: MX routing is independent of A/AAAA changes, and the web path never touches MX, so either is changed without the other. It is not decoupled in requirement. The site needs both to be fully functional, the zone for the certificates and authenticated sending and the apex mail records for the addresses it publishes to receive on, so neither alone is a testable system.
...
- The production distribution carries the apex and `www.footbag.org` as alternate domain names with a matching ACM certificate in us-east-1; the archive distribution carries `archive.footbag.org` with its own us-east-1 certificate.
- Records-actor: Terraform owns every record in the zone, and the operator applies them there.
- Any CAA record on the zone must authorize the issuing certificate authority before issuance is attempted, and a name taking a certificate from a different authority carries its own CAA record created in the same change as the name.
- `TRUST_PROXY` is 2 in staging and production, with no topology change at any milestone.
...
- A DNS change is not instantaneous for clients that cached the prior records; a low cache lifetime bounds this to minutes.

Impact:

- `terraform/production/route53.tf` carries the hosted zone and its records; the migration-notice flag rides the viewer-request function in `terraform/production/cloudfront.tf`, while `enable_planned_maintenance` serves the 5xx fallback and post-launch windows; no apex-redirector Terraform exists.
- GO_LIVE_PLAN.md (private GitHub repo) carries the cutover sequence itself: the zone move and registrar change, which flags flip at which step, the temporary preview name and its removal, the launch-day smoke gate, and the abort weighting. That material expires at launch and lives where it can be retired with the event.

## 6.12 Legacy Independence at Go-Live

Decision:

The migration is one coordinated production go-live. No required website, email, mailing-list, DNS, data, media, registration, or application function remains dependent on legacy infrastructure after go-live. Preparation may run in advance, but advance preparation is not phased delivery: every required service activates together at go-live, and "keep it running on legacy" is not an allowed final disposition. That is a statement about disposition, not about record mechanics: the independence of mail routing from the web records (§6.11) is about which records a change touches, never about whether both are required. Both are. Every legacy function is migrated, replaced, archived, or deliberately retired before shutdown. Post-go-live retention of legacy material is limited to encrypted, non-public recovery and reference artifacts under IFPA-controlled access.
...
- The go-live plan (GO_LIVE_PLAN.md, private GitHub repo) gates go-live on this principle; the maintainers' private tracker tracks the workstreams that satisfy it, the transfer of both the zone and the apex mail records among them.
- The decision on how the namespace is served (§6.11), the Closed Namespace decision (§6.13), the Legacy Archive decision (§6.4), the Sealed Legacy Email Archive decision (§6.5a), and the canonical email architecture (§5.5) implement it.

## 6.13 The Closed `footbag.org` Namespace

Decision:

From go-live, every name under `footbag.org` is operated by IFPA. The platform's served set is the apex, `www.footbag.org`, and `archive.footbag.org`; `preview.footbag.org` exists as a temporary pre-cutover name and retires after the post-cutover watch window. Beside it the Workspace tenant's own prefix domain and its aliases are served from this zone rather than delegated, carrying the tenant's mail records and its site aliases; they are IFPA's own infrastructure rather than a third party's. They are temporary: they retire after go-live, once every account and group has been renamed onto `footbag.org` and `footbag.org` has become the tenant's primary domain. The Workspace removes the prefix domain before its records leave the zone, because removing the records first would stop every mailbox and group still addressed there, including the account that recovers administrative control of the tenant. No subzone is delegated to a third party past go-live, and no name under the domain resolves to infrastructure IFPA does not control. A name IFPA wishes to retire is retired; a party wanting a name of their own is directed to a domain they hold.

Before go-live the position is transitional. Legacy names are served by another operator until the zone move completes, and a delegated subzone may be carried through the migration window as a bridge while an outgoing operator relocates what they run. That bridge ends at go-live. Retaining any name or mailbox under the domain past go-live requires two things together, and neither alone suffices: a written technical justification that IFPA accepts, weighed against the reasons below; and those risks stated to the retaining party and explicitly accepted by IFPA in writing. Prior arrangement, convenience, or long tenure is not a justification.

Where a name under the domain resolves to infrastructure IFPA does not control, two things together bound the risk and neither is sufficient alone. The apex certificate-authorisation record permits only the authority the platform's own certificates come from, which stops any other authority issuing for a name under the domain. That authority also accepts proof of control by mail to a fixed set of system addresses at the domain, so the second half of the bound is that IFPA receives all of them: `administrator@`, `hostmaster@`, `postmaster@`, `webmaster@` and `admin@`. A name whose operator receives any one of them can obtain a certificate the record permits.

Rationale:

- Browsers offer saved credentials by registrable domain. A sign-in page on any `footbag.org` name is offered members' `footbag.org` passwords by their own password manager, so a name IFPA does not control can collect member credentials with the browser's assistance and without any member error.
- Certificate authorization is read at the closest node, so a delegated child publishing its own CAA record overrides the apex record entirely. The operator of a delegated name can obtain a publicly trusted certificate for a `footbag.org` name from any certificate authority, and it appears in the public transparency logs under IFPA's domain.
- A delegated zone publishes its own SPF, DKIM and DMARC records, which take precedence over the organizational policy at the apex. The operator can send authenticated mail from a `footbag.org` address, and the apex subdomain policy does not prevent it.
- The archive's three signed cookies carry the parent-domain scope by necessity (§6.4), and cookie domain matching is suffix matching with no exclusion syntax. Every name under the domain answering over HTTPS receives a signed-in member's archive credentials. The confinement described in §3.9 bounds what those cookies grant; it does not bound who receives them. A closed namespace is what makes that confinement sufficient.
- Domain and sender reputation, including blocklisting and mail filtering, is assessed at the registrable domain. Content on one name under the domain carries consequences for every member's mail and for the site as a whole.
- The archived legacy site contains absolute links to historical sibling hostnames under `footbag.org`. Those names resolve to nothing once the cleanup apply retires the mirrored set, and until then they answer with the previous operator's hosts; either way, a name under the domain answering a request from inside a member-only archive page carries archive credentials to whoever serves it. The links are written as plain HTTP and the cookies are set Secure, so what turns that into a live exposure is a name obtaining a certificate. That is why the apex certificate-authorisation record publishes with the zone rather than at the flip: a certificate obtained before the record lands outlives it, so publishing early closes an opportunity window that publishing late would not shorten.
- None of the above is preventable by agreement or by restricting what a party publishes, because the browser resolves and attaches credentials by name without regard to whose content is served. The control is the namespace, not the arrangement.

Requirements:

- Terraform owns every record in the zone. No child `NS` record set survives go-live under `footbag.org`, and the absence is asserted mechanically rather than left to convention.
- The apex CAA record publishes with the zone, ungated, rather than at the alias flip, because the mirrored legacy names resolve to hosts IFPA does not control from the moment delegation lands and a certificate obtained before the record exists outlives it by months. It reaches the whole domain, which holds only while no child zone exists, and it governs which authority may issue rather than who may prove control to that authority: the five system addresses the permitted authority accepts as proof are IFPA-held receive addresses for that reason (§5.5). Any name under the domain taking a certificate from another authority carries its own CAA record, created in the same change as the name itself so no ordering can separate them.
- A name serving content IFPA does not own or moderate is not published under the domain, whether by delegation, by alias, or by redirect.
- Adding a name to the served set is a design change recorded here, not an operational convenience.

Trade-offs:

- IFPA cannot offer a subdomain as a courtesy to a community project, a sponsor, or an outgoing operator, and the alternative offered is a name on a domain that party controls. This is a real cost to goodwill and is accepted.
- Any future use of the domain by a partner requires IFPA to operate the name itself, including its certificate and its content.

Impact:

- Any migration-window delegation, and any request to retain a name past go-live, is coordinated in the go-live plan (private GitHub repo) rather than here.
- `archive.footbag.org` retains its parent-scoped signed cookies (§6.4, §3.9) on the strength of this decision.
- The strict-transport subdomain directive commits the names beneath the host that sends it, and only the canonical host reaches the application: the apex is answered by an edge redirect that sets no security headers, and the archive is served from storage through its own distribution. So the directive reaches `www.footbag.org` and its subtree, not sibling names under the domain. A future preload submission would commit the whole domain, which is consistent with this decision rather than constrained by it.
...
Routine host administration uses named non-root Linux operator accounts with `sudo`. Exactly one shared non-root account is retained deliberately, and it is not a deviation: it is the break-glass host route, and the bootstrap path onto a host that has nobody on it yet, where there is no existing key to authorize a newcomer against. It carries no routine work once an operator holds a named account, and it never substitutes for one. No other shared shell account exists. Shared private SSH keys are not allowed. Each approved System Administrator gets a separate SSH key pair and a separately attributable host-access path. Password authentication must be disabled, direct root login must not be the normal operator path, and port 22 must be restricted to approved operator source IPs or CIDR ranges.
...
Operator SSH has two paths and they are not peers. The routine path is direct CLI SSH from the declared operator IP CIDR list, which may carry multiple narrow `/32` entries or wider CIDRs to cover an operator who roams between networks. The second is the Lightsail access path, opened by declaring the `lightsail-connect` source-IP alias permanently on port 22 in the firewall HCL, and it is the way back in rather than a path for ordinary work: it is what reaches a host when no address on the allow-list still works, when the named accounts are damaged, and when a brand-new host has nobody on it to authorize a key against. An operator whose own address has changed edits the allow-list and applies it rather than reaching for this. Who may use it is decided by the AWS permission that mints its short-lived credential, so the control lives in the identity layer where it is declared in code, rather than on the host. The two paths differ in what they rest on, which is the point of keeping the second: the routine path requires the operator's public key in the host account's `authorized_keys`, while the Lightsail path does not — it returns temporary credentials for the default account, which is what lets it reach a host whose named accounts are damaged or which has nobody on it yet. That is also why the permission to make that call is the whole control, and why the job role is denied it.

Operator AWS identity is a named IAM user per person, and for a named operator the principal that acts is a session of the job role rather than that user. Each named operator authenticates as their own IAM user and assumes one shared job role to act, so the credentials every action carries are short-lived and scoped to the job. The named user holds no service permission of its own: its single inline policy allows one action, `sts:AssumeRole`, against one role. This departs from the AWS Well-Architected security pillar, whose guidance on temporary credentials pairs the move away from long-lived credentials with a move away from IAM users in favour of federation, and whose guidance on a centralized identity provider carries no exception for an estate of this size. The departure is deliberate and is accepted as a cost rather than claimed as compliance. What the design keeps is the part that carries the weight: no human credential reaches a service directly, and the credential that does reach services expires.

The named user's access key is long-lived, and what bounds it is that it can do nothing but assume the job role. A key reaching no service directly turns the loss of a workstation into a role session bounded by that role's own policy and its four-hour expiry, rather than into standing access to the estate; the security pillar names restricting such a key to `AssumeRole` on one role as the mitigation where a long-term credential must exist. Keys are replaced for cause rather than on a clock: an operator leaving, a suspected exposure, a credential that has been somewhere it should not. Time passing is not a reason, because a rotation cadence tests a promise to perform a ritual, while a recorded owner, issue date and written procedure test a fact on the day.

Operators assume one shared job role rather than one role each. A role models a job, not a person, so operators doing the same job assume the same role and one policy governs them all; two roles holding the same policy would drift apart the first time only one was updated. That role carries staging and the reads a deploy makes, and reaches no production resource, because deploying to production is a closely guarded operation and a contributor who develops and tests does not need it. Sharing costs no attribution, because the role's trust policy requires the session name to equal the assuming user's own user name: the trail records the person rather than only the job, answering who acted never requires opening a second identity, and a workstation configured otherwise is refused at the point of assuming the role rather than quietly logging somebody else's name. This is the security pillar's design principle of maintaining traceability, carried by the pattern AWS documents for it, a trust policy requiring `sts:RoleSessionName` to equal `${aws:username}`. It names the person for the hop that assumes the job role and no further: a job-role session that chains into the staging runtime role carries a session name the SDK generates, so an action taken under the runtime role is traced to the person through the recorded assumption of that role, not by a name it carries itself. Source identity (`sts:SetSourceIdentity`), the mechanism AWS provides for carrying the person through a chain, is not part of this design. A second, wider role is not declared, because the directly authenticated identity below already carries the whole estate and a role duplicating it would be a second way to the same place with a second policy to keep in step; a further role is warranted only when somebody needs permissions neither carries. The policy is derived from the observed usage of every operator who assumes the role, not one of them, since a policy derived from a single person's history under-grants the others and surfaces as a permission error mid-task.

The job role holds no permission over its own definition, and that restraint is carried as explicit denials inside the policy the role itself holds, because a single-account organisation has nowhere else to put it. An organisation service control policy never applies to the management account, and in an organisation of one the only account is the management account, so the guardrails such a policy carries elsewhere are structurally unavailable here rather than merely disproportionate. The denials name the role's own definition, organisation membership and account closure, every mutation of the directly authenticated identity and its second factor, and the whole lifecycle of a human operator: creating or deleting a user, minting or revoking an access key, attaching a policy, registering a second factor. That last denial is what makes the lifecycle tooling's own refusal more than a convention, because an operator who bypasses the wrapper and calls the API directly is refused by AWS rather than by a script they chose not to run, and revoking access is never the slower half. It follows that the Terraform tree declaring the role is applied by the directly authenticated identity rather than by the role, and it is the only tree an operator cannot apply. Declaring a role is privileged administration, rare and deliberate, and it is ordinary work for the identity that does it rather than an emergency. What does not change is that the role and its policy are declared in code like all other steady-state IAM, since the restriction is on who applies that tree and not on whether it exists.

A directly authenticated identity exists that does not depend on the job role being assumable. If that role or its trust policy fails, every operator session fails with it, so a way back in routed through the same mechanism would be unavailable exactly when it is needed; the security pillar names that dependency as an anti-pattern in its guidance on establishing an emergency access process, and the same guidance sanctions a small number of pre-created identities holding long-lived credentials for exactly this purpose. That identity is the IAM user `footbag-operator`: its credentials are held by the operators who carry the whole estate, every onboarding and offboarding runs as it, and it applies the tree declaring what an operator may do, which the job role is denied by design. It stands alongside a hardened account root rather than in place of it: root is the account's own last fallback and its custody is governed separately, but root has no least-privilege story at all and no way to act as anything narrower, so a directly authenticated identity narrower than root and broader than a scoped role is the rung between them. Because an identity that cannot resolve the failure is no use as a fallback, breadth of permission on it is acceptable where breadth of permission in a scoped role is not.

One identity therefore carries both jobs, and the cost is accepted rather than hidden. The guidance asks for an emergency identity used only in emergencies, alarmed on, and with its credentials rotated after each use; an identity that also does routine privileged work satisfies none of those, because routine and emergency use are indistinguishable in the trail. Rotation therefore runs on a named trigger rather than on use: an operator leaving, a suspected exposure, a credential that has been somewhere it should not. What this design does keep from the guidance is the written process: the failure modes that count are written down, confirming one is a documented step rather than a judgement call in the moment, and the path is walked on a schedule so its first real use is not its first use. The dedicated emergency AWS account, organisation-level protection of the cross-account roles, and central log-archive account that the full guidance describes are out of scope at this project's size and are not part of this design, and that is what leaves one identity doing both jobs.

The default for every run is the directly authenticated identity on AWS and the shared `footbag` account on the host, and a run acts as a named person, on both halves together, only when one flag deliberately puts it there: the wrapper, given that person's account name. Neither half is selected by an environment variable an operator must remember to export. Which host account a run connects as is decided by the SSH alias in that workstation's own `~/.ssh/config`, as `ssh -G` resolves it: no script passes `-l` or `user@host`, and no file in the repository names a person. The alias's own stanza names the shared account and is never edited by hiring or firing. Onboarding adds one block beside it that applies only while the wrapper has put the run on the job role, so a wrapped command connects as the named account and every other run as the shared one; firing removes that block. Both show the change before making it, nothing is maintained by hand, and nothing invents a stanza a workstation does not already carry. The operator's sudo credentials follow from it mechanically: one file per host account and environment, each holding one credential permanently, with the shared account's pair and the named account's pair living side by side on a workstation that holds both. No file ever holds a different credential at a different time, because a file whose meaning depends on when it is read gives an operator nothing to check, and the failure is a permissions error on the host that reads as a broken account. A run whose selected file is absent is refused by name rather than falling back to the other pair, since a silent fallback attributes a named operator's work to the shared account. The AWS credential is selected through a profile, which the shared script library supplies for the same reason. The directly authenticated identity keeps the profile name every operator script already uses, and the credential behind it is unchanged, so no script, no runbook and no established operator's everyday work learns a new name. The job role takes a profile of its own, chaining from the operator's own named-user profile, so the two never collide. Which identity a command ran as is what STS reports for the caller, never which profile supplied the credential. The directly authenticated identity remains the default for every run, and a command acts as the job role only where it has deliberately been put there, by being run through the wrapper that settles that profile and then proves the session it obtained; nothing is exported and the choice dies with the command. A run that reaches both a host and AWS derives a person's name twice over, once from each mechanism, and the two are held to being the same person: a run whose STS session name and whose host account disagree is refused before it acts, because its two halves would otherwise be recorded under two names with nothing saying so. That comparison applies only where the AWS side is a session of the job role, which is the one principal here that carries a person's name; the directly authenticated identity has no session name at all, and a runtime role's describes a workload.

An operator who holds the shared account's password can therefore connect as the shared `footbag` account at one time and as their own named account at another, indefinitely, rather than migrating from one to the other; any one run connects as exactly one of them: the shared account by default, the named account when the run is wrapped as that person. In the same way, a `footbag-operator` holder acts as the IAM user `footbag-operator` in one run and as a session of the job role, through their own named IAM user, in another; any one run acts as exactly one of them, the one STS reports. Holding both credentials on one workstation is a fact about the machine, never one person acting as two principals. A dev-and-tester has only their named host account and their named IAM user, and so acts only through the wrapper; an unwrapped run of theirs has no credential and fails. That is a consequence of the shared account remaining the break-glass route rather than a transitional state.

The runtime role's trust policy may also list a non-host AWS principal (the IAM user `footbag-operator` on both environments, and on staging the `FootbagDevTester` role) so the operator workstation can chain into the runtime role for read-only health probes such as the staging readiness smoke test. The chained AssumeRole derives short-lived credentials and inherits only the runtime role's narrowly scoped permissions. No second factor is used on the chained AssumeRole, which keeps unattended smoke runs possible; the bound is the runtime role's own narrow scope, which on staging reaches only staging resources.
...
- The pin governs **every** connection to a deployed host, and it achieves that by every connection being made by a script rather than by an operator. Scripts pass `StrictHostKeyChecking=yes` and `UserKnownHostsFile` on their own command line, which outranks any configuration file, so the pin holds regardless of how a workstation is set up. No runbook, card or guide hands an operator a raw `ssh` or `scp` command against a deployed host: a hand-typed command cannot carry the pin without the operator editing their own SSH configuration, which is theirs rather than this project's, so the answer is that the command is a script. That rule is what closes the gap, not a setting on anybody's machine. A runbook step that reaches a host by hand is a defect in the runbook.
- The pin is built by script from the cloud provider's host-key record and the environment's Terraform output, never hand-assembled. It is written for every algorithm on every SSH port the hosts listen on, since a non-default port matches only in the bracketed form, and the result is proved by reading it back rather than trusted. A hand-built pin fails silently: a malformed line is not found rather than rejected, and the symptom arrives much later as a refused connection that reads as a network fault.
- Both environments authenticate the workload the same way: a source-profile IAM user on the host chains by `sts:AssumeRole` into `footbag-<environment>-app-runtime`, with no condition on either trust, and each runtime role also names the directly authenticated user `footbag-operator` by literal ARN, permanently — that is the way back in when the job role is what failed, and AWS resolves a literal-ARN trust to the user's internal unique id, so removing it is not undone by recreating the user. The two trusts differ in exactly one principal, deliberately: staging's list carries the shared job role `FootbagDevTester`, because the reads a deploy makes are part of that job, and production's tree declares no variable that could add it. That absence is the enforcement rather than a divergence to reconcile: the role's bound, staging and the reads a deploy makes, is a property of the configuration instead of of anyone's care. The identity tree reinforces it by scoping that role's `sts:AssumeRole` grant to the staging runtime role alone, and the account-baseline check audits the pair for opposite answers — staging must name the role, and production naming it is a finding whose remedy is to remove the principal and apply production.
- The shared job role `FootbagDevTester` carries staging and the reads a deploy makes, and nothing else. It is granted staging's resources by name where a name can scope them, the staging Terraform state key, the reads the operator scripts and a deploy make, and `sts:AssumeRole` into the staging runtime role alone. Where a resource's ARN cannot carry its environment, the boundary is drawn by action, and by a denial on the `Environment` tag that fails closed, so an instance carrying no tag is refused as production is. It never holds: any permission over its own definition; any mutation of `footbag-operator`; the lifecycle of a human operator; organisation membership or account closure; the Lightsail call that mints a host-access certificate, on any instance; the firewall and delete calls on any instance not tagged staging; or attaching, detaching or releasing a static IP, because Lightsail cannot tag one and its ARN carries a generated id, so no condition could keep those calls off production's address. No second factor is used on this role or its chain; each grant is bounded instead. Every denial is simulated against the live role by `scripts/verify-operator-role-denials.sh` and pinned by a unit test against the Terraform.
- systemd units run with the hardening directives appropriate to what the unit actually executes, and a documented start-rate limit so a crash loop cannot overwhelm the host. The distinction is what the unit runs, not what it manages. A unit whose command is a client for the container runtime confines only that client, never the containers it starts, because those are spawned by the container daemon and inherit nothing from the unit; such a unit takes `NoNewPrivileges=true` and `PrivateTmp=true`, and its containers are confined by the compose-level capability and privilege settings above instead. A read-only system tree does not apply to it, since talking to the runtime means writing to the runtime's socket. A unit that runs a real workload takes the full set: a read-only system tree with only the paths it writes granted back, a private temp directory, no new privileges, and home directories read-only where it reads a credential file. Every granted write path is stated literally in the unit rather than derived from an environment variable, so the sandbox and the script cannot drift apart unnoticed.
...
- DEVOPS_GUIDE.md (private GitHub repo) and the onboarding documentation must define the canonical SSH operating rules: named non-root operator accounts, `sudo` usage, public-key installation flow, private-key custody rules, firewall source-IP restriction, the credential vault as the sole record of host access, onboarding steps, offboarding steps, and break-glass expectations.
...
- Terraform-managed infrastructure must include the IAM roles/policies, Lightsail firewall restrictions, and any documented bootstrap inputs required by the SSH access model.
...
CloudWatch metrics and alarms monitor key system health indicators: application errors (5xx rates, exceptions), Lightsail instance health (CPU, memory, disk), S3 operation failures, SES bounce rates, backup job failures or missed runs, Stripe webhook processing failures. SNS topics deliver alarms to the administrator mailbox, including alarms for KMS error rates/latency (auth signing, ballot encryption) and alerts for unusual Parameter Store access patterns.
...
In-process rate limiting via an in-memory bucket service (not a route middleware; see §1.9): state-changing actions are bucketed per action and request-derived key (per-member, per-IP, per-target), with bucket sizes and windows from system config. App-side rate limiting state (including IP-based counters for login/reset) is kept in memory only and is not persisted to the database or logs. Cloudflare Turnstile CAPTCHA gates login, register, password-reset, claim-lookup, and verify-email-resend form submissions; the server verifies the Turnstile response token before any DB read. Turnstile runs in Managed mode (Cloudflare-recommended default), which completes without user interaction for low-risk sessions and escalates to a checkbox challenge for higher-risk sessions. AWS Shield Standard, automatic on the CloudFront distribution, covers volumetric L3/L4 DDoS at no additional cost. A CloudWatch origin-spike alarm notifies the operator when sustained per-minute origin request volume exceeds the configured threshold. No edge-layer application abuse rule engine. No managed WAF or AI-based bot detection beyond Turnstile's risk scoring. The bucket store is bounded twice: expired buckets are swept on a schedule amortised across ordinary writes, and a hard ceiling evicts when a flood outruns the sweep. Every bucket is judged against its own action's window, because one store holds all of them and the windows differ by action. A caller may also set a cooldown, a lockout that outlives the counting window and runs from the attempt that crossed the ceiling; login uses one, so the penalty for exhausting the attempts does not shrink the later in a window an attacker starts. A bucket serving a cooldown counts as actively refusing and is not evicted. Eviction takes buckets that are not currently refusing attempts ahead of any that are, so reaching the ceiling can never clear a limit that is actively being enforced.
...
- Upload operation caps (application-level, per member) are admin-tunable via `system_config_current`; the normative default values live in USER_STORIES.md "Configurable Parameters" / DATA_MODEL.md "Seed Data" (`photo_upload_rate_limit_per_hour`, `video_submission_rate_limit_per_hour`).
...
Recovery: RPO 5 to 10 minutes for the most recent point, which the generations never affect. Reaching further back, the granularity available is what the generation for that age retains: within two days, the fine-grained stream; within a month, the nearest hour; within a year, the nearest day.

Cross-Region Disaster Recovery Replication (continuous):

Purpose: Protection against catastrophic regional failures.

Process: S3 replication copies objects from the primary snapshots bucket to a bucket in the backup region as they are written, with Object Lock in governance mode and its own lifecycle rules. Replication is scoped to the hourly and daily generations rather than the fine-grained stream: the off-region copy is a disaster hedge, and Object Lock holds every object it receives for the full retention window, so replicating a snapshot every few minutes buys ninety days of near-identical copies and the cross-region transfer to match.
...
S3 bucket configuration: point-in-time recovery comes from the snapshots themselves, each written under its own timestamped key and aged out by the lifecycle rules for its generation. Versioning is enabled on the primary backup bucket as a safety net against an overwrite or a stray delete, with its own expiry for superseded versions; it is not the mechanism that provides the recovery window. The cross-region backup bucket uses Object Lock (WORM - Write Once Read Many) in governance mode, so a compromised production credential cannot remove a copy, and carries lifecycle rules matched to that lock. Cross-region protection is provided by continuous replication of the promoted generations.
...
- Costs are bounded by the lifecycle rules that expire each generation at the end of its window.
...
Backup retention windows support data deletion policy. On the primary snapshot bucket the recovery window is set by the per-generation lifecycle rules: in production, two days of the fine-grained stream, thirty days of the hourly generation, four hundred days of the daily one; staging keeps deliberately shorter windows. The Object Lock retention on the cross-region disaster-recovery bucket is 90 days, set in Terraform beside the bucket it governs rather than in the configuration store, because nothing in the application reads or changes it. The normative default for the remaining backup retention value is defined in the Configurable Parameters section of the user stories: `audit_retention_days` (default: 7 years / 2555 days) records how long audit history must be kept.
...
Container memory limits: Docker memory limits are explicitly set for each container preventing unbounded memory consumption. Configuration: the `deploy.resources.limits.memory` directive, env-driven per environment (production: web 512M, worker 384M). These are deployment configuration values for container resource management, not Administrator-configurable application parameters.
...
All steady-state AWS infrastructure is defined in Terraform configuration files version-controlled in the repository under `/terraform`. A one-time manual bootstrap is allowed only to provision the AWS account baseline, the first operator principal that subsequently runs Terraform, and the Terraform remote-state S3 bucket; all steady-state IAM (source-profile users, the human job role and its policy, runtime roles, policies, instance profiles) is defined in Terraform. The bootstrap principal is the directly authenticated IAM user `footbag-operator`, and it is what the operator model is then built with, so it is the one identity that cannot itself be declared in code. Named human operator users are the deliberate exception and are not declared in Terraform at all: their lifecycle mints and revokes access-key secrets, and a secret Terraform creates is a secret held in its state, so they are created and retired by a tested script instead. That script is infrastructure discipline rather than an ad-hoc manual path, carrying caller preconditions, a deterministic policy shape, secret-safe credential handling, verification, offboarding, cleanup semantics and companion tests. After that handoff, manual console changes are prohibited except for emergency incident response, and any emergency change must be reconciled back into Terraform before the next `terraform plan` or `apply`. Terraform remote state is held in an S3 bucket with S3 native locking (`use_lockfile = true`, Terraform >= 1.11); DynamoDB locking and Terraform Cloud are excluded.
...
- S3 buckets with complete configuration (the primary snapshot bucket with versioning and per-generation lifecycle rules; the cross-region snapshot bucket with Object Lock).
...
- Route53 DNS records.
- SES email identities (sender, plus future bounce and complaint feed configuration).
- Budget alerts and SNS topics.
- Account-level security controls: the account-wide public-access block on storage, the IAM account password policy, IAM Access Analyzer with the account as its zone of trust, and the account's billing, operations and security alternate contacts. These belong to the account rather than to an environment, so they live in the shared tree and are applied by the directly authenticated identity; the human job role cannot read that tree's state.
...
Committed HCL carries each variable's declaration and description; the values live in a `terraform.tfvars` per environment, and that file is not held in this repository. It lives in the maintainers' private operations checkout and is reached from `terraform/<environment>/` through a gitignored symlink whose name is canonical on every maintainer machine, so Terraform loads it from the working directory exactly as it would a local file and no command, script, or document changes. What decides the placement is the content, not whether the file feels operational: the values carry operator CIDR ranges, which are personal data, and a gitignored file in a public repository is kept out of the history but not out of reach of an accidental commit. Any variable its declaration marks `sensitive` is excluded from that file and from every repository, and is supplied instead through a companion `secrets.auto.tfvars` reached by the same symlink convention, with the credential vault holding the canonical copy; Terraform auto-loads that filename, so no secret is ever passed as a command-line argument where any account on the host could read it. Hiding a value is not itself a security control — the controls are least privilege on the credentials and the audit trail — so what this buys is keeping personal data out of a public repository and denying a reader a free map of the estate, and nothing more is claimed for it. `terraform.tfvars.example` stays tracked here and carries placeholders that a variable validation or a `lifecycle` precondition rejects, so a placeholder cannot reach an apply.

Secrets Management:

Terraform creates Parameter Store parameters (paths and metadata) but does not store secret values in version control. Secret values (Stripe API keys, Stripe webhook secrets, and other non-KMS credentials) are set manually via AWS CLI or secure deployment pipeline after Terraform creates parameter structure. Terraform references secrets via parameter names, and every secret parameter is declared as a shell — the name, the type and the KMS key — carrying a placeholder value and `lifecycle { ignore_changes = [value] }`, so no secret value appears in a `.tf` file or in state. That shape is not cosmetic and the exception below is the reason it is stated so plainly: a value Terraform generates or reads back is written to state in plaintext by construction, and a saved plan is a zip archive that no text-based secret scan can see into. One such plan, committed under a filename an ignore rule did not match, held two live secrets in public history for seven weeks. There is exactly one deliberate exception: the origin-verify shared secret, which CloudFront must carry as a custom header, so its value is part of the distribution's own configuration and therefore in state no matter how the parameter is provisioned. It is treated accordingly — rotatable in one apply, and named in the incident-response procedure as a value to burn if state is ever exposed. JWT signing keys and ballot encryption keys use AWS KMS (non-exportable key material) and are provisioned via Terraform KMS resources; they are never stored in Parameter Store.

Requirements:

- Each environment's `backend.tf` names its state bucket and key as literals. The bucket is shared across environments and the key scopes each environment to its own object, so the values are reviewable in the repository and cannot vary per operator; supplying them at `terraform init` time instead would let an omitted or mistyped flag initialize against the wrong state silently. Locking is S3-native (`use_lockfile = true`); no lock table exists to configure.
...
- Workspace layout: `terraform/shared/` for one-time bootstrap (state bucket, account baseline); `terraform/staging/` and `terraform/production/` for per-environment resources, each with its own remote state; and one account-level tree, `terraform/identity/`, which declares what a human operator may do and is applied only by the directly authenticated bootstrap principal, because a role able to rewrite its own policy has no least-privilege story. Who the operators are is not a tree, because joining and leaving mint and revoke key material that must not enter Terraform state, and revoking somebody's access must never be the slower half.
...
The platform implements three-layer monitoring covering infrastructure health, application behavior, and business operations. CloudWatch provides the monitoring substrate. Signals come from three places: the metrics AWS publishes for the services in use, metric filters over the application's structured logs, and a small set of custom metrics the host backup script publishes directly. Application code publishes no metrics of its own, so every application-level signal is one a log line can carry. Alerts route through CloudWatch Alarms to SNS topics with email notification. Two alert severities (warning, critical) have different response time expectations and escalation paths. CloudWatch is the primary monitoring system; optional external error tracking/APM tooling may be adopted if it is budget-appropriate and reduces operational risk.
...
Layer 1 - Infrastructure Metrics: Lightsail instance CPU, memory and root-volume disk use, collected by the CloudWatch agent, which gathers only what the alarms and the operations dashboard read. Alongside it, the metrics AWS publishes unasked for the services in use: distribution errors and latency, and object-store replication health. Container-level metrics are not collected: on a single instance a failing container surfaces as host pressure, as an application error, or as the deploy's own readiness failure, and all three are watched. These detect resource exhaustion and infrastructure failures.

Layer 2 - Application Signals (CloudWatch log metric filters): Application error rate, login successes, outbox depth, and webhook delivery failures, each counted from the structured log stream. Origin request latency comes from the CloudFront distribution rather than from the application, so it measures origin processing and not the edge and network time in front of it. These detect application bugs and performance degradation at the granularity a log line supports, which is per-signal rather than per-route.

Layer 3 - Business Figures (read from the database through administrator surfaces): Member registrations, photo uploads, payment transactions, email delivery and event registrations are read on demand by an administrator from the records themselves, not published as metrics and not alarmed on. Email delivery is the exception that reaches CloudWatch, through the provider's own bounce and complaint metrics.

Alert Severities: Warning-level: Email to the primary maintainer, handled in the ordinary course of a day, indicates

degraded but functional state. Examples: CPU \>80% for 10 minutes, P95 origin latency \>2 seconds over two consecutive 5-minute periods.

Critical-level: Email to the primary maintainer, handled as soon as the maintainer is reachable, indicates service disruption or imminent failure. Examples: host CPU, memory or disk \>85% for 3 minutes, 5xx rate \>5% for 2 minutes. Database: backup age \>15 minutes, 3 consecutive backup failures, generation promotion failing on every run for an hour. Database file growth is watched through host disk rather than as a signal of its own, and the engine's internal health (lock contention, write-ahead log size, checkpoint latency) is not instrumented: on a single instance those surface as host pressure or as application errors, both of which are alarmed.

Dashboards:

Operations Dashboard: a CloudWatch dashboard carrying distribution error rates, database backup age, and host CPU and memory. Technical audience only. Business figures have no dashboard of their own: an administrator reads member activity, payments, email delivery and the moderation queue from the administrative surface that owns each one.
