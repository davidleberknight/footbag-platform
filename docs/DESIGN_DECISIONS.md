...
- The Google Workspace that serves these addresses has `footbag.org` as its primary domain, so each canonical address is a first-class address of the domain: it is what a member sees in the From line, and it is the address Calendar invitations and sharing notices are sent from. An address reachable only as an alias of some other domain does not satisfy this list.
...
Operator AWS identity is federated, and the authoritative operator principal is an assumed role rather than a long-lived user. Each human operator authenticates as themselves through a centralized identity provider and assumes a role to act; no operator holds a long-lived AWS access key for work done under a role. This follows the AWS Well-Architected security pillar on both counts: its guidance on temporary credentials pairs the move away from long-lived credentials with a move away from IAM users in favour of federation and roles, and its guidance on relying on a centralized identity provider names IAM Identity Center as the route for a single-account organisation. Per-person identity and short-lived credentials are therefore one change rather than two, because the identity provider issues a distinct identity per person and hands out temporary credentials by construction. Provisioning one long-lived IAM user per operator is explicitly rejected: it builds the thing that guidance moves away from and would have to be dismantled to reach the same end state.

Sign-in to that federated identity carries a second factor every time rather than only when the provider judges the sign-in context to have changed, and an operator who has not yet registered a device registers one at sign-in rather than being admitted on a mailed one-time code. The security pillar's guidance on strong sign-in mechanisms is the reason, and the stricter of the two settings is chosen because the identity being protected can apply Terraform across the whole account, which makes a per-sign-in factor cheap against what a taken session reaches.

Operators assume one of two shared roles, split by what a job needs rather than by who holds it, and attribution does not depend on the split. A role models a job, not a person, so operators doing the same job assume the same role and one policy governs them all; two roles holding the same policy would drift apart the first time only one was updated. The split exists because two jobs genuinely differ. A super-admin role carries the whole estate, including production deploys and the roster that hires and fires. A dev-and-tester role carries staging and the reads a deploy makes and reaches no production resource, because deploying to production is a closely guarded operation and a contributor who develops and tests does not need it. The trail records the identity that assumed either role against every action it takes, so sharing a role loses no attribution; what it records is the identity provider's own user identifier rather than a person's name, so both roles carry directory read, and answering who acted never requires leaving the role or opening a second identity. A further role is warranted only when somebody genuinely needs a permission set neither of these carries. Each policy is derived from the observed usage of every operator who assumes that role, not one of them, since a policy derived from a single person's history under-grants the others and surfaces as a permission error mid-task. The narrower policy is derived as a strict subset of the wider one rather than written beside it, so a grant added to one is visible as absent from the other.

The operator roles hold no permission over their own definition, and that restraint is carried as explicit denials inside the policies the roles themselves hold, because a single-account organisation has nowhere else to put it. An organisation service control policy never applies to the management account, and in an organisation of one the only account is the management account, so the guardrails such a policy carries elsewhere are structurally unavailable here rather than merely disproportionate. The denials name the roles' own definitions in the identity provider, the provider's directory, organisation membership and account closure, and every mutation of the super-admin identity, its keys and its second factor. It follows that the Terraform tree declaring the roles is applied by the super-admin identity rather than by a role, and it is the only tree an operator cannot apply. Declaring a role is privileged administration of the federation, rare and deliberate, and it is ordinary work for the identity that does it rather than an emergency. What does not change is that the roles and their policies are declared in code like all other steady-state IAM, since the restriction is on who applies that tree and not on whether it exists. Assigning an existing role to a newly joined operator stays ordinary work, because handing somebody a role that already exists mints no permission that did not.

A directly authenticated identity exists that does not depend on federation. If the identity provider or its configuration fails, federated sign-in fails with it, so a way back in routed through the same mechanism would be unavailable exactly when it is needed; the security pillar names that dependency as an anti-pattern in its guidance on establishing an emergency access process. That identity is the super-admin identity: held by the operators who carry the whole estate, it is what onboards everybody else, and it applies the tree declaring what an operator may do, which the operator roles are denied by design. It stands alongside a hardened account root rather than in place of it: root is the account's own last fallback and its custody is governed separately, but root has no least-privilege story at all and no way to act as anything narrower, so a directly authenticated identity narrower than root and broader than a scoped role is the rung between them. Because an identity that cannot resolve the failure is no use as a fallback, breadth of permission on it is acceptable where breadth of permission in a scoped role is not.
...
- Workspace layout: `terraform/shared/` for one-time bootstrap (state bucket, account baseline); `terraform/staging/` and `terraform/production/` for per-environment resources, each with its own remote state; and two account-level trees for human operator identity, separated by who applies them: `terraform/identity/` declares what an operator may do and is applied only by the directly authenticated bootstrap principal, because a role able to rewrite its own policy has no least-privilege story, and `terraform/operators/` holds the roster of who the operators are and is applied by any super admin as themselves, because joining and leaving are ordinary work and revoking somebody's access must never be the slower half.
