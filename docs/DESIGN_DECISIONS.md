...
- The application's IAM grant to SES names the specific actions the application uses (`ses:SendEmail`, `ses:SendRawEmail`), never `ses:*`, and bounds them to the verified sending identity through a `ses:FromAddress` condition rather than through the resource ARN. The condition carries the bound because the resource cannot: while the account is inside the SES sandbox, SES authorises each send against the destination identity as well as the sender, so a resource pinned to the sender refuses every message to a real verified recipient; and once domain authentication is enabled the identity becomes the whole domain, at which point a resource-scoped grant would authorise sending as any address at it. The Configuration Set used for sending is named explicitly in the policy.
