...
- The delegated-subzone arrangement offered to the legacy webmaster runs to go-live and ends there; retention past go-live is refused absent an accepted written technical justification. The coordination record lives in the go-live plan (private GitHub repo) rather than here.
...
Operator AWS identity is a named IAM user per person, and the authoritative operator principal is an assumed role rather than that user. Each human operator authenticates as themselves and assumes one shared job role to act, so the credentials every action carries are short-lived and scoped to the job. The named user holds no service permission of its own: its single inline policy allows one action, `sts:AssumeRole`, against one role. This departs from the AWS Well-Architected security pillar, whose guidance on temporary credentials pairs the move away from long-lived credentials with a move away from IAM users in favour of federation, and whose guidance on a centralized identity provider carries no exception for an estate of this size. The departure is deliberate and is accepted as a cost rather than claimed as compliance. What the design keeps is the part that carries the weight: no human credential reaches a service directly, and the credential that does reach services expires.
...
A directly authenticated identity exists that does not depend on the job role being assumable. If that role or its trust policy fails, every operator session fails with it, so a way back in routed through the same mechanism would be unavailable exactly when it is needed; the security pillar names that dependency as an anti-pattern in its guidance on establishing an emergency access process, and the same guidance sanctions a small number of pre-created identities holding long-lived credentials for exactly this purpose. That identity is the super-admin identity: held by the operators who carry the whole estate, it is what onboards and offboards everybody else, and it applies the tree declaring what an operator may do, which the job role is denied by design. It stands alongside a hardened account root rather than in place of it: root is the account's own last fallback and its custody is governed separately, but root has no least-privilege story at all and no way to act as anything narrower, so a directly authenticated identity narrower than root and broader than a scoped role is the rung between them. Because an identity that cannot resolve the failure is no use as a fallback, breadth of permission on it is acceptable where breadth of permission in a scoped role is not.

One identity therefore carries both jobs, and the cost is accepted rather than hidden. The guidance asks for an emergency identity used only in emergencies, alarmed on, and with its credentials rotated after each use; an identity that also does routine super-admin work satisfies none of those, because routine and emergency use are indistinguishable in the trail. Rotation therefore runs on a named trigger rather than on use: an operator leaving, a suspected exposure, a credential that has been somewhere it should not. What this design does keep from the guidance is the written process: the failure modes that count are written down, confirming one is a documented step rather than a judgement call in the moment, and the path is walked on a schedule so its first real use is not its first use. The dedicated emergency AWS account, organisation-level protection of the cross-account roles, and central log-archive account that the full guidance describes are out of scope at this project's size and are not part of this design, and that is what leaves one identity doing both jobs.

Host identity and AWS identity are selected by different mechanisms, and neither is selected by an environment variable an operator must remember to export. Which host account a run connects as is decided solely by the `User` line of the SSH alias in that workstation's own `~/.ssh/config`: no script passes `-l` or `user@host`, and no file in the repository names a person, so changing that one line is the whole act of switching identity. The operator's sudo credentials follow from it mechanically: one file per identity and environment, each holding one credential permanently, with the shared account's pair and the named account's pair living side by side. No file ever holds a different credential at a different time, because a file whose meaning depends on when it is read gives an operator nothing to check, and the failure is a permissions error on the host that reads as a broken account. A run whose selected file is absent is refused by name rather than falling back to the other pair, since a silent fallback attributes a named operator's work to the shared account. The AWS identity is selected by the profile, which the shared script library supplies for the same reason. The directly authenticated identity keeps the profile name every operator script already uses, and the credential behind it is unchanged, so no script, no runbook and no established operator's everyday work learns a new name. The job role takes a profile name of its own, chaining from the operator's own named-user profile, so the two never collide and which identity a command runs as is a property of the profile it names rather than of which credential happens to resolve first. A dev-and-tester holds their own named profile and the job-role profile beside it, and never the directly authenticated one.

An operator therefore holds both identities at once and moves between them freely, indefinitely, rather than migrating from one to the other. That is a consequence of the shared account remaining the break-glass route rather than a transitional state.

The runtime role's trust policy may also list a non-host AWS principal (the operator principal) so the operator workstation can chain into the runtime role for read-only health probes such as the staging readiness smoke test. The chained AssumeRole derives short-lived credentials and inherits only the runtime role's narrowly scoped permissions. Where the runtime role's permissions are a strict subset of the operator's existing permissions, MFA on the chained AssumeRole is not load-bearing and may be omitted to permit unattended smoke runs; where the runtime role grants permissions the operator would not otherwise hold, MFA on the chain remains required.
