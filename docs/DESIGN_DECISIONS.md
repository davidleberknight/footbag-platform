...
Mail routing is independent of the web records: MX is unaffected by address-record changes, and nothing in the web path touches it.
...
- Mail is decoupled from the web records: MX routing is independent of A/AAAA changes, and the web path never touches MX.
...
Before go-live the position is transitional. Legacy names are served by another operator until the zone move completes, and a delegated subzone may be carried through the migration window as a bridge while an outgoing operator relocates what they run. That bridge ends at go-live. Retaining any name or mailbox under the domain past go-live requires a written technical justification that IFPA accepts, weighed against the reasons below; prior arrangement, convenience, or long tenure is not one.

Where a name under the domain still resolves to infrastructure IFPA does not control, the apex certificate-authorisation record is what bounds the risk: it is what stops that name obtaining a publicly trusted certificate, and so what stops it answering over HTTPS and receiving the archive's parent-scoped cookies.
...
- The strict-transport subdomain directive and any future preload submission commit every name under the domain, which is consistent with this decision rather than constrained by it.
