...
|---|---|---|---|---|
| `__Host-footbag_session` | `issueSessionCookie` in `src/lib/sessionCookie.ts` | RS256 signature; private key in AWS KMS (production) or a local PEM (dev and test), per §3.5 | 24 hours | JWT claims: `sub` (member id), `passwordVersion`, `role` hint, standard `iat`/`exp` (§3.4) |
...
Backup retention windows support data deletion policy. On the primary snapshot bucket the recovery window is set by the per-generation lifecycle rules: in production, two days of the fine-grained stream, thirty days of the hourly generation, four hundred days of the daily one; staging keeps deliberately shorter windows. The normative defaults for the remaining backup retention values are defined in the Configurable Parameters section of the user stories: `cross_region_backup_retention_days` (default: 90 days) records the Object Lock retention on the cross-region disaster-recovery bucket, and `audit_retention_days` (default: 7 years / 2555 days) records how long audit history must be kept.
