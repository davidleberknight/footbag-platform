...
| `noreply@footbag.org` | Transactional sender (account verification, password reset, receipts, system notifications); never monitored, never a reply target. **Target state, not current.** The domain is not held yet, so nothing can receive the verification click SES requires, and production sends from an interim verified identity until the zone moves. The address in force is the `ses_sender_identity` Terraform output, never a value written down | Sends | `CommunicationService.processSendQueue` via SES |
...
Go-live is the operator lifting the migration notice on Route 53-served infrastructure the domain already points at. Before go-live the `footbag.org` zone moves to Route 53 under IFPA-controlled access as advance preparation: the zone is mirrored from the webmaster's bind9 servers (fresh zone snapshot first, verified record-for-record), the registrar name-server change is made in the IFPA-held registrar account, and the webmaster's bind9 service retires once nothing resolves through it. At the switch `www.footbag.org` and the apex `footbag.org` become Route 53 ALIAS records to the production CloudFront distribution; `www` is the canonical host and the apex 301-redirects to `www` through the distribution. No separate apex redirector exists. The legacy site leaves the live path at the switch: no reverse proxy or front door on the legacy server, and no legacy TLS certificate. Recovery is fix-forward or a platform restore first, with any DNS change operator-made on the pre-lowered low-TTL zone; no legacy server remains running for rollback.

The migration notice lives in the CloudFront viewer-request function on the default behaviour (the same function that carries the apex redirect), behind a compile-time flag, and is served per-hostname: `www` answers 503 with the inline notice while `preview.footbag.org` and the distribution's own address pass through to the platform. It does not live in the `enable_planned_maintenance` origin swap, and the reason is decisive rather than stylistic: the origin swap replaces the default cache behaviour's origin, and that behaviour serves every hostname on the distribution, preview included — a mechanism that blanks preview cannot satisfy the requirement that preview is the real site on the real production database throughout the window. `enable_planned_maintenance` and the S3 maintenance origin remain for what they are good at: the automatic 5xx origin-failure fallback, and deliberate post-launch maintenance windows where blanking every hostname is the intent. The Stripe webhook path is exempt from the notice inside the function; the health path needs no exemption because it rides its own ordered behaviour that the function never runs on. The cutover-window alarms (the CloudFront 5xx alarm, which counts the notice's own 503s, and the zero-logins watch) are conditioned off behind the same flag that raises the notice, so the freeze apply silences them and the launch apply restores them, with no declared date and no hand-armed switch.

The Route 53 apex/`www` alias records (`enable_apex_alias_records`) stay gated off through the zone move — Route 53 first serves the zone's existing records faithfully — and flip at the write-freeze together with the notice flag, so the distribution serves the notice on the public names from that moment and go-live is the notice flag lifting rather than a DNS change. The archive capture completes before the freeze: the crawler reads the legacy site by name with no address override, so the top-up runs and the mirror freezes ahead of the record switch, and nothing after the switch can reach the site to capture. The SES domain-auth records (`ses_enable_domain_auth`) flip once the zone move completes, taking over from any hand-applied copies.
...
- The switch is deliberately the cheapest step in the sequence. Every novel part (certificate issuance, the distribution, the origin path, the archive's signed-cookie flow, and the incremental deploy) is stood up and proved in advance on names that have never resolved: `archive.footbag.org` under its real name, and the temporary `preview.footbag.org` platform subdomain retired after the post-cutover watch window. Go-live therefore changes a record whose target is already known good, instead of exercising anything for the first time inside the freeze window.
- The temporary pre-cutover platform subdomain is `preview.footbag.org`, and the name must be one that has never existed in the legacy zone: the zone move mirrors every existing record faithfully, so a repurposed name would silently override a webmaster-owned record. `test.footbag.org` already exists in the live zone (a legacy CNAME to `v2.footbag.org`) and is preserved untouched, while `preview` carries no record of any type. The name is re-verified against the fresh zone snapshot before its record is created, is a SAN on the production certificate from first issuance (a later addition forces a certificate replacement), and its record is removed once the post-cutover watch window has run, not in the launch act itself: while the notice is up and through the watch, it is the only hostname an operator can still reach the platform through, so removing it at cutover would remove the reversal path.

Requirements:

- The production distribution carries `footbag.org`, `www.footbag.org` and the temporary `preview.footbag.org` as alternate domain names with a matching ACM certificate in us-east-1; the archive distribution carries `archive.footbag.org` with its own us-east-1 certificate. Preview being a first-class alias on the same distribution is load-bearing: it is what the per-hostname notice serves the real site through during the window.
...
- `terraform/production/route53.tf` gains the hosted zone and the mirrored records; `enable_apex_alias_records` flips at the write-freeze, paired with the migration-notice flag on the viewer-request function in `terraform/production/cloudfront.tf` (`enable_planned_maintenance` stays for the 5xx fallback and post-launch windows, and both cutover-window alarms condition off the notice flag); `ses_enable_domain_auth` flips at the zone move; no apex-redirector Terraform exists.
...
- The DNS Cutover decision (§6.11), the Closed Namespace decision (§6.13), the Legacy Archive decision (§6.4), the Sealed Legacy Email Archive decision (§6.5a), and the canonical email architecture (§5.5) implement it.

## 6.13 The Closed `footbag.org` Namespace

Decision:

From go-live, every name under `footbag.org` is operated by IFPA. The served set is the apex, `www.footbag.org`, and `archive.footbag.org`; `preview.footbag.org` exists as a temporary pre-cutover name and retires after the post-cutover watch window. No subzone is delegated to a third party past go-live, and no name under the domain resolves to infrastructure IFPA does not control. A name IFPA wishes to retire is retired; a party wanting a name of their own is directed to a domain they hold.
...
- The archived legacy site contains absolute links to historical sibling hostnames under `footbag.org`. Those names resolve to nothing after cutover; restoring any of them to a third party converts links inside member-only archive pages into live requests carrying archive credentials.
...
- Terraform owns every record in the zone. No child `NS` record set is created under `footbag.org`, and the absence is asserted rather than left to convention.
- The apex CAA record is authoritative for the whole domain, which holds only while no child zone exists.
