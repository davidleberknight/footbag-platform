...
Operators sharing one role is deliberate, and attribution does not depend on splitting it. A role models a job, not a person, so the operators who do the same job — applying Terraform, deploying, running diagnostics — assume one shared human-operator role carrying a single least-privilege policy. Two roles holding the same policy drift apart the first time only one is updated. The trail records the identity that assumed the role against every action that role takes, so a shared role loses no attribution; what it records is the identity provider's own user identifier rather than a person's name, so the role carries directory read as well, and answering who acted never requires leaving the role or opening a second identity. A second operator role is warranted only when a person genuinely needs a different permission set. The policy that role carries is derived from the observed usage of every operator who assumes it, not one of them, since a policy derived from a single person's history under-grants the others and surfaces as a permission error mid-task.

The operator role holds no permission over its own definition, and that restraint is carried as explicit denials inside the policy the role itself holds, because a single-account organisation has nowhere else to put it. An organisation service control policy never applies to the management account, and in an organisation of one the only account is the management account, so the guardrails such a policy carries elsewhere are structurally unavailable here rather than merely disproportionate. The denials name the role's own definition in the identity provider, the provider's directory, organisation membership and account closure, and every mutation of the emergency identity, its keys and its second factor. It follows that the Terraform tree declaring the operator role is applied by the emergency identity rather than by the role, and it is the only tree an operator cannot apply; what does not change is that the role and its policy are declared in code like all other steady-state IAM, since the restriction is on who applies that tree and not on whether it exists. Assigning the existing role to a newly joined operator stays ordinary work, because handing somebody a role that already exists mints no permission that did not.

An emergency access identity exists that does not depend on federation. If the identity provider or its configuration fails, federated sign-in fails with it, so an emergency path routed through the same mechanism would be unavailable exactly when it is needed; the security pillar names that dependency as an anti-pattern in its guidance on establishing an emergency access process. The emergency identity is therefore a directly authenticated one, it is not used for routine work, its use is alarmed on, and its credentials are rotated after use. It stands alongside a hardened account root rather than in place of it: root is the account's own last fallback and its custody is governed separately, but root has no least-privilege story at all and no way to act as anything narrower, so a directly authenticated identity narrower than root and broader than routine work is the rung between them. Because an emergency identity that cannot resolve the emergency is not one, breadth of permission on it is acceptable where breadth of permission in routine use is not. What makes the path real is the process rather than the identity, which the security pillar treats as the practice itself: the failure modes that count as an emergency are written down, confirming one is a documented step rather than a judgement call in the moment, use of the identity raises an alarm, its credentials are rotated afterwards by that identity itself, and the path is walked on a schedule so that its first real use is not its first use. The dedicated emergency AWS account, organisation-level protection of the cross-account roles, and central log-archive account that the full guidance describes are out of scope at this project's size and are not part of this design.

Host identity and AWS identity are selected by different mechanisms, and neither is selected by an environment variable an operator must remember to export. Which host account a run connects as is decided solely by the `User` line of the SSH alias in that workstation's own `~/.ssh/config`: no script passes `-l` or `user@host`, and no file in the repository names a person, so changing that one line is the whole act of switching identity. The operator's sudo credentials follow from it mechanically: one file per identity and environment, each holding one credential permanently, with the shared account's pair and the named account's pair living side by side. No file ever holds a different credential at a different time, because a file whose meaning depends on when it is read gives an operator nothing to check, and the failure is a permissions error on the host that reads as a broken account. A run whose selected file is absent is refused by name rather than falling back to the other pair, since a silent fallback attributes a named operator's work to the shared account. The AWS identity is selected by the profile, which the shared script library supplies for the same reason, and that profile keeps one name across the move to federated sign-in: what changes is the credential behind it, so no script, no runbook and no workstation learns a new name.
