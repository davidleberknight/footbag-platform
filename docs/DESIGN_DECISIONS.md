...
**Self-serve claim flow (umbrella).** The member's experience of claiming pre-existing identity (an old website account, a competition record, or both) is a single act composed of several mechanisms: card confirmations (auto-link candidates surfaced by the wizard, cross-source candidate prompts, registration-time conflict prompts), declared-anchor entry (optional former surnames and optional declared old emails, both member-and-admin private), an optional mailbox-control round-trip via a confirmation link to a declared old email (upgrades the audit evidence tier), and a direct historical-record claim affordance on the historical detail page. The wizard's claim task is the primary entry surface; the profile and historical-record pages are secondary surfaces. Auto-link sends no notification emails. Card-at-login is the only post-link member-facing surface; effects (tier upgrade, attribution, badges) apply only after the member explicitly confirms a card. Every confirmed claim transaction carries an evidence-strength tag on its audit row (`declared_anchor_only`, `currently_controls_modern_email_matching_legacy`, `mailbox_control_via_link_click`, or `admin_vetted_evidence`). The umbrella story and case-by-case mechanics live in the `M_Claim_Legacy_Account` user story.

**Tier handling.** Tier state is written as a single `member_tier_grants` ledger row at claim time (`reason_code = 'legacy.claim_tier_grant'`), applying the blanket annual-to-lifetime mapping (honors-only fallback if the legacy tier-state fields are unavailable). Unclaimed `legacy_members` rows have no ledger row. No tier cache columns exist on `members`; membership-tier reads go through `MembershipTieringService.getTierStatus(memberId)`; Active Player reads go through `ActivePlayerService.getStatus(memberId)`.
...
**Automated legacy-data cleanup.** Cleanup transitions on `legacy_person_club_affiliations`, `club_bootstrap_leaders`, leaderless live clubs, and convergent auto-merge holds run as a daily system process when the predicate is unambiguous (orphan legacy id, purged member, inactive onboarding, stale provisional leader, leaderless active club, convergent auto-merge). Each automated transition writes one `audit_entries` row with `actor_type='system'`. The admin queue surfaces only the residue that requires human judgment. The set of automatic predicates is fixed by `A_Periodic_Club_Cleanup`; adding a predicate or loosening a threshold requires an explicit story extension. Rationale: the migration-window ~1,600-row `pending` backlog plus the long tail of orphan / purged / inactive states would otherwise require unbounded admin attention. Automation removes admin from the loop for cases where the data is definitive; admin remains the sole decision authority for any case requiring judgment (mistaken linkage, junk override, content edits, source-disagreement merges, force-keep requests).
...
- Stage-and-confirm auto-link with no notification emails balances throughput and trust: nothing applies silently, so a wrong candidate match never produces an unnoticed wrong link; the member's wizard confirmation is the gate. Honors-bearing direct claims apply without admin pre-screening because the platform's other defenses (member self-confirmation, registration-time conflict prompt, post-facto admin oversight feed, member-initiated dispute path) cover the realistic threat model in a small community. Members who cannot resolve their identity self-serve use the admin help-request path; admin involvement is reactive, not gating.
