...
> **Recommended one-shot:** `./run_dev.sh` at repo root installs npm deps + Python venv if missing, resets the DB only if missing or stale (pass `--reset` for a fast committed-seed reset, `--from-csv` for the deploy-parity rebuild, or `--soup-to-nuts` for the legacy-mirror rebuild), and launches the dev web server + image worker together with clean Ctrl+C teardown. The rest of §1.7-1.9 is the manual breakdown for diagnostics.
