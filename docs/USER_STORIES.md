...
*Uploader hashtags* identify the source of a media item with a tag that matches the uploader's identity. Member uploads carry the tag `#{member_slug}` automatically. Curator (FH) uploads carry `#curated`. These tags are reserved for the matching uploader: the system applies them on every upload, and rejects them when supplied by anyone else as user input. This makes per-uploader views (a member's Personal Gallery, the all-FH gallery) plain tag-criteria queries that share the same query path as event and club galleries. The slug pattern matches the member URL slug (lowercase letters, digits, underscores), so a member's Personal Gallery is just `criteria_tags = [#{slug}]`. Member-owned named galleries created by the owner also include `#{owner_slug}` in their criteria-tag set automatically, so the gallery's tag-AND filter scopes to the owner's content by default. Edits do not re-add the slug-tag if the owner removes it; once a gallery exists the owner curates its filter intentionally.
...
- If mirror-derived club affiliation suggestions or leadership suggestions exist for the claimed identity, the member is prompted to review them before confirming (see M_Review_Legacy_Club_Data_During_Claim).
- On confirmation, the merge runs atomically: transfers `legacy_member_id`; merges allowed profile fields (import fills only if active value is empty or absent; active account always wins for credentials, display name, and contact fields); applies `first_competition_year` via COALESCE (member value wins if already set, import value fills if member is NULL); writes a single tier grant per the blanket mapping in `MIGRATION_PLAN.md` §3 ("Tier handling at claim"); writes confirmed club affiliations; may promote confirmed leadership into live club leadership; marks the `legacy_members` row claimed (`claimed_by_member_id` + `claimed_at` set; the row is preserved as the permanent archival record).
...
Story: As a member claiming my legacy account (or completing registration onboarding), I can review mirror-derived club suggestions and any leadership assignments so I can confirm or correct club data before the merge is finalized.
...
- Personal Gallery is the default per-member named gallery, with `criteria_tags = [#{member_slug}]`. Items appear via the same tag-AND query that powers every named gallery, because every member upload automatically carries the uploader's slug-tag (per §1.1 Uploader hashtags). Avatars are excluded platform-wide via `is_avatar = 0` at the named-gallery query layer; the rule is not specific to Personal Gallery.
