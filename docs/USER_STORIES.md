...
Story: As an admin, I can review and resolve member-initiated help requests for legacy-identity claims when the platform's self-serve mechanisms (auto-link card confirmation, declared-anchor entry, optional mailbox-link-click round-trip, direct historical-record claim) did not produce a candidate the member could confirm. I can also handle after-the-fact disputes where a member believes a previously-confirmed claim is wrong. Because self-serve legacy claiming is confined to the onboarding wizard, this admin path is also how a member links or corrects a legacy identity after onboarding is complete — a record they did not claim during the wizard, a newly surfaced match, or a correction.
