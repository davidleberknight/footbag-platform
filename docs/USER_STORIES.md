...
- Entry point: the wizard's claim task. A name-matched record the platform surfaces on a card carries a claim control opening the confirmation page (`GET /history/:personId/claim`), reachable from that card and nowhere else; the record's own public detail page (`GET /history/:personId`) carries no claim control for any viewer. The confirmation proceeds only when the record's name ends with the registrant's recorded family name OR with any declared former surname, the record is unclaimed, and the record is not flagged deceased (`historical_persons.is_deceased = 0`).
...
- The link opens a slug-scoped owner-only form (`/members/:slug/contact-admin`) with: a category dropdown (Display name correction, Profile URL correction, Tier-status question, Old account or competition-record link, Group creation request, Vote creation request, Other), a free-text message textarea (required, up to 2000 characters), and a submit button.
...
- Every item resolves in exactly one of two ways, and its options follow from which one it is.
...
- For a recurring donation the comment is stored in Stripe Subscription metadata as well as in the local record, so it survives across all subsequent billing cycles; a renewal is generated by Stripe with no checkout behind it, and the metadata is what carries the context to each one. A one-time gift needs no such carrier, so its comment stays in the local record and is not sent to the payment provider at all.
...
- The history includes at least: date, type (Donation, Membership, Event Registration, etc.), amount, payment status (succeeded, pending, etc.), and a concise descriptor (for example “Membership: Tier 2 IFPA Organizer Member”, “Recurring Annual Donation”, “Event Registration: Worlds 2027 – Singles”). A donation descriptor never carries the donor’s note, which renders in its own field beside it.
...
- Behind the aggregate outbox figures, admins can open a per-message outbound-email log: one row per message showing recipient, template, and delivery status (pending, sent, failed, or dead-lettered). Where the mail provider has reported a bounce or a complaint against that message, the row says so and when, because the first question after a bad send is which send it was, and delivery status alone answers only what happened at the moment of sending. Where a message has a body, it is shown as the underlying template with its data-merge fields left clearly unpopulated, so the log conveys what was sent without exposing the recipient's rendered personal data. The log is read-only about content: nothing on it edits, resends or deletes a message. It carries exactly one act, on a message in a terminal failure state (dead-lettered, or held for manual review), which no other part of the platform can move: an admin records that they looked at it and judged it settled, with a required note, and that review is audited. It asserts nothing about delivery, so the message keeps its status and the all-time figures above keep counting it; what it ends is the message's claim on the dashboard's urgent block, which otherwise has no resolution short of the row ageing out of retention. A message the sender may still deliver is not reviewable.
...
- On customer.subscription.updated (e.g., amount or status changes made in the Stripe Dashboard by a System Administrator): the system updates the local subscription record to reflect the new state and audit-logs the change. The cancel-at-period-end flag is mirrored in both directions, because an administrator can set or clear it at the provider as readily as the member can on their own page and neither direction moves the status or the amount: unmirrored, a cleared flag leaves the member's page saying the gift is ending while the provider goes on charging it, and a set flag leaves the eventual deletion reported to administrators as the provider exhausting its retries. A subscription whose collection the administrator paused is raised as an administrator work item: pausing collection leaves the reported status active while nothing is collected, which is distinct from the provider's own paused status (a trial ending with no payment method) and is the shape an administrator actually produces.
...
The system runs the job once per UTC day from the worker's daily loop, self-gated on the date of its last successful run, in four passes. The window spans the configured number of days back, or back to the last successful run when that is older, so an outage longer than the configured span does not leave a period no later run ever examines again.

Pass 1; One-time payments: Compares local payment records (membership dues, event registrations, one-time donations) against Stripe PaymentIntent records for the reconciliation window. Discrepancies flagged: local records with no matching Stripe PaymentIntent, Stripe PaymentIntents with no matching local record, amount or status mismatches. A local record carrying no PaymentIntent reference is a checkout still in flight and is left alone until the provider's checkout session has had time to expire; past that it is flagged, because a checkout nobody completed and a settlement whose webhook was lost are indistinguishable locally and only one of them is free to ignore. Where the provider reports a settled PaymentIntent whose local record is still pending, the two are matched on the correlation key the platform stamped on the intent and the status disagreement is flagged: the provider creates the intent when the buyer pays, so the reference reaches the local record only on the settlement event and its absence is not evidence the money never moved.
...
Pass 4; Refunds: Compares refunds the provider recorded in the window against the local payment records they belong to. This pass exists because a refund leaves no trace anywhere else in the provider's ledger: it never moves the PaymentIntent off succeeded, so to every other pass a refunded payment and an unrefunded one are the same record, and a refund whose webhook was lost would be invisible permanently. Refunded amounts are summed per PaymentIntent and compared against the charge rather than merely counted, because the payment record deliberately does not move for a partial refund; a discrepancy is flagged only where the provider has returned the whole charge and the local record still reads settled. Two partial refunds that together return the whole charge are therefore flagged, and a refund that did not succeed returned no money and is ignored.
