...
- Public hub at `/media` lists every deliberately-named gallery URL bookmark (FH-owned and member-owned), ordered FH first then alphabetically by name; each card shows the gallery name, description, item count, criteria-tag pills, and owner attribution. Auto-materialized per-member default galleries (Personal Gallery) are excluded from the hub list; they remain reachable at their `/media/{gallery_id}` URL for direct sharing.
...
- An admin gallery-management UX at `/admin/curator/galleries` lets the system administrator (acting as the FH system member) create, edit, and delete FH-owned named galleries: edit name, description, sort order, criteria tags, and exclude tags via `/admin/curator/galleries/:id/edit`; create a new gallery via `/admin/curator/galleries/new`; delete via the per-row Delete action. FH gallery writes (create, edit, delete) round-trip through the JSON sidecar at `/curated/galleries/<slug>.json` so the design state is captured in git as the source of truth, and the seeder loads the same files on each run. Admin moderation can also edit a member-owned gallery via the same admin URL.
- Members can manage their own named galleries at `/members/{memberKey}/galleries`, including create, edit, and delete. Member-owned galleries are public on `/media` alongside FH-owned bookmarks but are stored in the database only (no `/curated/` JSON sidecar); the named-gallery hero links the owner display name to the member's profile.
...
- **Post-verify onboarding:** After email verification, the member is routed to `M_Complete_Onboarding_Wizard` with applicable outstanding tasks. The wizard owns the club affiliation flow (Stages 1A, 1B, 2A, 2B, and the wrap-up landing). Both `first_competition_year` and `show_competitive_results` are collected as fields within the `personal_details` task. Registration itself does not block on these tasks; they are skippable and resumable from the dashboard task widget.
...
- Honors-bearing direct claims apply without admin pre-screening (no platform gate); the admin oversight feed (see `A_View_Honors_Oversight_Feed`) surfaces them post-facto for read-only audit visibility.
...
- The `club_affiliations` task content covers Stages 1A, 1B, 2A, 2B, and the wrap-up landing, plus a no-match exit that links to `M_Create_Club`.
...
- Stage 2A (pre-populated clubs nearby) and Stage 2B (onboarding-visible clubs nearby) match at the registrant's region level (state or province) rather than country. Within the region, same-city candidates render first.
- Each Stage 2 card shows the club's or candidate's name, location, description, and external URL, and asks one question, "Is {clubName} still active?", with four options: Still active / Not active anymore / Not sure / Never heard of it. "Not sure" is a safe escape recording no activity evidence; "Never heard of it" is stored but feeds no predicate gate.
- A Stage 2A card (live club) additionally renders an "I'd like to join" link routing to the club's join page on the club detail surface; following it records no wizard signal, because the wizard's role is cleanup, not duplicating the regular club-join flow.
- On a Stage 2B card, "Still active" and "Not active anymore" each confirm the candidate exists and promote it to a live `clubs` row; the activity signal is then recorded against the promoted club.
- The wizard does not offer in-wizard club search. After Stage 2 cards are exhausted, the `club_affiliations` task advances to the wrap-up landing. The wrap-up links to the clubs browse page filtered to the registrant's country (`/clubs/{countrySlug}`) where the member can browse clubs and use the regular join flow. The wizard itself never creates a new `clubs` row.
...
Story: As an admin, I can read a daily feed of every honors-bearing direct historical-record claim that confirmed in the prior 24 hours, so that the community has post-facto visibility into a category of claims the platform applies without admin pre-screening.

Success Criteria:

- The feed lists every direct historical-record claim where the linked record carries an honor flag (Hall of Fame, Big Add Posse) confirmed in the prior 24 hours.
- Each row shows the confirming member's display name, the linked historical record, the confirmation timestamp, and the evidence-strength tag.
- The feed is read-only. No gating action; the claim has already applied.
- Admin can navigate from a feed row to the member's profile and to the historical record to spot suspect claims.
- If admin (or community signal received out-of-band) indicates a claim is wrong, admin uses the dispute revert affordance from A_Review_Member_Link_Help_Requests to revert.
- The feed cadence runs from cutover through a configurable post-cutover monitoring window (default 56 days, extensible). The digest itself is delivered to the admin-alerts mailing list; the in-app feed view persists alongside.
