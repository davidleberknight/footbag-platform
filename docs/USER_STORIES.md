...
- The legacy_claim task must be completed — by claiming a record or choosing nothing to claim — before onboarding can finish, so no legacy-claim card lingers once onboarding is complete. Continuing without linking is the member's statement that they never held an old-site account, which resolves every card still open for them as declined in the same transaction that completes the task.
