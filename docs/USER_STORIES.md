...
- On the trick dictionary at `/freestyle/tricks`, the family filter and the by-family, by-ADD, by-movement-system, movement-neighborhoods, by-dex, and by-modifier views open the filtered dictionary, and the search box opens search results. Each hashtag, filter, and Trick Detail control is a real link the visitor can follow directly or deep-link to.
...
- Defer is a temporary skip within the wizard, not a way to postpone the required task: the member can set a card aside to resolve others first, but the legacy_claim task must be completed — by claiming a record or choosing nothing to claim — before onboarding can finish, so no legacy-claim card lingers once onboarding is complete.
...
- Within the wizard's claim task, which is the surface for declaring anchors (reached during onboarding and afterward from the profile's legacy-claim link, see M_Edit_Profile), the member can declare optional anchors: one or more former surnames and one or more old email addresses. The claim task also requires the member's date of birth: when none is on file the task collects it as a required field before any candidate can be confirmed, a search submitted, or the task resolved without linking, and when one is on file the task displays it read-only. It is stored as the member's date of birth, shown pre-filled in the personal-details task, and validated by the same rule as that task (a real calendar date, after 1900, not in the future). Anchors are self-asserted; no proof required for the declaration itself. Anchors are always private (member-and-admin only) per M_Edit_Profile.
...
- The round-trip is mandatory for confirming on an old-email anchor: an old email the member has not proven control of is not sufficient on its own to confirm a claim. A member whose old mailbox is unreachable falls back to another sufficient anchor (their verified login email or a former surname) or the admin help request (see A_Review_Member_Link_Help_Requests).
...
- Immediately after a successful claim of one source, the platform searches the other source for a plausible candidate (surname agreement using current or declared former surname as the required criterion; no other claimant; country is a corroborating signal, a country mismatch weighs against the candidate but never by itself blocks the match). If a candidate is found, the wizard surfaces an inline follow-up card within the same `legacy_claim` task: "we also found a record that might be you, is this you?" The member can confirm, defer, or decline. Deferring or declining does not block progression to the `club_affiliations` task; a deferred cross-source card must still be resolved before onboarding completes (defer is a temporary within-wizard skip), and no legacy-claim card persists on the dashboard after onboarding.
...
- Confirming a club affiliation creates a new `member_club_affiliations` row with `source='legacy_claim'`. A member may hold at most two current club affiliations (primary and secondary); the first confirmed club is primary, the second is secondary. When the registrant already has two `is_current=1` affiliations, the cap is hit: no `member_club_affiliations` row is written, the legacy affiliation row is NOT marked `'confirmed_current'` (it stays actionable), and the card surfaces a cap message ("You are at the two current-club limit; leave one of your current clubs to add this"), mirroring the affiliated-only branch used when the five-leader cap is hit. Transferring current-affiliation status requires the explicit `M_Join_Club` / `M_Leave_Club` surface or admin remediation via `A_Reassign_Club_Leader`.
...
- On the wrap-up landing, members with Tier 1 benefits, and Tier 0 members who have never held Active Player (whose first club creation grants it), see the "Create a new club" action directly. Only a Tier 0 member who has already used their one-time Active Player grant and is no longer active sees the create-club action replaced by a notice that creating a club requires Tier 1 benefits, with a pointer to the upgrade surface on their profile; after regaining benefits the wrap-up renders the create action. The notice does not block; the member can browse clubs or skip regardless.
- At the end of the `club_affiliations` task, if no `member_club_affiliations` row was written during the wizard run, the wizard's final screen displays a "Find or create your club" landing with three options: "Join an existing club" routes to the clubs browse surface and `M_Join_Club`, "Create a new club" routes to `M_Create_Club` (a Tier 0 member who has already used their one-time Active Player grant and is no longer active sees the tier-requirement notice above instead of the create action), and "Skip for now" completes the wizard with no club affiliation. The dashboard task widget surfaces "Continue onboarding" until the member explicitly dismisses or completes the task.
...
Access: Members who do not already co-lead a club can create a new club if they hold Tier 1 benefits, or if they are a Tier 0 member who has never held Active Player (for whom creating a first club grants the one-time Active Player period).
...
- Creating a club requires Tier 1 benefits, with one bootstrap exception: a Tier 0 member who has never held Active Player may create their first club, and creating it grants the one-time Active Player period (as a first club join does), giving them Tier 1 benefits and co-leadership. A Tier 0 member who has already used that one-time grant and is no longer an Active Player cannot create until they hold Tier 1 benefits again.
...
- If the creating member is Tier 0 and has never previously been an Active Player, creating their first club grants one 730-day Active Player period, co-committed in the club-creation transaction on the same terms as the first-club-join grant: it does not change membership tier, and creating further clubs grants no additional period. The grant is audit-logged.
