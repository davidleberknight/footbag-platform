...
- An admin gallery-edit UX at `/admin/curator/galleries` and `/admin/curator/galleries/:id/edit` lets the system administrator (acting as the FH system member) edit name, description, sort order, criteria tags, and exclude tags of any FH-owned named gallery. Gallery creation and deletion remain seeder-driven; the admin UX edits existing galleries only.
