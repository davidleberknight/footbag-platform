...
- The `personal_details` task precedes and gates the `legacy_claim` task: no candidate matching, search, direct historical-record claim, or continue-without-linking decision runs until the required personal details (including date of birth) are on file, so the matcher never runs before the disambiguation anchors exist.
...
- The optional `club_affiliations` task offers a `Skip for now` action that transitions its `member_onboarding_tasks` row to `skipped`. The required tasks complete instead of skipping: `personal_details` by saving its fields, and `legacy_claim` by an explicit decision (claiming a record, or choosing to continue without linking, which requires affirming the member never had an account on the old footbag.org). While a required task is outstanding, the member is routed to the wizard from the member, club, and admin capability surfaces; browse pages, the member's own profile, and the wizard's own affordances stay reachable. Each task transition emits an `audit_entries` row.
...
- On the wrap-up landing, members with Tier 1 benefits see the "Create a new club" action directly. A Tier 0 member without current Active Player status sees the create-club action replaced by a notice that creating a club requires Tier 1 benefits, with a pointer to the ways to earn Active Player (join an existing club, attend a qualifying event, or be vouched for) and to the upgrade surface on their profile; after gaining benefits the wrap-up renders the create action. The notice does not block; the member can browse and join clubs or skip regardless.
- At the end of the `club_affiliations` task, if no `member_club_affiliations` row was written during the wizard run, the wizard's final screen displays a "Find or create your club" landing with three options: "Join an existing club" routes to the clubs browse surface and `M_Join_Club`, "Create a new club" routes to `M_Create_Club` (a Tier 0 member without current Active Player status sees the tier-requirement notice above instead of the create action), and "Skip for now" completes the wizard with no club affiliation. The dashboard task widget surfaces "Continue onboarding" until the member explicitly dismisses or completes the task.
