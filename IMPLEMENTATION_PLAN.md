...
- **Net player-route redirect cleanup** (S): `/net/players/:personId` and `/net/players/:personId/partners/:teamId` (`src/routes/publicRoutes.ts:55-59`) exist only to 302 to `/history/:personId` and `/net/partnerships/:teamId`. Grep for consumers; if none, delete both handlers and remove VIEW_CATALOG §5 entries. Audit other routes that exist solely to redirect beyond the intentional `/history` → `/members` canonical redirect.
