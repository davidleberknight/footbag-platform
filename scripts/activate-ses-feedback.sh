...
#      key-masked diff, confirm, push back leaving no backup on the host.
...
#
# The feed authenticates on this key AND on its publishing topic, so the script
# refuses to install a key onto a host whose SES_FEEDBACK_TOPIC_ARN is unset:
# that pairing would refuse every delivery while looking merely quiet. That ARN
# is a non-secret operator-owned value and belongs to scripts/set-host-env.sh.
#
...
#
# Separate mode, returning before any key is generated or installed: this reads,
# it does not activate. The URL is a bearer token the application deliberately
# keeps out of the audit trail and surfaces on its log line, so retrieving it is
# a scripted step here rather than an operator typing `docker logs | grep` at a
# deployed host.
#
...
if (( SHOW_CONFIRMATION )); then
  require_operator_stdin "scripts/activate-ses-feedback.sh --target $TARGET --show-confirmation" || exit 1
  require_ssh_alias "$SSH_ALIAS" || exit 1

  LINE="$(host_log_tail "$SSH_ALIAS" 'ses_feedback.subscription_confirmation_pending')" || {
    echo "" >&2
    ...
    echo "  terraform -chdir=terraform/${TARGET} apply -replace='aws_sns_topic_subscription.ses_feedback_webhook[0]'" >&2
    exit 1
  }
  ...
  echo "  aws sns list-subscriptions-by-topic --topic-arn <ses feedback topic arn> \\"
...
#
# The ssh preflight belongs inside the ssh branch. Run before it, it makes the
# synthetic --env-file mode demand a configured deploy alias on a machine that
# never opens a connection, which is the opposite of what that mode is for.
# -----------------------------------------------------------------------------
OLD_LOCAL=""
NEW_LOCAL=""
OLD_LOCAL_OWNED=0
...
if [[ -n "$ENV_FILE_OVERRIDE" ]]; then
  ...
  require_operator_stdin "scripts/activate-ses-feedback.sh --target $TARGET" || exit 1
  require_ssh_alias "$SSH_ALIAS" || exit 1
...
if (( WRITE_TFVARS )) && [[ -z "$ENV_FILE_OVERRIDE" || -n "$TFVARS_OVERRIDE" ]]; then
  ...
  if TFVARS_PATH="$(resolve_tfvars_target "$TFVARS_LINK" "$REPO_ROOT")"; then
    if write_tfvars_url "$TFVARS_PATH" "ses_feedback_webhook_url" "$WEBHOOK_URL"; then
      TFVARS_WRITTEN=1
    else
      exit 1
    fi
...
echo "       terraform -chdir=terraform/${TARGET} apply -replace='aws_sns_topic_subscription.ses_feedback_webhook[0]'"
...
echo "         | grep ses_feedback.subscription_confirmation_pending | tail -1'"
