#!/usr/bin/env bash
# Synthetic-only identifier gate for committed test, content, and script
# surfaces. Real community or maintainer identifiers must never be committed
...
hits=$(grep -rnoE "[A-Za-z0-9._%+-]+@(${PROVIDERS})\.[A-Za-z.]{2,}" \
  tests/ src/content/ src/testkit/ src/views/ scripts/ 2>/dev/null \
  | grep -v 'scripts/\.venv/' \
