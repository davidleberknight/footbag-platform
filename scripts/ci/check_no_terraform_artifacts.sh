...
if [ -n "$violations" ]; then
  echo "$violations" >&2
  echo "FAIL: only *.tf, *.hcl, *.tfvars.example, *.tftpl, cloudfront-functions/*.js, and maintenance-page/*.html may be tracked under terraform/; state and plan artifacts embed secret values" >&2
  exit 1
fi
