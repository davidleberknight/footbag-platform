...
Parses SCALE10_PROSE_DRAFTS.md, generates apply CSV + pre-state audit + rollback SQL,
then applies UPDATEs to populate short_description / execution_summary / learning_notes /
prerequisite_notes on the 6 SCALE-10 slugs inside a single transaction.
...
DRAFTS = ROOT / "exploration" / "freestyle-notation-grammar" / "SCALE10_PROSE_DRAFTS.md"
APPLY_CSV = ROOT / "exploration" / "freestyle-notation-grammar" / "SCALE10_PROSE_APPLY.csv"
REPORTS = ROOT / "legacy_data" / "reports" / "scale10"
AUDIT_PRE = REPORTS / "scale10_load_audit_pre.csv"
ROLLBACK_SQL = REPORTS / "scale10_load_rollback.sql"

SLUGS = [
    "dada-curve", "eclipse", "jani-walker", "reverse-drifter", "ripstein", "terrage",
]
...
def write_rollback_sql(pre: dict[str, dict[str, str | None]]) -> None:
    ...
    lines = ["-- Rollback SCALE-10 standalone-with-records closeout (6 rows)", "BEGIN TRANSACTION;"]
    for slug in SLUGS:
        parts = []
        for field in FIELDS:
            v = pre[slug][field]
