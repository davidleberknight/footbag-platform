...
# by the SSM parameter itself: terraform/{env}/ssm.tf declares the shell with
# a placeholder and ignore_changes, and scripts/provision-ssm-secret.sh writes
# the real value out of band so it never enters Terraform state. This fetch
# keeps the host env in sync after a fresh `store`. Rotation invalidates
...
# denial naming an unfamiliar account id. Ask the container who it actually is. This
# path carries it for the same reason the code-only path does: every defect the first
# production bring-up found was a guard one deploy path had and the other did not.
_expected_role="footbag-${FOOTBAG_ENV_VAL}-app-runtime"
_identity=$(compose_cmd exec -T web node -e '
