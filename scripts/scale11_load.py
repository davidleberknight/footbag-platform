...
Parses SCALE11_PROSE_DRAFTS.md, generates apply CSV + pre-state audit + rollback SQL,
then applies UPDATEs to populate short_description / execution_summary / learning_notes /
prerequisite_notes on the 6 SCALE-11 slugs inside a single transaction.
...
DRAFTS = ROOT / "exploration" / "freestyle-notation-grammar" / "SCALE11_PROSE_DRAFTS.md"
APPLY_CSV = ROOT / "exploration" / "freestyle-notation-grammar" / "SCALE11_PROSE_APPLY.csv"
REPORTS = ROOT / "legacy_data" / "reports" / "scale11"
AUDIT_PRE = REPORTS / "scale11_load_audit_pre.csv"
ROLLBACK_SQL = REPORTS / "scale11_load_rollback.sql"

SLUGS = [
    "parkwalk", "bullwhip", "barrage", "rev-whirl", "double-around-the-world", "high-plains-drifter",
]
...
def write_rollback_sql(pre: dict[str, dict[str, str | None]]) -> None:
    ...
    lines = ["-- Rollback SCALE-11 mixed-composition Tier-1 (6 rows)", "BEGIN TRANSACTION;"]
    for slug in SLUGS:
        parts = []
        for field in FIELDS:
            v = pre[slug][field]
