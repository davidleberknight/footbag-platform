...
# whole src/dev-bootstrap/ subtree is removed at production cutover.
#
# Actions:
#   --seed-dev-admins   Seed maintainer admin accounts via
#                       src/dev-bootstrap/seed.ts. Reads from
...
# so relative paths to ./database/ and ./src/dev-bootstrap/ resolve.
cd "$(dirname "$0")/.."
...
action_seed_dev_admins() {
  ...
  FOOTBAG_ENV="${SEED_ENV}" npx tsx src/dev-bootstrap/seed.ts --db "${DB_FILE}"
}
