...
#
#   < ~/AWS/HOST_OPERATOR_PRODUCTION.txt bash scripts/outbound-mail-pause.sh --target production --status
#   < ~/AWS/HOST_OPERATOR_PRODUCTION.txt bash scripts/outbound-mail-pause.sh --target production --pause  --reason "wrong template went out"
#   < ~/AWS/HOST_OPERATOR_PRODUCTION.txt bash scripts/outbound-mail-pause.sh --target production --resume --reason "template corrected"
...
require_operator_stdin "scripts/outbound-mail-pause.sh --target ${TARGET} ${ACTION}" \
  "$ALIAS" "$TARGET" || exit 1
