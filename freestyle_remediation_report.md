...
Progress (2026-07-07): partially closed. Three curation stories are merged into `docs/USER_STORIES.md`: `A_Browse_Freestyle_Content` (the admin curation browse), `A_Edit_Freestyle_Trick` (the trick row plus its aliases, sources, and modifier links, with the validation and audit criteria), and `SYS_Freestyle_Content_Source_Of_Truth_Cutover` (the go-live source-of-truth switch). These ground the trick, alias, source, and modifier-link write paths, so the story gate is satisfied for those slices. Still open: the nine visitor dictionary stories, and stories for the remaining write paths the ratified scope names (records, consecutive-kicks records, tips, the modifier registry, relations, and the symbolic layers), which gate those slices.
...
**FS-36 — Quantum and Miraging: one set or two (doctrine decision)**
P-external | external-decision | owner: curator + Red | replaces plan item: line 420 | new-from-audit: no
Context: the platform currently treats Quantum and Miraging as distinct sets (verified in `symbolicSetEducation.ts` and content). Whether they are one set under modern naming or two is an open doctrine question for the curator and the rules expert. Until ruled, they stay distinct. Blocks parts of FS-28 and FS-32.
Done: a recorded ruling; set pages, dictionary rows, and teaching content updated to match; the ruling logged in `freestyle/doctrine/RED_RULINGS.md`.
...
- **Batch I — Post-V1 (parked until pulled).** Tasks: FS-13 (test de-epoch), FS-15 (service-size seams), FS-21, FS-26, FS-27, FS-28, FS-29, FS-30, FS-31, FS-32, FS-34, FS-35, and the vision directions. External blockers FS-23, FS-24, FS-33, FS-36 close as answers arrive, independent of batches.
...
**Still-open human inputs (tracked as tasks, not decisions for this report):** the Red Wave 3 doctrine answers (FS-23), the curator video viewings behind the same-side record names (FS-24), the Quantum-vs-Miraging ruling (FS-36), the IFPA Sick 3 rules wording (FS-33), the genuine-hyphen exception list (inside FS-06), the terraging-rider send confirmation (FS-18), and the demo poster/MP4 assets (FS-22).
...
**Verdict, updated.** No P0. FS-01 and FS-02 are closed. FS-16 is partially closed: the three curation stories (admin browse, trick edit, source-of-truth cutover) are merged, grounding the trick, alias, source, and modifier-link write paths; the nine visitor dictionary stories and the stories for the remaining write paths (records, consecutive-kicks records, tips, modifier registry, relations, symbolic layers) are still open. Remaining P1: FS-03 (test nets), FS-04 (reduced to the slug-convention and stale-mechanism components), FS-16 (the remainder above), FS-19 (in-app curation cutover, in progress — the `freestyle_tricks` scalar-row edit slice, all three attached-relationship write paths (alias add/remove, source-link attach/detach, and modifier-link attach/detach), and the pre-go-live persona guard on every write path are shipped, each with field-shape validation, scoping, and one audit entry per operation; the remaining in-app curation work is the record and tip write paths, now grounded by three drafted stories: freestyle world-record editing and consecutive-kicks editing are launch-signoff work (the consecutive-kicks path gated by a small stable-id-and-timestamps schema slice), trick-tip moderation is post-launch acceptable, and the symbolic and observational tables stay deferred until their authoring and validation model is designed; plus a data-preserving deploy, written rollback guidance, and the documented cutover procedure with its tests, see the FS-19 progress note). Freestyle is ready for external testing; launch signoff still gates on FS-16, FS-19, and FS-03. This supersedes section 17's "no remediation code has been written," which was true only of the original audit session.
