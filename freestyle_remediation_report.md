...
4. **`exploration/` is fenced from live dependencies, not retired.** It stays as the home for research projects (unfinished analysis, experimental registries, exploratory notes); the only launch requirement is that live runtime, public build, and committed-data regeneration never depend on it. That fence is verified in place (see FS-25): the one apparent live tie was dead code in the producing symbolic-grammar generator, now removed.
...
- **DB / source-of-truth: proven clean at runtime, unguarded at build time.** Runtime provably reads only SQLite and compiled content modules; no code in `src/` reads pipeline CSVs or writes `freestyle_*` tables. But no loader has any production guard (FS-02, now closed), two generated content files have no drift guard (FS-10; neither is an orphan, the tracked-names module is test-consumed), and the symbolic-grammar CSVs were thought unregenerable once `exploration/` is fenced off (FS-25 finding: that dependency was dead code, now removed, so the committed CSVs regenerate from in-pipeline inputs alone).
...
- FS-32 — authorable Technique Notes. **M. CLOSED.** Authored execution/learning/prerequisite notes for `quantum_legover`, `paradox_whirling_swirl`, `quantum_guay` (full) and `atomic_reverse_guay` (partial: no learning note, the unsourced reversed-side feel omitted). Applied via `red_corrections` + rebuild; all four pages verified rendering.
- (FS-43 Zulu / Weaving notation: on inspection NOT a clean derivation, moved to curator-decision below.)
- FS-28 Set Encyclopedia pages: **CLOSED for V1.** The four canonical set pages (Atomic, Quantum, Furious, Nuclear) are authored, committed, and verified rendering. Miraging is NOT a canonical set (a historical nickname, like Barraging), so there is no Miraging set page; the only remainder is an optional nickname note, tracked in the curator/nickname group below.

### 3. Medium work
- FS-20 — remaining media-gallery links. **M. CLOSED.** The originally-named targets already surfaced media through their own idioms (the `?view=family` browse renders the `dictionary-trick-row` hashtag gallery link; the landing's twelve-foundation mosaic links each atom to its clip; the media hub lists collection galleries), so no redundant per-trick links were added there. The genuine gaps were the text-only family-detail and set-detail pages: each now renders a per-member (and per-example-trick) `/media/browse?context=<slug>` gallery link where the trick has curated media, plus a single family/set-level "watch a clip" representative-media link to the strongest-covered member (a tutorial over a demo). Pure projection over the same per-trick coverage map the browse rows use; no schema, no new query beyond reusing the index shaping context. Route tests pin the covered/uncovered rendering on both pages.
- FS-07 — controller 404 / branching cleanup. **M. CLOSED.** The two redundant `NotFoundError` catch branches now delegate to `handleControllerError`; the two null-return 404s use a local `renderNotFound` helper (keeping the documented null-return contract); `tricksIndex`'s `emerging` redirect and `sort` derivation moved into the service. Behavior-preserving; full freestyle suite green.
- FS-10 — generated-content drift guards. **M. CLOSED.** Regeneration-is-a-no-op guard (`scripts/ci/assert_generated_content_current.sh`) added for both generated modules and wired into the database-load CI job. The `freestyleTrackedNames.ts` module is NOT an orphan (four tests consume it) and stays; both modules brought current. Generators honor `FOOTBAG_DB_PATH`.
...
- Miraging nickname note. **XS. CLOSED.** Added a "Quantum vs Miraging" disambiguation to the Quantum teaching page (`symbolicSetEducation.ts`), mirroring the atomic page's "Atomic vs illusioning" note: Miraging is a historical mirage-family nickname, not a canonical set and not an equivalent term for Quantum, which stays the official set identity (its own historical name is Toe Blur). No standalone Miraging set page. The Quantum/Miraging relationship followed the Illusioning/Atomic pattern (nickname by analogy, not an equivalent), not the Furious/Barraging identity model, because the operator reference is explicit that Miraging is not a Quantum equivalent. The doctrine-guard test that previously forbade any Miraging mention on the Quantum page was aligned to the atomic precedent (no Miraging set-link, plus the disambiguation required).
...
- FS-25 — fence `exploration/` from live dependencies. **CLOSED.** Ruling: `exploration/` stays as the research home (it may hold unfinished analysis, experimental registries, exploratory notes); the only launch requirement is that live runtime, public build, and committed-data regeneration never depend on it. Verified: no `src/` code reads `exploration/`; the freestyle rebuild loads the symbolic-grammar layer from committed CSVs via loader 26 and never reads `exploration/`; the generator that produces those committed CSVs now reads only in-pipeline inputs after three dead `exploration/` path constants (defined, never read) were removed. The two curated registries stay in `exploration/` as research artifacts. Not a launch blocker; no corpus retirement.
...
- FS-14 — rename `inputs/noise/` + extend rule globs. **S. CLOSED.** Renamed `freestyle/inputs/noise/` to `freestyle/inputs/base_dictionary/` and updated every freestyle-scoped code/test path reference (loader 17, both generators, the CI row-count checker, four tests); loader 17 verified. The stale doc references in `freestyle/doctrine/*.md` and the skill REFERENCE.md are updated. The comment-hygiene and db-write-safety rule globs now reach `freestyle/loaders/**`, `freestyle/scripts/**`, and `freestyle/tools/**` (maintainer applied the `.claude/rules/*` edits the self-modification gate had blocked).
- FS-17 — records-correction ergonomics. **M. CLOSED.** Loader 10 now warns loudly on edited-but-skipped record rows (compares incoming vs stored on an INSERT OR IGNORE skip), preserving the additive contract; a genuine duplicate does not warn. New pytest covers it; the correction procedure is stated in the warning. No CLAUDE.md edit needed.
...
| Path/catch (terminal-mechanic) encoding standard | Notation / Red, then internal parser | ~76 of the ~103 band (31 core-atom + 45 resolved non-core; the other ~17 are identity and ~10 are incomplete operator-ending) | parser work + reclassification | Drafted as a framing section in the Notation paper; a notation rule plus parser implementation and named-terminal decomposition, not a single yes/no |
...
- **The plan / IP**: `IMPLEMENTATION_PLAN.md`, the repository's operational to-do document. Line references (line 94, etc.) refer to its state on 2026-07-07, before the consolidation edit this report directed. (Superseded by the 2026-07-10 reconciliation: live unfinished freestyle work now appears in `IMPLEMENTATION_PLAN.md`; this report is retained for evidence and history only. The earlier "the plan carries no freestyle to-do items and this report is the single home for freestyle work" no longer holds.)
...
Status: CLOSED. `RESOLVED_FORMULAS_SPRINT_1` renamed to `RESOLVED_ADD_FORMULAS` across all call sites (the export plus its two content-module consumers and the seven service uses/comments). The observational header phase label is removed at its source in the generator, and the single header line in the generated file is brought into line with what the generator now emits; the file's stale data rows are left untouched (a separate generated-content-drift concern outside this rename). The caller reference in `freestyleLandingContent.ts` is reworded to a self-contained statement. `S3/S5` in `freestyleSymbolicEquivalences.ts` is domain set-slot notation, not an epoch label, and was left alone.
FS-08 follow-up regression repair: the initial rename updated only `src/`, so it missed about twenty freestyle test files (unit and integration) and two `legacy_data/scripts/*.py` pipeline scripts that import or name the old export. The undefined import crashed those tests, and the incomplete verification (only three test files were run at the time) did not catch it. The rename was completed across all 24 remaining consumers; the full freestyle suite (2122 tests across 140 files), the tsc build, and the two scripts' parse checks now pass. The old symbol survives only in Markdown that documents the rename or in dated research/archive snapshots.
...
**FS-10 (P2, test + data; audience: maintainer). Generated content files have no drift guard.**
Evidence: `src/content/freestyleObservationalUniverse.ts` is guarded only by an internal-consistency test (`tests/unit/observational-universe-consistency.test.ts` checks totals against itself, never reads the source inputs), so a stale regeneration passes. `src/content/freestyleTrackedNames.ts` is generated and committed; no `src/` code imports it, but four integration tests do (its `TRACKED_UNPUBLISHED_NAMES` / `TRACKED_UNPUBLISHED_TOTAL` exports), using it as promotion-regression coverage (a promoted name must leave the tracked-unpublished list), so it is a test-consumed data module, not an orphan. CI never runs the generators. The single-CSV parity pattern in `tests/unit/freestyleEmbeddedCoverage.test.ts` (re-parse one source CSV, assert row-for-row equality) fits a 1:1 CSV mirror but NOT the observational module, whose generator opens the DB read-only and reads several CSVs then classifies each row with logic; that one needs a regeneration-is-a-no-op guard, not CSV parity. The observational module is also currently stale: a fresh regeneration rewrites about 17 rows.
Fix (corrected): keep `freestyleTrackedNames.ts` as a test-consumed generated data module (do not delete it or its generator). Give BOTH generated modules a regeneration-is-a-no-op drift guard, run in the tier that already builds the DB, not a unit CSV-parity test. Regenerate the stale observational module first (a public-facing content change, about 17 rows, reviewed before commit).
...
Status: CLOSED. Loader 10 now detects edited-but-skipped rows: when INSERT OR IGNORE skips a row whose id already exists, it compares the incoming content against the stored row, and if they differ it counts the row as `Skipped edited` and prints a loud, delimited warning that names the affected record ids and states that the additive loader preserved the stored rows, so record corrections take effect only on a fresh rebuild. A genuine re-run duplicate does not trigger the warning. The correction procedure is stated in the warning itself (rebuild the freestyle records to apply edits); the additive-loader behavior is already documented in `freestyle/CLAUDE.md`, so no gated CLAUDE.md edit was needed. Covered by a new pytest (`test_freestyle_records_loader_warns_on_edited_but_skipped_row`): the edit is not applied, the stored value is unchanged, the loud warning fires and names the id, and an unchanged re-run does not false-positive. The full loader-idempotency suite (22 tests) passes.
...
Status: CLOSED. Part (a): directory renamed to `freestyle/inputs/base_dictionary/`; loader 17, both content generators, the CI row-count checker, and four tests updated; loader 17 verified. Part (b): the stale path references in `freestyle/doctrine/*.md` and the skill REFERENCE.md are updated, and the `comments.md` + `db-write-safety.md` `paths:` lists now include `freestyle/loaders/**`, `freestyle/scripts/**`, and `freestyle/tools/**` (the maintainer applied these `.claude/rules/*` edits, which the harness self-modification gate had blocked). Loader-pipeline directories are now under both enforced rules.
...
**Doctrine intake model:** the open OPERATIONAL doctrine questions live in `freestyle/doctrine/RED_QUEUE.md` (the older "Wave 3" operational packet, whose delivery to Red is unconfirmed in the repository: blurry-expansion predicate gating 64 rows, operator structural definitions, cross-body rake, terraging chain, rider list), with rulings integrated into `RED_RULINGS.md` and topic docs, and curator-owned questions listed separately (POD vs Dimmier, Kiwi, Clipper Symposium Whirl, atomic-pickup alias target). This is distinct from the five FORMAL doctrine papers, whose send-state is: Identity sent and answered, Frontier sent and awaiting an answer, and Notation, Scoring, and History drafted and not yet sent. The remaining operational questions fold into the unsent Notation, Scoring, and History papers (blurry-expansion / operator-definitions / terraging-arithmetic into Scoring; cross-body rake into Notation; down-family embedded-base into Identity's open half) rather than new ad hoc asks. This is a healthy, delete-on-answer queue; the earlier "confirm the terraging rider email was sent" caveat is subsumed by the queue's blanket unconfirmed-delivery posture for the operational packet.
...
- **Ratified disposition (decision 4, revised):** `exploration/` is fenced from live dependencies, not retired. It stays as the research home; the launch requirement is only that live runtime, public build, and committed-data regeneration never depend on it. The one apparent live tie, three dead `exploration/` path constants in the producing symbolic-grammar generator, was removed and the boundary is documented in `freestyle/run_freestyle.sh` and `freestyle/README.md`. Research tooling may still read and write inside `exploration/`; curated registries nothing live reads stay in place as research artifacts, promoted into `freestyle/inputs/` later only if a live consumer needs them. See FS-25.
...
| 203 | "Trick-promotion decisions (Red Wave 3, sent; curator)." | BLOCKED | Reworded: the operational "Wave 3" packet's delivery to Red is unconfirmed, not "sent". The five formal doctrine papers are Identity (sent + answered), Frontier (sent + awaiting), and Notation/Scoring/History (unsent). | **Keep** (the earlier FS-18 "confirm the terraging rider email was sent" caveat is subsumed by the queue's blanket unconfirmed-delivery posture) |
...
| 226 | "Decide the go-live disposition of the exploration/ corpus." | KANBAN | Accurate; 1,072 tracked files | **Absorbed** into FS-25 (decided: FENCE from live dependencies, not retire; `exploration/` stays as the research home) |
...
| 288 | Strip embedded design-doc references | post-V1 backlog | **STALE**: parts (1)/(2) already done (zero design-doc refs remain in freestyle tests/content; the "18 in freestyle.routes.test.ts" are glossary page anchors); part (3) (three exploration files cite the deleted legacy plan) is moot under the fence ruling: research files may keep stale exploratory references | **Absorbed**: the test-side residue into FS-13; the three exploration files stay as research artifacts (FS-25 fence, no action) |
...
CLOSED (2026-07-09). The trick-detail route 301-redirects a pure alias slug to its canonical trick URL: `freestyleController.ts` calls `trickRouteRedirectTarget`, which resolves the slug against the alias table and returns the canonical path, so one indexable URL renders per trick. `tests/integration/freestyle.trick-alias-redirect.routes.test.ts` pins the contract: a pure alias 301s to canonical, the canonical slug renders 200 with no redirect, a canonical row wins when an alias shares its slug, a modifier slug still 301s to its modifier page, and an unknown slug 404s (five assertions, all green this run). This report's later launch-signoff verdict already recorded FS-01 closed; this marker reconciles the Kanban entry with it.
...
CLOSED (2026-07-09). Standalone guard `freestyle/_assert_dev_db.sh` refuses the rebuild unless FOOTBAG_ENV is unset or exactly "development" AND the canonicalized target is one of this checkout's own disposable databases (`database/footbag.db`, or `database/footbag-ci.db` for the CI loader-smoke gate); `run_freestyle.sh` calls it before any loader, paths are realpath-canonicalized so a symlink cannot slip a live database past it, and there is no in-band bypass flag. Pytest `legacy_data/tests/test_freestyle_rebuild_guard.py` pins allow-dev-path, allow-CI-path, refuse-non-development-environment, refuse-alternate-path, and refuse-alternate-path-with-a-missing-environment (all green this run). This report's later launch-signoff verdict already recorded FS-02 closed; this marker reconciles the Kanban entry with it.
...
CLOSED (2026-07-09). Swept as visitor-facing copy only, across a set of small approved commits, one per template or small group. The glossary carried the bulk (134 rendered entities across its ten chapters, swept chapter by chapter) and now renders none; the other freestyle templates (about, combo-analysis, competition, add-analysis, compositional-sets, leaders, moves, tricks, sets-encyclopedia, and the trick-equivalence-topology partial) had their prose em dashes replaced with commas, colons, semicolons, or parentheses, each chosen to preserve meaning. Two non-prose cases were handled deliberately: the set-notation reference "Next" pointer keeps its meaning with a colon and comma, and the trick-detail footer link separator became a middle dot. The table empty-cell placeholder glyphs (records, competition, leaders, trick-records, trick-structural, and the ADD-analysis empty-formula cell, fifteen in all) are retained by curator decision as the conventional "no value here" marker, since the ban targets run-on prose, not empty-cell indicators. The original evidence's ~154 service-string and the content-module figures were literal em dashes in code comments, JSDoc, and one provenance metadata field, not rendered copy; the visitor-facing strings in `freestyleService.ts` and `src/content` were already clean, and the conventions gate enforces that going forward. The member-profile "More Features — Coming soon" heading renders outside the freestyle visitor surface and is untouched. Full freestyle route suite and the conventions gate are green.
...
Status: CLOSED. The two catch-block branches that duplicated `handleControllerError`'s NotFoundError mapping (trick, modifierFamily) now just delegate; the `NotFoundError` import is dropped. The two detail routes that render 404 on a documented null return (setDetail, familyDetail) call a local `renderNotFound(res)` helper, matching the per-controller pattern in memberOnboarding/payment and keeping the deliberate null-return contract the service file-header documents. `tricksIndex` is thinned: the `emerging` redirect decision moved to a new `freestyleService.tricksIndexRedirectTarget(view)` (mirroring the existing `*RouteRedirectTarget` helpers), and the `addSort` derivation moved into `getFreestyleTricksIndexPage`, whose third parameter is now the raw `sort?` query. Behavior is fully preserved: tsc build clean, the full freestyle integration suite (2117 tests across 138 files) and the dedicated emerging-redirect and alpha-sort route tests pass, and the conventions gate passes.
...
CLOSED (2026-07-09). Landed as visitor-facing copy only, across a set of small approved commits. Edited: the minor-lineages band and the search empty-state onward links; the three "Platform-tracked" labels replaced with surface-specific wording (the operator/modifier status pill reads "Listed, no guide yet", the set-source provenance badge reads "Tracked here", the Set Encyclopedia beginner pill reads "Listed here"), keeping every data key and CSS class; the insights hero subtitle rewritten to orient before naming the Sick 3 archive and to gloss it; the first rendered IFPA reference glossed as International Footbag Players Association; the partnerships table header expanded from "Apps" to "Appearances"; a visible source-key legend added to Emerging Vocabulary for the PB / FM / SG badges; a two-sentence orientation intro added to the notation article; and plain-language hero intros for the Set Encyclopedia and Compositional Sets. Route-test assertions were added for the new copy where a natural test home existed. Deliberately left unchanged as already clear, rather than churned: the leaders intro (already a plain one-liner), the Emerging Vocabulary title and hero (already split into short oriented sentences, with the badge gloss the only remaining gap, now filled by the legend), the "lenses" notes, and the family-view intro. Affected route tests, tsc, and the conventions gate are green across the commits.

**FS-10 — Drift guards for both generated content modules**
P2 | test + data | audience: maintainer | status: CLOSED | new: yes | replaces: none
Evidence/remediation: see the finding above. Do NOT retire `freestyleTrackedNames.ts` (four integration tests consume it as promotion-regression coverage; the earlier retire ruling rested on a `src/`-only import check). Both generated modules (`freestyleObservationalUniverse.ts`, `freestyleTrackedNames.ts`) get a regeneration-is-a-no-op guard in the tier that already builds the DB; the single-CSV parity pattern does not fit the DB-plus-multi-CSV observational generator. The stale observational module is regenerated first, with its roughly 17-row public delta reviewed before commit.
Progress: both generators now honor `FOOTBAG_DB_PATH` (fallback to the local dev database) so a fresh regeneration can run against the CI-built database; the observational module is committed current (three-row delta) and the tracked-names module is brought current (one row: the promoted `carousel` drops out); `scripts/ci/assert_generated_content_current.sh` runs both generators and fails on any diff, verified to catch the stale tracked-names module and pass on the current observational module. The guard is wired into the database-load CI job as a new step after the row-count assertion, pointing the generators at the CI database via `FOOTBAG_DB_PATH`.
Done: a stale regeneration fails the check; both generated modules are guarded; the tracked-names module and its generator are retained.
...
P3 | data + harness | audience: maintainer (approval: `.claude/` edit for the glob part) | status: CLOSED | new: yes | replaces: none
...
P1 (raised from P3 by the launch-scope decision) | docs-stories | owner: James drafts, Dave approves | status: launch-gating stories DONE, not a launch blocker; only the deferred-slice stories (the modifier registry, relations, and the symbolic layers) remain, and they gate only those deferred slices, never launch | replaces IP item: line 198 | new-from-audit: no (expanded)
...
**FS-17 — Records-correction ergonomics** — P3, section 6; edited-but-skipped detection or a scoped rebuild mode plus documented procedure; done when a curator correcting a record row gets either the change or a loud warning. CLOSED: loader 10 detects edited-but-skipped rows and prints a loud warning naming the affected ids and the rebuild procedure; new pytest covers it; the full loader-idempotency suite passes.
...
P1 | code + curator-workflow + production-safety | owner: Dave (platform) with James (content model) | status: LAUNCH BLOCKER — the only true launch blocker remaining; code-complete (admin edit surfaces, write paths with audit entries, pre-go-live persona guard, source-of-truth cutover tests, and the destructive-rebuild post-cutover refusal guard are all shipped and tested), so the sole remaining piece is the operator-run staging rehearsal: a code-only deploy that leaves live admin edits intact, plus a destructive-rebuild refusal demonstration, both run on staging by the authorized deploy operator | replaces plan item: line 417 ("There is no in-app way to edit the trick dictionary") | new-from-audit: no (was deferred; scope decision is new)
...
P2 | UX-copy/code | owner: Dave | status: CLOSED | replaces plan item: line 94 | new-from-audit: no
...
CLOSED. The media-gallery connectivity workstream for the family and set surfaces is engineering-complete. On investigation the originally-named targets already surfaced media through their own idioms and were intentionally not redundantly changed: the by-family browse (`?view=family`) renders the `dictionary-trick-row` hashtag gallery link, the landing's twelve-foundation mosaic links each atom to its clip, and the media hub lists collection galleries. The genuine gaps were the text-only family-detail and set-detail pages: each now renders a per-member (and per-example-trick) `/media/browse?context=<slug>` gallery link where the trick has curated media, plus a single family/set-level "watch a clip" representative-media link to the strongest-covered member. Pure projection over the same per-trick coverage map the browse rows use, no schema and no new query. A route test pins the covered and uncovered rendering on both pages. Future tagged clips flow into these pages automatically. The remaining media work (the 72 BAP clip tagging pass, learn-by-watching embeds, local MP4 ingestion, and broader curation) stays post-launch as the Freestyle 1.3 media roadmap.
...
CLOSED (2026-07-09). Seventeen poster frames were extracted from the committed `.mov` sources with ffmpeg (one representative frame per clip via the thumbnail filter) and committed as `curated/freestyle_demos/*.poster.jpg`, matching the committed-poster asset convention (`curated/site/mosaic-*.poster.jpg`). The "MP4 source" half of the original done-condition was a misread of the pipeline: the seeder transcodes each `.mov` to MP4 itself at seed time, and `.mov` is already an accepted video extension, so no MP4 is committed. With the posters present, `seed_fh_curator.py` ingested all 17 demos into `media_items` (0 to 17 `#footbag_org` rows; total active media 284 to 301); a second run was idempotent; the media-tag invariant QC passed on all 301 active items. Captions are platform-authored and name-free, per the governance ruling that the no-individual-names rule covers only platform-authored text and not the burned-in contributor/copyright watermark inside the twelve Aleš Zelinka source clips, which is source footage left untouched (no cropping, no re-encoding). Verified on the local dev database and media store only, with no production or staging write: the flurry demo renders on its trick page (the pathway card reads one tutorial and one demonstration available) and its poster loads over HTTP on the `/media/browse?context=flurry` gallery.
...
Send-state correction (2026-07-10): the "Wave 3 letter" here is the older OPERATIONAL question packet, whose delivery to Red is unconfirmed. It is distinct from the five FORMAL doctrine papers, whose send-state is now: Identity sent and answered, Frontier sent and awaiting an answer, and Notation, Scoring, and History drafted and not yet sent. The remaining operational questions fold into the unsent Notation, Scoring, and History papers, not new ad hoc asks; questions already answered by the Identity paper are integrated and removed from `RED_QUEUE.md`.
...
**FS-25 — Fence the exploration corpus from live dependencies (keep it as the research home)**
P3 | code + docs | owner: James | status: CLOSED | replaces plan items: line 226, plus the surviving part of line 288 | new-from-audit: no (ruling is new)
Ruling: `exploration/` remains the home for research projects and may hold unfinished analysis, experimental registries, and exploratory notes; it is NOT retired. The only launch requirement is that live runtime, public build, and committed-data regeneration never depend on it. Research tooling may still read and write inside `exploration/`, and curated registries that nothing live consumes may stay there as research artifacts unless a live consumer later needs them (promote into `freestyle/inputs/` then).
Finding: the fence already held. No `src/` code reads `exploration/`. The rebuild loads the symbolic-grammar layer from committed CSVs under `freestyle/symbolic_grammar/` via loader 26 and never reads `exploration/`. The generator that produces those committed CSVs, `legacy_data/scripts/build_symbolic_grammar_2.py`, read only `freestyle/inputs/observational/SYMBOLIC_GRAMMAR_MASTER.csv` plus the DB; its three `exploration/` path constants were dead (each defined once, never read) and falsely implied a live dependency. The separate analysis generator, `build_symbolic_grammar_master.py`, is research tooling that reads the legacy output tree and writes into `exploration/`; nothing live consumes its outputs. The two curated registries in `exploration/freestyle-notation-grammar/` that nothing live reads stay in place.
Done: the three dead `exploration/` constants removed from the producing generator (verified it still parses and reads only in-pipeline inputs); the boundary documented in `freestyle/run_freestyle.sh` and `freestyle/README.md` (the build never reads `exploration/`; the committed CSVs regenerate from in-pipeline inputs; `exploration/` is research-only and never a required build input). No corpus deletion; not a launch blocker.
...
**FS-28 — Set Encyclopedia teaching pages for the four canonical sets**
...
Status: CLOSED for V1. The four canonical set teaching pages (Atomic, Quantum, Furious, Nuclear) are authored, committed in `src/services/symbolicSetEducation.ts`, and verified rendering on their set-detail routes, each grounded in the operator reference and canonical-sets doctrine. Miraging gets NO standalone Set Encyclopedia page: under settled doctrine Miraging is a historical/nickname naming pattern by analogy to downtime Mirage (already classified `historical-nickname` in the operator reference, "not a canonical set"), while Quantum is the official uptime set identity, mirroring the Furious/Barraging model (official set first, historical nickname second). The Miraging terminology/nickname note is authored as a "Quantum vs Miraging" disambiguation on the Quantum teaching page (mirroring the atomic page's "Atomic vs illusioning" note): Miraging is a historical mirage-family nickname, not a canonical set and not an equivalent term for Quantum. No standalone Miraging set page. Do not author a standalone Miraging set page unless Miraging is explicitly reclassified as a canonical set.
...
Status (2026-07-09): none of the 8 is resolvable by a data edit. A tip's status is computed at rebuild time from whether a matching canonical trick or alias exists, so resolving one means authoring a new trick or alias (a curator decision), not editing the tip. Each is now marked with its exact missing input: (1,2) Pixie Same Side Butterfly (two tips) and (7,8) Clipper Set Illusion (two tips) are clear, well-described, not-yet-authored compounds that need a curator decision to author a canonical trick and slug; (6) Barraging Paradox Mirage is a plausible missing sibling of the paradox-mirage family but needs curator confirmation that the described technique is the paradox variant and not duplicate advice for the already-published Barraging Mirage; (4) Stepping Paradox Symposium Whirl and (5) Stepping Down Double Down are genuinely ambiguous and need a curator ruling on whether each is an incomplete legacy name for an existing canonical trick or a distinct unauthored compound; (3) Pogo Op Whirling Swirl is an existing tracked doctrine deferral, blocked on reactivating the pogo operator. None is a launch blocker; the 2 Net tips stay parked until a Net home exists.

**FS-32 — Technique Notes: three authored in full, one partial**
...
Status (2026-07-09): re-scoped and shrunk. Three of the four are authorable now without fabrication, by composing the base trick's already-sourced technique notes with the documented operator-reference mechanic and a literal reading of the curator-verified notation (the same method already used to author guay's own notes): `quantum_legover`, `paradox_whirling_swirl` (which additionally has its own footbag.org description), and `quantum_guay`. `atomic_reverse_guay` is partial: its execution and prerequisite notes are authorable the same way, but a learning note about how the reversed side feels has no source and is left out rather than invented. The edit path is `execution_summary` / `learning_notes` / `prerequisite_notes` correction rows in `freestyle/inputs/curated/tricks/red_corrections_2026_04_20.csv`, applied by a rebuild. Not a launch blocker.
Status: CLOSED. Eleven correction rows authored and applied: `quantum_legover`, `paradox_whirling_swirl`, and `quantum_guay` each have execution, learning, and prerequisite notes; `atomic_reverse_guay` has execution and prerequisite notes only, with the unsourced reversed-side learning note intentionally omitted (leaving the field empty is the correct source discipline). Each note was composed from the base trick's already-sourced notes, the operator-reference mechanic, and a literal reading of the curator-verified notation, with no fabrication. Verified after a rebuild: all four pages render the notes (HTTP 200), `atomic_reverse_guay` renders with no Learning Notes heading, the generated content modules are unchanged (drift guard is a no-op), and the freestyle trick-detail and content-link tests pass.
...
RESOLVED (2026-07-09, doctrine). The one-set-or-two question is settled: the maintainer working ruling keeps Quantum and Miraging distinct (Quantum the uptime set, Miraging the downtime or descriptive mirage-family reading), no dictionary rows merge, and the data already matches. No open doctrine question remains, so this is no longer a doctrine blocker. Any residual work is a short terminology/nickname note (Quantum is the official set identity; Miraging is a historical nickname), not a Miraging set page; it is not tracked here as an unresolved doctrine item and does not gate launch.
...
RESOLVED (2026-07-09, V1 ruling). The V1 decision is that public Butterfly is far/opposite-side by default (the base atom already renders that way), and the eleven butterfly-family compound notations are NOT migrated family-wide for launch: they stay side-either, and any resulting stored-notation-versus-public-atom mismatch is already carried as the accepted, tracked deviation FS-37. No open doctrine question and no launch block. A future family-wide compound migration, if ever undertaken, is non-blocking data work, not a doctrine gate.
...
RESOLVED (2026-07-09, V1 ruling). For V1 the model stands as-is: Far Butterfly stays retired into Butterfly (its row 301-redirects to Butterfly) and Infinity stays a display-suppressed alias of Butterfly; neither is promoted to a distinct clipper-set row or a new subform, and Infinity does not reopen the Butterfly model for launch. No open doctrine question and no launch block. Any residual copy, search, or sidebar cleanup around the Infinity naming is tracked as non-blocking polish only, not as an unresolved doctrine item.
...
Context: the doctrine is already settled and is NOT reopened — Furious is the canonical set; Barraging is not a canonical set. `src/content/freestyleCanonicalSets.ts` reflects this (its set-route map redirects `/freestyle/sets/barraging` to a glossary "not a set" note). The remaining work is a stale-reference cleanup: `src/content/freestyleOperatorReference.ts` still carries a `barraging` entry framed as a settled compound-set (`decomposition: 'high stepping'`, curator-confirm not pending), which contradicts that doctrine. The maintainer ruling (2026-07-09) is that the canonical-set doctrine wins, so any operator or reference surface still describing Barraging as a settled compound-set is corrected to match, not treated as an open question. The `barraging_*` trick rows may remain for name continuity, but they are legacy naming residue, not evidence that Barraging is a set.
Done: the operator reference (and any other surface) no longer describes Barraging as a canonical set, matching the canonical-sets doctrine that Furious is the set and Barraging is not.
CLOSED (2026-07-09). Corrected as a general official-set-vs-historical-nickname model rather than a one-off. The official uptime sets are Atomic, Nuclear, and Furious; the historical nickname patterns (Illusioning X, Miraging X, Barraging Y) are the downtime-analogy names players used, which persist for name continuity but do not override the official set identity, and decomposition resolves to the official set. Applied: in `freestyleOperatorReference.ts` the `barraging` entry is reframed as "a legacy name pattern for the Furious set, not a canonical set" with worked examples reading Furious Osis / Furious Paradox Mirage; the `atomic` and `miraging` entries are aligned to the same wording (Atomic and Quantum are the official sets, Illusioning and Miraging the historical downtime nicknames); the `nuclear` entry needed no change (its "illusion" is a decomposition component, not a nickname override). In `freestyleCanonicalSets.ts` the furious movement explanation and source citation drop the superseded "distinct by timing, pending audit" framing and state Barraging is a legacy name pattern for the Furious set. The `barraging` slug is kept as a compatibility route (no rename this pass); the coupled glossary test was updated to the new doctrine; the full freestyle suite (2113 tests), tsc, and conventions pass. The explicit-grouping refinement was then applied: a dedicated `historical-nickname` operator category now classifies Barraging and Miraging (no longer `compound-set` / `body`), and the glossary's operator reference tags those entries with a visible "historical name" flag plus an explanatory note, so the official-set-vs-nickname split is explicit on the reference surface, not carried by prose alone.

**FS-46 — Emerging Vocabulary resolver: gate already-represented names off the work surface**
P2 | code (generated-content correctness) + test | owner: James (maintainer) | status: DONE, not a launch blocker | new: yes (2026-07-09 authoring-opportunity audit)
Product distinction (read this first): this change does NOT shrink the historical trick-name universe. Every name is preserved. It shrinks the unresolved Emerging Vocabulary work surface by removing names that are already represented by a published canonical trick or an explicit alias, so the surface can be trusted as an authoring backlog instead of over-counting published work as unresolved.
Context: the observational generator's canonical dual-gate compared only each row's own normalized slug against the published/alias set, so a name that resolves to a published trick under a folk-nickname suffix ("Nuclear Drifter (69)"), an abbreviation ("Nuclear DLO"), or a parenthetical folk name ("Gyro Torque (Mobius)") was left on the surface as if it were unresolved vocabulary.
Approved resolver rules (all in `freestyle/scripts/build_observational_universe_content.py`, helper `_represented_norm_candidates` feeding the gate): (1) strip parenthetical folk nicknames before matching; (2) expand the unambiguous abbreviations DLO / DSO / DOD / DDD / DATW to their full-form slugs; (3) resolve against explicit aliases (the pre-existing live-DB gate already loads `freestyle_trick_aliases`); (4) preserve positional forms unless the exact positional alias exists; (5) never collapse a same-side / far / near / opposite-side form to its base trick by assumption. The positional guard is load-bearing and is covered by a test: a parenthetical that is a bare positional marker ("(ss)", "(far)", "(same side)") is never stripped, and no positional token is expanded, so a side configuration is matched only by its own exact full-form alias. Confirmed against a real case: "Fairy DLO (ss)" stays even though `fairy_double_leg_over` is published, while "Nuclear ss Butterfly (Barfry)" drops only because `nuclear_ss_butterfly` is itself an explicit alias.
Result (regenerated `src/content/freestyleObservationalUniverse.ts`): total Emerging Vocabulary rows shown 970 to 819; Needs Authoring (authoring state) 134 to 118; 27 false positives removed from non-alias work buckets (authoring 16, doctrine 6, governance 2, identification 2, folk 1); 124 already-alias rows removed from the surface because their canonical object is published; 31 ambiguous same-side / far / near positional forms were refused (kept as distinct, authorable-with-a-curator-equivalence-check). Frantic remains the cleanest first authoring batch after the surface is cleaned (all 9 Frantic rows survived, one shared mechanic, on no exclusion list; every larger surviving cluster is either excluded or positional-pending or Red-blocked).
Not implemented, tracked separately (follow-up 1): no dedicated "inactive-by-design set/operator" gating was added. The pre-existing live-DB gate already excludes every database-tracked slug regardless of active state (it selects all `freestyle_tricks` slugs, not just active ones, plus all aliases), so inactive rows that exist in the database are already off the surface. Any residual notion of an inactive-by-design set/operator that is not a database row is not handled by this change and is not claimed as handled.
Not a display bug (follow-up 2): the "Ready 1 vs stats 2" difference is not visible to users. All three rendered "ready" surfaces (the headline chip, the ecosystem-matrix column, and the summary sentence) read the nine-state ladder count `evStates.ready` (1) consistently; the section-based `stats.ready` (2) is an unused field on the stats object, never rendered. The two `ready`-section rows each stack the same operator twice, so the repeated-operator doctrine gate stamps them `evState=doctrine`, which is why the section total and the ladder total differ. Optional future cleanup is to drop the unused section-count stats fields; there is nothing to fix on the rendered page.
Files: `freestyle/scripts/build_observational_universe_content.py` (helper + gate), `src/content/freestyleObservationalUniverse.ts` (regenerated), `tests/unit/observational-universe-consistency.test.ts` (folk-suffix / abbreviation / alias / positional-kept / positional-base-never-collapsed contracts).
Verification: focused resolver + consistency tests 13/13; Emerging Vocabulary route and redirect tests 20/20; rendered-output audit 18/18; `npx tsc` clean; regeneration is byte-identical idempotent, so `scripts/ci/assert_generated_content_current.sh` passes once the regenerated module is committed.

**FS-47 — Emerging Vocabulary formula-identity audit (read-only research)**
P2 | read-only audit | owner: James (maintainer) | status: DONE, research artifact | new: yes (2026-07-10)
A read-only pass over all 819 Emerging Vocabulary rows testing each as a movement-language expression (formula identity) rather than a literal trick name. Artifact: `exploration/ev-formula-identity-audit-2026-07-10/` (`REPORT.md` + `EV_FORMULA_IDENTITY_ROWS.csv`). No content authored, no generator/resolver/parser/dictionary/alias/status change, no Red questions created.
Result (819 rows): already represented 317 (38.7%); new and formula-resolved authorable-now 12 (an upper bound, see below); formula-blocked 318; identity-blocked 71; source-blocked 2; not-a-trick-object 99. Roughly 39 percent of observed names are duplicate labels for formulas that already exist, on top of the 151 the resolver fix already removed.
Two authoring candidates were withdrawn on the stricter standard. Frantic is a recognized token but not a settled operator in the operator authority (no defined ADD/structure/X-Dex; the generator groups it with folk operators), so Frantic rows are formula-blocked and route to the Scoring paper, not the authoring queue. Inward Gyro then failed the notation-token authority check: "inward" is not a notation token (it exists only as prose; the notation expresses direction as IN/OUT on the dex, and a gyro's dex direction is already intrinsic), so each Inward Gyro row is either a redundant restatement of an existing gyro_X canonical (already represented) or a direction collision with the base's own OUT dex (a Notation-paper question). No clean ready-to-author batch survives on the current surface.
Residual blocker map (no new Red questions; each Red-facing band assigned to a paper): Scoring 179 (blurry-expansion, operator structural definitions, repeated-operator, terraging); Notation 35 (zulu/weaving notation-governance, cross-body rake base, directional syntax); parser-internal 103; curator equivalence data 40 (positional); curator Identity-frame 21 (down-family); curator identity ruling 10; source/video 2; internal deferral 1 (pogo).
Working understanding to carry forward (the audit's durable lesson): freshly generated does not mean correctly classified (a ready/authoring generator state is a lead, not a warrant); recognized name tokens do not equal formula authority (a known token is not a settled operator with a defined ADD/structure/notation, nor a valid notation token); formula identity, not trick-name matching, drives authoring (author only when the formula is settled, every token is defined and representable, and the row is neither a redundant restatement nor a collision with an existing canonical).
...
| 288 | Strip embedded design-doc references | FS-13 (test-side residue); the freestyle test/content parts were verified already done, and the three exploration files stay as research artifacts under the FS-25 fence (no action) |
...
- **Batch C — Source-of-truth hardening.** Goal: generated artifacts provably match inputs; pre-cutover records corrections safe. Tasks: FS-10 (drift guards for both generated modules; the tracked-names module is not an orphan and stays), FS-17 (records-loader edited-but-skipped detection), FS-22 (demo posters and MP4s). Done: drift guard fails on stale regeneration; records edits warn or land; demos seed.
- **Batch D — Pipeline hygiene and exploration fence.** Goal: the freestyle pipeline is self-contained and `exploration/` is fenced off from every live dependency while staying as the research home. Tasks: FS-14 (rename `inputs/noise/`, extend rule globs), FS-25 (remove dead `exploration/` references from the producing generator and document the boundary; `exploration/` is not retired), FS-20 (gallery links). Done: rebuild green after the rename; committed-data regeneration is self-contained; `exploration/` fenced but retained; gallery links live.
...
**DC-4 — Orphaned generated file: SUPERSEDED (not an orphan, keep and guard).** `src/content/freestyleTrackedNames.ts` is consumed by four integration tests as promotion-regression coverage; the earlier retire ruling rested on a `src/`-only import check. It is retained and given a regeneration-is-a-no-op drift guard, which answers the second-source-of-truth concern without deletion. Revised under FS-10.

**DC-5 — Symbolic-grammar regenerability: RESOLVED by fence, no relocation.** The committed symbolic-grammar CSVs regenerate from `freestyle/inputs/observational/SYMBOLIC_GRAMMAR_MASTER.csv` and the DB alone; the producing generator's `exploration/` path constants were dead and are removed. `exploration/` stays as the research home and is never a required live build dependency (superseding the earlier relocate-before-retiring plan). Executed by FS-25.
