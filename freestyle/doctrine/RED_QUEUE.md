...
The live list of freestyle doctrine questions awaiting the rules expert (Red Husted)
or the curator. This document supersedes the reduction audit's final question list
(`exploration/red-reduction-audit-2026-07-02/AUDIT.md`, "Part C — Final reduced Red
question list") as the list of record. Maintenance rule: when a question is
...
2. **Embedded-base labeling frame for down-family operator compounds** — the
   ...
   itself; what remains is one line of doctrine: which coordinate frame names the
   embedded base of an operator compound (entry-relative after stripping operator
   contacts, with or without the evidenced body-spin frame inversion). It decides
   the three shipped base-label tension rows (shooting_star,
   tapping_double_over_down, venom — detail table in `DOWN_FAMILY.md`); those rows
   and any relabeling that depends on the frame stay deferred until it lands. No ADD
   changes ride on it.
...
- **The atomic-pickup alias target** — "atomic pickup" is already omelette's alias
  ...
  structure. Rebinding the alias without an explicit call would silently overwrite
