...
## Blazing = +1 by arithmetic (structure still with Red)
...
**Ruling: the ADD question is closed by the corpus — blazing contributes +1.** What
remains for the expert is only the movement/notation definition (what the blazing
movement physically IS); that question is tracked in `RED_QUEUE.md`. If the
structural answer ever implies a different count, the correction is a per-row ADD
fix.
