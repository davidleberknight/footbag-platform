...
- **Grammar-closure study.** `exploration/grammar-closure/`. Whether a fifth primitive is still hidden; the pogo and bare-surface-kick question. Upstream of any higher-dex generation.
- **Grammar v1 generation and Phase II-A.** `exploration/grammar-v1-generation/`, `exploration/trick-universe-phase-2a/`. The frozen grammar's generation runs the Atlas builds on.
- **Parked Movement Composition branch.** `exploration/movement-composition/INDEX.md`, with the concept at `exploration/state-transition-checker/CONCEPT_SPEC.md`. How one whole trick follows another (combos). Intentionally paused; not an Atlas volume.
- **Emerging Vocabulary formula-identity audit.** `exploration/ev-formula-identity-audit-2026-07-10/`. A production-facing audit that read the dictionary's emerging vocabulary as movement-language formulas rather than literal names; the same formula-over-name principle the Atlas confirms.

## Known unresolved questions

Carried forward, not answered here:

1. **The primitive floor.** Is a fifth basic building block still hidden (the pogo and bare-surface-kick question)? This gates sound higher-dexterity generation.
2. **The handoff state.** The four-part model is incomplete; extend it with the body-or-orientation join, richer direction, and paradox modifier, then re-validate against the corpus.
3. **The notation-invisible components.** Which foot frames a movement and which leg circles are absent from 100% of the notation; resolving them needs evidence from outside the notation (video or expert), a distinct step from the notation-only work.
4. **The simultaneity operator.** How two circles performed together (rather than in order) should score is unresolved.
5. **The parameter grades.** How rotation, paradox, and posture change the score is not pinned.
6. **The unresolved surface.** The peak surface is undecided as a landing versus a timing marker.
7. **Higher dexterity.** Three-or-more-circle movements are unmapped; the fiber law predicts their geometry but the join constraints compound.

## Restart conditions

The Atlas resumes only when one of these holds:

- **A grammar-foundation ruling lands** (the primitive floor, question 1), making higher-dexterity generation sound; then a higher-dex volume can begin.
- **Outside evidence becomes available** for the notation-invisible components (question 3), enabling the handoff-state model to be completed and re-validated.
- **A curator seeds the transition matrix**, resuming the parked Movement Composition branch (which studies combos, not single movements).
- **A separately approved research phase** is opened for any of the unresolved questions.

Until then, the Atlas is applied, not extended: the handoff (`PRODUCTION_HANDOFF.md`) carries the conclusions mature enough to improve the public dictionary and its quality checks, and the research trail above stays intact for whoever returns.
