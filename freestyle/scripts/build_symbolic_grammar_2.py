...
TOPOLOGY_GROUPS = [
    ("butterfly-wing-topology", "Butterfly wing topology", "topology",
     ...
     "butterfly, ripwalk, dimwalk, sidewalk, bigwalk, parkwalk, phoenix, dada_curve, matador"),
    ("whirl-rotational-topology", "Whirl rotational topology", "topology",
     ...
     "whirl, spinning_whirl, ducking_whirl, paradox_whirl, stepping_whirl, atomic_whirl, hatchet"),
    ("swirl-rotational-topology", "Swirl rotational topology", "topology",
     "Mirror of whirl topology: reverse-direction rotational dex.",
     "swirl, whirling_swirl, paradon_swirl"),
    ("osis-rotational-topology", "Osis rotational topology", "topology",
     ...
     "osis, stepping_osis, ducking_osis, miraging-osis (=torque)"),
    ("torque-rotational-topology", "Torque rotational topology", "topology",
     ...
     "torque, spinning_torque, atomic_torque, blurry_torque, mobius"),
    ...
    ("drifter-miraging-clipper-topology", "Drifter (miraging-clipper) topology", "topology",
     "Drifter = miraging clipper canonical reading; topology mirror of clipper_stall family.",
     "drifter, reverse_drifter, paradox_drifter, smoke, royale"),
    ("atw-leg-circle-topology", "ATW leg-circle topology", "topology",
     ...
     "around_the_world, double_around_the_world, reverse-around_the_world"),
    ...
    ("xdex-topology", "X-Dex topology (full-circle dex variant)", "topology",
     ...
     "atom_smasher, witchdoctor, sumo, merlin"),
    ...
    ("stall-1add-topology", "1-ADD stall topology", "topology",
     ...
     "toe_stall, clipper, inside_stall, outside_stall, heel_stall, sole_kick, spin, spyro"),
]
...
MODIFIER_GROUPS = [
    ("spinning-family", "Spinning modifier family", "modifier", "+1 flat per pt10; rotational body modifier.",
     "spinning_whirl, spinning_osis, spinning_clipper, spinning_torque, spender, montage"),
    ("paradox-family", "Paradox modifier family", "modifier", "+1 universal body modifier; in-then-out dex pair structure.",
     "paradox_mirage, paradox_whirl, paradox_blender, paradox_drifter, paradox_symposium_whirl"),
    ...
    ("ducking-family", "Ducking modifier family", "modifier", "+1 body modifier; duck head movement.",
     "ducking_whirl, ducking_osis, ducking_butterfly, ducking_clipper, mind_bender, phoenix, montage"),
...
CONTACT_GROUPS = [
    ("toe-start", "Toe-stall start", "contact-position", "Trick begins from a toe_stall plant.",
     "atomsmasher, mirage, whirl, blaze, etc. (~172 FM rows)"),
    ("clipper-start", "Clipper-stall start", "contact-position", "Trick begins from a clipper_stall plant.",
...
DEX_GROUPS = [
    ("hippy-in-dex", "Hippy in-to-out dex", "dex-mechanic", "Hip-driven inside-to-outside dex motion (mirage canonical).",
     "mirage, smear, blur, atom_smasher, sumo"),
...
EXECUTION_PATTERNS = [
    ("directional-reverse-pair", "Directional-reverse pair (direction-is-structural)", "compositional",
     ...
     "drifter/reverse_drifter, whirl/rev_whirl"),
    ("productive-multiplicity-stabilized", "Productive-multiplicity stabilized doubling (pt8 §10)", "compositional",
     ...
     "double_leg_over, double_around_the_world, double_spin"),
    ...
    ("same-side-execution", "Same-side execution (ss qualifier)", "positional",
     ...
     "sidewalk, parkwalk, ripstein, dada_curve"),
    ...
    ("uptime-dex-downtime-osis", "Uptime dex + downtime osis", "compositional",
     ...
     "stepping_osis, ducking_osis, ripped_warrior"),
...
def write_groups_csv(rows: list[tuple], path: Path, source_basis: str) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=GROUP_FIELDS, quoting=csv.QUOTE_ALL, lineterminator="\n")
        w.writeheader()
...
def derive_topology_for_trick(t: dict, mod_links: dict[str, list[str]]) -> list[tuple[str, str]]:
    ...
    base_topology_map = {
        ...
        "reverse_drifter": "drifter-miraging-clipper-topology",
        "around_the_world": "atw-leg-circle-topology",
...
def derive_contact_memberships(t: dict) -> list[tuple[str, str]]:
    ...
    if any(w in name for w in ("toe", "blur", "mirage", "atom_smasher", "smear")):
        out.append(("toe-ending", "compound terminates in toe_stall via canonical-name pattern"))
    if "clip" in name or "butterfly" in name or "wing" in name or t["base_trick"] == "butterfly":
        out.append(("clipper-ending", "compound terminates in clipper_stall via wing-motion or canonical-name pattern"))
    return out
...
def derive_execution_memberships(t: dict, mod_links: dict[str, list[str]]) -> list[tuple[str, str]]:
    ...
    if slug in ("reverse_drifter", "rev_whirl", "rev_up"):
    ...
    if slug in ("double_leg_over", "double_around_the_world", "double_spin"):
        out.append(("productive-multiplicity-stabilized", "pt8 §10 community-stabilized doubling"))
    # §3.2 policy class
    if slug in ("nemesis", "bullwhip"):
    ...
    if slug in ("barrage", "barfly", "barfry", "bubba_beater"):
...
def write_membership_csv(state: dict) -> int:
    ...
    with (OUT_DIR / "symbolic_group_membership.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=MEMBERSHIP_FIELDS, quoting=csv.QUOTE_ALL, lineterminator="\n")
        w.writeheader()
        w.writerows(rows)
    return len(rows)
...
EQUIVALENCE_CLUSTERS = [
    ("wing-on-butterfly", "Wing motion on butterfly base", "modifier + butterfly",
     "butterfly, ripwalk, sidewalk, dimwalk, bigwalk, parkwalk, phoenix, dada_curve, matador, tripwalk, arcwalk",
    ...
    ("hippy-in-on-mirage", "Hippy-in dex on mirage base", "modifier + mirage",
     "mirage, smear, blur, atom_smasher, sumo, witchdoctor, paradox_mirage, stepping-paradox_mirage",
    ...
    ("rotational-on-whirl", "Rotational dex on whirl base", "modifier + whirl",
     "whirl, spinning_whirl, ducking_whirl, paradox_whirl, stepping_whirl, atomic_whirl, hatchet, symposium_whirl, paradox_symposium_whirl, spinning-symposium_whirl",
    ...
    ("rotational-on-osis", "Rotational dex on osis base", "modifier + osis",
     "osis, stepping_osis, ducking_osis, ripped_warrior, spinning_osis",
    ...
    ("leg-pass-on-legover", "Leg pass on legover base", "modifier + legover",
     "legover, eggbeater, pigbeater, scrambled_eggbeater, smog, flurry",
    ...
    ("scoop-on-pickup", "Under-foot scoop on pickup base", "modifier + pickup",
     "pickup, paste, legeater, pigbeater, scrambled_eggbeater, omelette",
     "pixie/quantum/atomic/illusioning-pending; ADD 2-3", "2-3",
     "pickup-topology",
     "Omelette's illusioning modifier is dictionary-flagged data-debt."),
    ("circle-on-atw", "Leg circle on around_the_world base", "modifier + ATW",
     "around_the_world, double_around_the_world, reverse-around_the_world",
    ...
    ("drifter-direction-variants", "Drifter direction-variant pair", "drifter / reverse_drifter",
     "drifter, reverse_drifter, paradox_drifter, smoke, royale",
    ...
    ("whirl-direction-variants", "Whirl direction-variant pair", "whirl / rev_whirl",
     "whirl, rev_whirl, rev-up",
    ...
    ("policy-3-2-trio", "§3.2 stated-ADD-without-stated-structure trio", "row-asserted (no modifier+base arithmetic)",
     "nemesis, bullwhip",
     "Published §3.2 policy-class exemplars at pilot tier; row-level ADD assertion", "5-6",
     "(cross-cuts topology)",
    ...
    ("blurry-pt12-cluster", "Blurry transitive-expansion cluster (pt12 Red-blocked)", "blurry + X (transitive)",
     "blur, blurry_whirl, blurry_torque, food_processor, blurriest",
...
def write_equivalence_clusters_csv(state: dict) -> int:
    ...
    with (OUT_DIR / "symbolic_equivalence_clusters.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=EQUIVALENCE_FIELDS, quoting=csv.QUOTE_ALL, lineterminator="\n")
        w.writeheader()
        w.writerows(rows)
    return len(rows)
...
def write_family_analysis_csv(state: dict, memberships: list[dict]) -> int:
    ...
    for family, tricks in family_tricks.items():
        ...
        # Ties are broken by name, not by insertion order. A family whose topology
        # groups are level (a singleton in two groups is the common case) would
        # otherwise pick a different winner per run, because the counts are built
        # by iterating a set and set order varies between processes. That makes
        # the artifact unreproducible, which the freshness check would report as
        # drift on a run where nothing changed.
        ranked = sorted(topology_counts.items(), key=lambda kv: (-kv[1], kv[0]))
        if ranked:
            primary, primary_n = ranked[0]
        else:
            primary, primary_n = "(none)", 0
        coherence = (primary_n / size) if size > 0 else 0.0
        cross_cutting = ", ".join(t for t, _ in ranked if t != primary)
    ...
    with (OUT_DIR / "symbolic_vs_ifpa_family_analysis.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=ANALYSIS_FIELDS, quoting=csv.QUOTE_ALL, lineterminator="\n")
        w.writeheader()
        w.writerows(rows)
    return len(rows)
...
ARCHETYPES = [
    ("uptime-dex-downtime-butterfly", "Uptime dex + downtime butterfly wing",
     ...
     "ripwalk, dimwalk, sidewalk, bigwalk, parkwalk, phoenix, dada_curve, matador, tripwalk",
     3, 5,
    ...
    ("uptime-dex-downtime-osis", "Uptime dex + downtime osis arc",
     ...
     "Stepping-osis and ducking_osis form a clean cohort."),
    ...
    ("spinning-drifter-rotational", "Spinning + drifter rotational topology",
     ...
     "alex-zerbe, smoke (when spun), reverse_drifter (mirror)",
     3, 5,
    ...
    ("whirl-rotational-topology-anchor", "Whirl rotational topology (no modifier)",
     "set-into-rotational-dex", "(rotation)", "toe_stall-finish",
    ...
    ("xdex-named-compound", "X-Dex named compound (pt1 narrow)",
     "set-into-hippy-in-dex", "(none)", "X-Dex flick + toe_stall-finish",
    ...
    ("policy-3-2-row-asserted", "§3.2 row-asserted ADD compound",
     ...
     "Published §3.2 exemplars at pilot tier."),
...
def write_archetype_registry(state: dict) -> int:
    ...
    with (OUT_DIR / "movement_archetype_registry.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=ARCHETYPE_FIELDS, quoting=csv.QUOTE_ALL, lineterminator="\n")
        w.writeheader()
        w.writerows(rows)
    return len(rows)
