...
-- The net review queue, the raw-fragment and candidate-match staging tables, and
-- the curated-match audit trail are retired. They served an internal QC review
-- subsystem that no longer exists; QC findings from the net loaders are written
-- to legacy_data/out/net_team_qc_issues.jsonl instead.
...
-- The recovery-alias and team-correction approval tables are retired with the
-- operator review subsystem that filled them. Both fed exports into curated
-- files that are committed and complete: overrides/person_aliases.csv and
