...
CREATE UNIQUE INDEX ux_media_items_source_video
  ...
  WHERE source_id IS NOT NULL AND video_url IS NOT NULL
    AND moderation_status = 'active';
-- media_items_linkable_video: the only read surface for "does this subject have a
-- watchable video". Filters the three conditions that make a video row reachable in
-- a gallery: it is a video, it is not admin-removed, and its embed still resolves
-- (rows tagged '#unavailable_embed' point at footage the host has taken down).
-- Source is deliberately NOT a condition: a member-uploaded clip leaves source_id
-- NULL and is trick media exactly as a curator-published clip is, so any query
-- deciding whether media exists reads this view rather than filtering the bare
-- table, which is how the browse rows and the trick detail page came to disagree.
