...
`scripts/install-cwagent-staging.sh` is the model (stdin-piped sudo password + cat-piped remote-half + `printf %q` assignments); preserve that wire-pattern verbatim. Operator-facing rationale: `docs/DEVOPS_GUIDE.md` §20.3. Enforced by code review.
