...
- **No dedicated "Cross-references" sections** or trailing "Cross-references:" bullets in DD, DATA_MODEL, or other long-term docs. Inline refs in prose are fine when they're load-bearing for comprehension. AI routing between docs is handled via CLAUDE.md + skill triggers + memory, not in-doc navigation.
...
**External stakeholders get role labels too.** The stakeholder-coordination layer lives in GO_LIVE_PLAN.md (private GitHub repo), so no public doc names any of the people involved in the migration: the same role labels as above, plus "the legacy webmaster" and "the IFPA secretary" for the two external stakeholders. Private-tracker issues and private-checkout documents still name people by first name. For legacy data still use "the legacy data" / "the legacy data import", not possessive constructions tied to a person — the neutral framing is about data ownership, not naming.
