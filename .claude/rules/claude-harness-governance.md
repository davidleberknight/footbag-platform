...
- A new destructive command is guarded by a `permissions.ask`/`deny` entry, not only a hook — project permissions inherit into subagents, whereas a `settings.json` hook does not fire in one (see Subagent safety below). Reserve hooks for guards that a static rule cannot express (a positional `find -delete`, a `curl -X POST` flag anywhere in the args).

## Subagent safety

Project `permissions.*` rules inherit into subagents; this inheritance is the guaranteed, version-proof floor, so mirror every statically-expressible guard into `permissions.deny`/`ask`. A hook wired in `settings.json` does NOT fire for a subagent's tool calls — but a PreToolUse hook declared in an agent's own `.claude/agents/*.md` frontmatter does fire for that agent's calls, running alongside the inherited permission rules. So a custom agent that has `Bash` (the `auditor`) declares the full Bash guard chain and the read-only approver in its frontmatter, restoring the secret-read block and destructive-command guards and keeping read-only work prompt-free inside it; an agent with no `Bash`/`Edit`/`Write` (the `researcher`) declares none because it has nothing to guard. Because cross-version subagent hook-firing is best-effort, the frontmatter hooks are defense-in-depth on top of the permission floor, never a replacement — and the read-only approver is kept self-sufficient (it refuses any write- or exec-capable form itself, not by leaning on a sibling guard) so it is safe as the lone approver inside a subagent. A built-in Task agent (`Explore`, `general-purpose`, `Plan`) carries no frontmatter, so it gets only the permission floor plus its tool restriction. A phantom permission prompt for a safe command inside a subagent is a config gap — diagnose it, never silently approve. Read-only subagents keep read-only tools and a non-mutating posture.
