...
# device (/dev/null), and the AI's own session scratchpad (/tmp/claude-*/.../scratchpad/) -- then
# ask if any redirect remains. The `/scratchpad/` segment is required, so a bare /tmp/claude-*
# path (not the scratchpad) still gates, matching guard-rm.sh's delete exemption. `..` in the
# command keeps the scratch exemption off so a path escape stays
...
case "$COMMAND" in
  *..*) : ;;
  *) RSCAN="$(printf '%s' "$RSCAN" | sed -E 's#[0-9]*>>?[[:space:]]*/tmp/claude-[A-Za-z0-9._/-]*/scratchpad/[A-Za-z0-9._/-]*##g')" ;;
esac
...
# intact: a `>` in a single-quoted region is always inert (bash never expands there), and a
# `>` in a DOUBLE-quoted region is neutralized unless that region contains a command
# substitution -- `$(` or a backtick -- that runs a command. A plain parameter expansion
# ($x, ${n}) does NOT block neutralization: the shell parses redirections before expanding a
# variable, so a value can never introduce one. A redirect hidden in a command substitution
# ("$(cmd > f)") keeps its `>` and still asks; arithmetic `$((a>b))` stays conservative too.
