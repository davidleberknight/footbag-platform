...
- A coverage ledger lists every deployed controller, service, middleware, adapter, shared lib, template, worker/entry point, client-side JS file, route file, config module, operational script, terraform file, systemd unit, and git hook with a read / not-read mark and (for not-read) an explicit reason; the sweep is not complete while any security-relevant surface is unread.
...
4. `IMPLEMENTATION_PLAN.md` — the `[DEVIATION]`-tagged entries (inline under the owner sections; there is no dedicated deviations block). A behavior that looks like a bug may be an explicitly tracked deviation; cross-check before flagging. The check is bidirectional: a `[DEVIATION]` or `[BUG]` entry describing a state the code no longer has is itself a finding (stale plan entry). The legacy-pipeline deviations recorded there are mandatory to check even though the `legacy_data/` pipeline itself is out of scope, because they can surface in deployed routes.
5. **All path-scoped rules in `.claude/rules/*.md`** — enumerate the directory fresh and read every rule; the set changes, so never trust an embedded list — and the relevant `.claude/skills/*` procedures. These are the operational rules and workflows every finding must respect (the layer-boundary rules are the canonical patterns the §4.4B design-divergence sweep checks against; `script-secret-safety.md` is the canonical credential convention §4.4.40 audits against; `claude-harness-governance.md` grounds §4.4.48).
...
Organize the primary (security/correctness) sweep around the expanded categories in §4.4.1-§4.4.48. Each one names the typical site, the specific pattern, and an example signal. After the primary sweep, run the secondary design/hygiene sweep in §4.4B (controller/template/service/db divergence, cross-layer leaks, comment + JSDoc drift, test-code rule violations) and report its findings in the dedicated `BUGS.md` design-divergence group.

The full category catalog is in `REFERENCE.md`: the primary security/correctness categories (§4.4.1–§4.4.48), each naming its typical site, pattern, and example signal, and the secondary design-pattern-divergence and hygiene sweep (§4.4B, with §4.4B.1–§4.4B.8). The methodology below refers to these category numbers; read the relevant category in `REFERENCE.md` before sweeping for it.

Freestyle boundary: the freestyle routes get this skill's full generic sweep (auth, XSS, caching, error handling, and every other §4.4 class), but the freestyle DOMAIN invariants — dictionary layer separation, ADD math, slot governance, naming/slug/hashtag conventions, the trick-tag invariant, propagation — belong to the `freestyle-bug-hunt` skill. Hand a domain-rule candidate there; do not re-derive freestyle doctrine here.
...
5. **Sweep every route file in `src/routes/` + `src/app.ts`** for route registrations (enumerate the directory — public, admin, internal, ipc, health, seo, and anything newer). Build a mental map of which controller serves each route. Note which routes traverse each middleware gate — enumerate `src/middleware/` fresh (`requireAuth`, `requireAdmin`, the tier gates including `requireTier1Benefits` / `requireTier2Plus` / `requireTier3`, `requireOnboardingComplete`, `requireOriginPin`) — plus rate limits, body parsers, upload middleware, and any route-specific cache/header behavior. Any divergence from the expected gate pattern is a candidate finding.
6. **For each deployed-US controller method** (per §1 inventory and the traceability matrix, including `freestyleController`):
   ...
   - Cross-check the path against §4.4.1-§4.4.48.
...
13. **Sweep the client-side JavaScript** (§4.4.45): every file in `src/public/js/`. These ship to browsers; check DOM-XSS sinks (`innerHTML`, unsanitized insertion), CSP compliance, endpoint wiring against real routes, and progressive-enhancement fallback for the controls they enhance.
14. **Sweep the workers and entry points**: `src/server.ts`, `src/worker.ts`, `src/imageWorker.ts`, `src/transcodeWorker.ts`, `src/runBatchAutoLink.ts`, and any newer entry point. Apply the same categories as services (races, leases, idempotency, error swallows, env reads, redaction).
15. **Review deterministic verification coverage** (§4.4.38 and §5): inspect `package.json`, `scripts/ci/`, `run_all_tests.sh`, `.github/workflows/`, test commands, lint/typecheck commands, security scanners, custom convention checkers, and repo-level gates. The repo already carries a gate set — audit each existing gate's COVERAGE against the class it pins, and record missing or partially-covering checks as Testing and CI/CD verification gaps.
16. **Cross-check runtime filesystem reads against the deploy image** (§4.4.42). List every `readFileSync` / `existsSync` / `readdirSync` and every `resolve(...)` / `path.join(...)` data-path construction in `src/`, resolve each to its in-container path, and confirm a matching `COPY` exists in `docker/web/Dockerfile` (and worker/image where relevant). Verify the existing CI gate for this class still covers every read pattern. Any read of a path no Dockerfile ships is a finding, especially when a missing-file fallback would mask it.
17. **Review clickable-route/form-target coverage** (§4.4.39): determine whether the repo has a deterministic rendered-link/form-action/button-navigation smoke test. If not, identify the deployed surfaces it should cover and record the gap.
18. **Sweep operational scripts and systemd units** (§4.4.44): `scripts/**` (deploy, backup, ops, validation) and `ops/systemd/*`. Logic and safety, not just credentials: destructive-action guards, shell strictness, trap cleanup, remote-half quoting, idempotency, timer cadence vs documented intent.
19. **Sweep the infrastructure code** (§4.4.43): `terraform/**` (staging, production, shared), `docker/**`, `.githooks/`. Security posture and gate integrity, plus staging/production parity.
20. **Check harness guard coverage** (§4.4.48): `.claude/settings.json` deny/ask rules against the guard hooks and the harness-governance rule's subagent-safety requirement; hook fixture coverage in the CI hook tests.
21. **Secondary sweep: layering divergence** (§4.4B.1-§4.4B.5). Re-scan the controllers/services/templates/db already read against the canonical layer boundaries: business logic in controllers/templates, controller mutating service output, raw-enum branching in views, services duplicating other services' logic, hardcoded operational values, cross-layer leaks. Record under the `BUGS.md` design-divergence group.
22. **Secondary sweep: comment + JSDoc drift** (§4.4B.6, including `scripts/**` comments), service-JSDoc consistency (§4.4B.8), and test-code rule violations (§4.4B.7) across the files read: stale/bogus comments, forbidden comment content, deviation comments missing `Current:`/`Target:`, service header/exported contract JSDoc that no longer matches the contract, committed-test rule breaks.
23. **Cross-check `src/` directly** (and the §3 table for explicitly-tracked actionable items) to confirm a finding has not already been remediated in code before flagging. Landed items are not tracked in §3; the code is authoritative.
24. **Self-verify** (§4.7) before writing to `BUGS.md`.
...
- **Class:** <one of §4.4.1-§4.4.48 or §4.4B.* categories>
...
**Testing/CI gap handling — a testing gap is a bug.** A missing mandatory-class deterministic check (the testing rule's Mandate list, the `docs/TESTING.md` §4.5 catastrophic-surface floor, §7.2 adapter parity, the §17.2 generative sweeps, or any §4.4 class a cheap stable check can pin) is a first-class finding with a real severity — calibrate it to the surface the gap leaves unguarded per `docs/TESTING.md` §3, never a default Low. It is still reported in the **Testing and CI/CD verification gaps** group for structure. Set `File:` to the closest place the guard should live (`package.json`, `.github/workflows/*`, `scripts/ci/*`, `docs/TESTING.md`, or the affected test directory). Set `Class:` to `§4.4.38 Deterministic automated-verification gaps`, `§4.4.39 Clickable-route, form-target, and navigation smoke-test gaps`, `§4.4.40 Script credential-handling and password-leak convention violations`, or `§4.4.41 Secrets, PII, and credential-leak automated-check gaps`. The reproduction states the exact defect class the missing check would allow, not merely "add more tests."
...
1. **Scope filter held + both plans' deviations cross-checked**: zero findings against the `legacy_data/` pipeline unless the kickoff prompt included it (migrated data surfacing in deployed routes is fine); any per-run exclusions from the kickoff prompt were honored and recorded in the `BUGS.md` scope note. Every deployed-route finding was cross-checked against the `[DEVIATION]`-tagged entries of `IMPLEMENTATION_PLAN.md`; a behavior already tracked there is not flagged, and a tracked entry the code has outgrown was recorded as a stale-plan finding.
...
- **Vague coverage complaints** ("needs more tests" with no named invariant) — not findings. But a missing MANDATORY-CLASS check is a bug, not a nicety: an untested success criterion of a deployed story, a missing regression/edge-case/contract test the testing rule mandates, a catastrophic surface below its §4.5 floor, or a missing deterministic guardrail for a known defect class (§4.4.38) all carry real severity. Name the exact missing check and the invariant it pins.
...
The sweep is not done when the steps are run; it is done when the surface is exhausted. Exhaustion means: (a) every surface class the §0.0 coverage ledger enumerates (controllers, services, middleware, adapters, shared libs, templates, workers/entry points, client-side JS, route files, config modules, operational scripts, terraform files, systemd units, git hooks) appears in the ledger marked read; (b) every §4.4 / §4.4B bug class has been applied to every surface it can touch; (c) the surface has been re-swept until two consecutive passes surface no new candidate; and (d) a completeness critic has asked "what class, surface, actor, or success criterion did this pass not touch?" and that list is empty or itself swept.
...
- You have grep-swept for the expanded bug categories in §4.4.1-§4.4.48.
...
This section is complementary to `docs/TESTING.md`. A testing gap is a bug: it is a real, severity-carrying defect to lack a deterministic check when the check is cheap, stable, and would prevent a known class of defects — calibrate severity to the surface the gap leaves unguarded, and record it like any other finding. The goal is to move repeatable verification out of the agent's head and into code that runs automatically. Audit the EXISTING gate set first (`scripts/ci/*`, `.github/workflows/ci.yml` jobs, `run_all_tests.sh`, the pentest harness under `scripts/pentest/`): most classes below already have a gate, and the finding shape is usually "the gate exists but does not cover X", which requires reading the gate's implementation, not just noting its presence.
...
- **No browser-level testing**. Live-browser verification (visual regression, real-browser cookie attribute interaction, runtime axe runs) is out of scope here; it lives in `tests/e2e/` (multiple Playwright specs, including a dedicated accessibility spec) and the operator-invoked `browser-qa` skill. Static accessibility review and a11y-harness coverage auditing ARE in scope (§4.4.46) — the go-live gate set includes a WCAG conformance gate.
...
- **CI/CD visibility may be incomplete**. Repo files show the committed gate set (package scripts, `scripts/ci/*`, the GitHub Actions workflow with its type-check / lint / dependency-audit / secret-scan / convention / harness / test / CodeQL / terraform-validate jobs), but they cannot reveal branch protection, required status checks, GitHub-side secret-scanning/push-protection settings, dependency-review enforcement, environment protection rules, artifact retention, log masking, or organization-level policies. If those cannot be verified from the repo, record a narrowly scoped Lead or ask one maintainer question rather than guessing.
