...
- **Em-dash scope:** an em dash in a code comment, JSDoc, script, or doc file is NOT a finding; do not sweep for or report them there. Em dashes are a finding only in PUBLIC-FACING content a site visitor can read: rendered template text, editorial/content strings, UI labels and notices, and notification email bodies. The no-em-dash rule applies only to that public-facing text, not to docs or code.
