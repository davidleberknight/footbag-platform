...
**Modifier Reference rendering is currently disabled on the public surface** (the `/freestyle/tricks` "Modifier Reference" section is a Handlebars block comment — editorial decision). The service still shapes `content.modifiers` for future reuse. Do not re-enable without explicit human approval (see memory `feedback_modifier_public_visibility.md`). Public modifier editorial content belongs on `/freestyle/glossary` only, in plain language.
