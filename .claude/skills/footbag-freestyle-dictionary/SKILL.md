...
- **Atomic = +1 on every base; any X-Dex is separate:** atomic contributes +1 ADD on every base, with no rotational class. Any X-Dex is a separate, receiver-gated +1 on a *following* far-form dex (receivers: mirage / illusion / whirl / torque / drifter), never inferred from the atomic name; where it fires, that dex carries the `[XDEX]` bracket (atom-smasher pattern: `TOE > OP OUT [DEX] > OP IN [DEX] [XDEX] > OP TOE [DEL]`). The doctrine is settled and `operatorReference.ts` is canonical for it; the stored ADD values of the affected rotational-base compounds are migrated, and the standalone atomic modifier registry is +1 on every base.
...
- **Atomic + miraging on non-rotational base (W9o atomic-miraging-butterfly):** `atomic-butterfly` canonical (4 ADD; non-rotational, atomic = +1 per W4a) + miraging `OP IN [DEX]` injection between atomic prefix and butterfly's OUT-dex → 5 ADD. Rotational-atomic is no longer blocked (the atomic question is answered: +1 flat on every base); extensions author under the settled doctrine.
...
**Observational page post-wave cleanup (W9 cleanup hygiene, see [[feedback_observational_canonical_promotion_cleanup]]):** After any wave that adds to `red_additions_2026_04_20.csv`, check `freestyle/inputs/observational/RECONCILIATION.csv` for stale rows whose slug is now canonical but `governance_state` still starts with anything other than `"1"`. Bulk-update via `sed` (NEVER `csv.DictReader → csv.DictWriter`). Regenerate `freestyleTrackedNames.ts` via `python3 freestyle/scripts/build_tracked_names_content.py`. The generator's dual-gate filter (2026-05-27 hardening) provides a safety net by also reading canonical CSVs directly.
...
| `atomic-pickup` | `omelette` | 3 (alias target open with the curator: omelette carries Red's naming ruling, scrambled-eggbeater is the byte-identical structure) |
...
- Toe Blur → Stepping Paradox Mirage (per the pt10 rules-expert adjudication; supersedes the earlier pt2 "Quantum Mirage" reading which gave 3 ADD vs asserted 4: see pt10 candidate-core resolution)
