...
// task. Public BROWSE stays reachable mid-onboarding: the clubs index at /clubs
// (no trailing slash, so never fenced), a club or country detail page at
// /clubs/:key, and these /members/ pages: own/any profile read, profile and
// password edit, and contact-admin. The /clubs exemption is scoped to a single
// browse segment and excludes the capability paths that share that shape (the
// create form and swap-primary), so those and the two-segment /clubs/:key/...
// action POSTs stay fenced. Admin pages carry their own requireAdmin gate; a
...
// outside /members/ and /clubs/ — the legacy-claim anchor and auto-link actions
// under /register/wizard/..., and the historical-person claim under /history/ —
// are never fenced here: claiming is part of finishing onboarding, so the
// wizard depends on those routes staying open mid-onboarding.
const UNGATED_PATTERNS = [
  /^\/members\/[^/]+$/,
  /^\/members\/[^/]+\/edit/,
  /^\/members\/[^/]+\/contact-admin/,
  /^\/clubs\/(?!create$|swap-primary$)[^/]+$/,
];
