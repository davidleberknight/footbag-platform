...
export const OPERATOR_REFERENCE_ENTRIES: readonly OperatorReferenceEntry[] = [
  ...
  },
  {
    ...
    name:           'Barraging',
    category:       'historical-nickname',
    oneLineMeaning: 'A historical name for the Furious set, not a set or modifier of its own. Furious is the official two-dex set (+2 ADD, two in-direction dexes; operational form CLIP > OP IN [DEX] > SAME IN [DEX] >). Some tricks carry barraging-based names for continuity, but the set is Furious and their decomposition resolves to Furious.',
    decomposition:  null,
    pendingNote:    'Barraging is retired as a scored, formula-bearing operator: it is a historical name for the Furious set, so no "barraging" component presents as a scored decomposition. Barraging-named tricks resolve to Furious (+2).',
    workedExamples: ['Baroque = Furious Osis', 'Fury = Furious Paradox Mirage'],
    lineageNote:    'A historical name for the Furious set; Furious is the official two-dex +2 set.',
    curatorConfirmPending: false,
  },
  {
    slug:           'inspinning',
