...
export const ALIAS_GOVERNANCE_ENTRIES: readonly AliasGovernanceEntry[] = [
  ...
  // that a direction-reversed movement is the same trick.
  {
    ...
    surfaceOnBrowse: false,
    displayAs:       null,
    reason:          'Infinity names the clipper-set Butterfly, not a generic alias of Butterfly: what distinguishes it is the set it opens from, since the far execution is the Butterfly default and only a same-side execution needs asserting. Suppressed from the generic Butterfly "Also called" line so the page does not present it as an exact alias; the historical Infinity name is preserved in the alias table. The name is instead surfaced on the Butterfly page as a curator naming note explaining that Infinity is the clipper-set butterfly, so a visitor who searched Infinity and landed on Butterfly learns what it points to without it being listed as an exact alias. Do not re-enable it on the alias line to make it visible.',
  },
];
