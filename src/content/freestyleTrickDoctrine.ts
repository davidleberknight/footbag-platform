...
export const DOCTRINE_DIVERGENCE_REGISTRY: ReadonlyMap<string, DoctrineDivergenceEntry> = new Map([
  ['blurrage', {
    ...
    sourceSystem:       'an outside source',
    sourceClaim:        3,
    canonicalValue:     4,
    noteVisibility:     'public',
    provenanceNote:     'An outside source historically lists blurrage at 3 ADD. The executable IFPA-grammar derivation yields 4 ADD via stepping(+1) + barrage(3); the 1-ADD divergence is part of a systemic pattern under review.',
    internalNote:       'Doctrine-divergence pilot publication. PB-claim 3 vs IFPA-derived 4. Gap=1; cleanest divergence pattern. Promotion from observational corpus per the pilot evaluation.',
  ...
  ['schmoe', {
    ...
    canonicalValue:     3,
    noteVisibility:     'public',
    provenanceNote:     'An outside source historically lists schmoe at 2 ADD. The executable IFPA-grammar derivation yields 3 ADD via stepping(+1) + legover(2); the 1-ADD divergence reflects the systemic gap under review.',
    internalNote:       'Doctrine-divergence pilot publication. PB-claim 2 vs IFPA-derived 3. Gap=1; same shape as blurrage. Reading `Stepping near Legover` carries positional `near` which is +0 per current convention.',
    relatedRedQuestion: 'Q7',
    status:             'published',
  }],
]);
...
export const OPERATOR_WEIGHT_DIVERGENCE_POLICY: ReadonlyMap<string, OperatorWeightDivergencePolicy> = new Map([
  ['furious', {
    ...
    affectedSlugs:        ['clown_face', 'genesis', 'rage', 'nebula'],
  ...
  ['railing', {
    ...
    sourceSystem:         'FootbagMoves',
    sourceOverCountRange: [1, 2],
    affectedSlugs:        ['dorshanatrix', 'flying_fish', 'rail_warrior'],
