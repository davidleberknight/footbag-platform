...
- `runtime.ts`. The four registration-time and request-time shortcuts (autologin, register-allowlist bootstrap, skip-claim-email, tier2 invariant repair), plus the boot banner.

## Five shortcuts (umbrella)
...
All persisted rows from these shortcuts carry `created_by` values under `dev-admin-shortcuts/*`.

## Cutover removal procedure

When production launch is imminent and the SSM-token `/admin/bootstrap-claim` path is ready:

1. `bash scripts/audit-dev-admin-shortcuts.sh` against the production DB. All four prefix counts must return zero.
2. `grep -rn "CUTOVER-REMOVE" src/` to find every callsite. Delete each line plus any block it gates.
3. `git rm -r src/dev-admin-shortcuts/`.
4. `git rm scripts/manage-dev-admin-seed.sh scripts/audit-dev-admin-shortcuts.sh`.
...
9. `npm test && npm run build` should pass with zero references to anything dev-admin-shortcut-related.
...
sqlite3 database/footbag.db "
  ...
  UNION ALL
  SELECT 'created_by', COUNT(*) FROM member_tier_grants WHERE created_by LIKE 'dev-admin-shortcuts/%';
"
```
