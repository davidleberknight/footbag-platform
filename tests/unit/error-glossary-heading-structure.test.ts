/**
 * Heading-structure conformance for the error page and the freestyle glossary.
 *
 * The error page's primary heading is the page's single <h1> (page.title per
...
// Every failing route renders one error template, so this is the whole set.
const ERROR_PAGES = ['error'];

function headingLevels(html: string): number[] {
  return [...html.matchAll(/<h([1-6])\b/g)].map((m) => Number(m[1]));
}

describe('the error page carries a single top-level <h1>', () => {
