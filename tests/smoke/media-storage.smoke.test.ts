/**
 * Media storage staging-smoke test.
 *
 * Long-term, opt-in smoke suite. Exercises createS3MediaStorageAdapter
 ...
 * keys are preserved exactly as the adapter writes them (no `media-store/`
 * prefix on the storage side; the `/media-store/` prefix is purely a URL
 ...
 * MEDIA_STORAGE_S3_BUCKET from terraform output, and gate behind
 * RUN_STAGING_SMOKE=1).
 *
 * Failure modes:
 ...
 *     the media bucket.
 *   - MEDIA_STORAGE_S3_BUCKET unset: terraform-output media_bucket_name
...
import { createS3MediaStorageAdapter } from '../../src/adapters/mediaStorageAdapter';
...
const SMOKE_PREFIX = `smoke/media-storage-${process.pid}-${Date.now()}`;
const trackedKeys: string[] = [];

describe.skipIf(!RUN)('media storage adapter against staging S3', () => {
  if (!bucket) {
    throw new Error(
      'MEDIA_STORAGE_S3_BUCKET must be set; check scripts/test-smoke.sh',
    );
  }

  const adapter = createS3MediaStorageAdapter({ bucket, region });
  ...
  it('constructURL: returns /media-store/{key} unchanged from the storage key', () => {
