/**
 * /freestyle/tricks?view=modifier — By modifier browse view tests.
 *
 * Pins the behavior:
 ...
 *   3. `?view=sets` is not a supported view value and falls through to the
 *      default ADD view, with no alias and no redirect.
...
describe('/freestyle/tricks?view=modifier — findability of representative ecosystem tricks', () => {
  ...
  it('cluster jump nav surfaces the clusters', async () => {
    const res = await request(await createApp()).get('/freestyle/tricks?view=modifier');
    expect(res.text).toContain('class="modifier-view-jump"');
...
describe('/freestyle/sets — Set Encyclopedia remains separate', () => {
  ...
  it('/freestyle/sets does NOT include the By modifier browse-view markup', async () => {
    ...
    expect(res.text).not.toContain('class="modifier-view-jump"');
  });
});
...
describe('?view=sets is not a supported browse value', () => {
  ...
  it('no rendered page emits a ?view=sets link', async () => {
    for (const path of ['/freestyle', '/freestyle/tricks', '/freestyle/tricks?view=modifier',
                        '/freestyle/tricks?family=mirage', '/freestyle/sets', '/freestyle/sets/pixie',
                        '/freestyle/about', '/freestyle/glossary']) {
      const res = await request(await createApp()).get(path);
      expect(res.status, path).toBe(200);
      // Handlebars escapes `=` to `&#x3D;` inside interpolated href values, so
      // check both spellings of the query string.
      expect(res.text, `${path} emits a retired ?view=sets link`).not.toMatch(/view(?:=|&#x3D;)sets/);
    }
  });
});

describe('the former Set Hub controls land on the Set Encyclopedia', () => {
  it('/freestyle/about links to /freestyle/sets under a truthful label', async () => {
    const res = await request(await createApp()).get('/freestyle/about');
    expect(res.text).toMatch(/<a href="\/freestyle\/sets" class="btn btn-outline">Set Encyclopedia<\/a>/);
    expect(res.text).not.toMatch(/Set Hub/);
  });

  it('a set detail page links back to /freestyle/sets under a truthful label', async () => {
