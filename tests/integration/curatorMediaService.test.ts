...
describe('curatorMediaService.updateGallery', () => {
  it('FH-owned: writes DB metadata + tag sets AND writes a JSON sidecar at /curated/galleries/<slug>.json', async () => {
    ...
    try {
      ...
      // FH-owned galleries are topic-defined: criteria are stored exactly as
      // given, with no `#curated` auto-prepend. A curated collection lists
      // `#curated` in its own criteria; a topic gallery omits it and shows all
      // matching content with `#curated` as the opt-in.
