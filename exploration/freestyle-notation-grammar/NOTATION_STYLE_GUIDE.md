...
| Atom-layer style (§13.9)               | Descriptive lowercase form for rows where `base_trick == slug`; `[set]` placeholder; `ss`/`op` short-form sides; `hippy`/`leggy`/`spin` descriptive modifiers; no `[DEL]` at terminal (RATIFIED 2026-05-15) |
...
### 13.9 Atom-layer descriptive style

**Status: RATIFIED 2026-05-15** for foundational/atom rows in `operational_notation`. Permanent for the atom layer. Coexists with §13.2–§13.8 (compound-layer style); does not replace them.

#### Scope

The atom-layer style applies to rows where `base_trick == slug` — i.e., foundational tricks that anchor a family (clipper, mirage, butterfly, etc.). Compound rows (rows where `base_trick != slug`) use the compound-layer style of §13.2–§13.8.

A row is **either** atom-layer or compound-layer; mixing styles within a single row is prohibited.

#### Conventions

Atom-layer notation reads as a **movement-language sentence**, not a parser-serialized token sequence. The full register:

- **All tokens lowercase.** (`toe`, `clipper`, `op`, `ss`, `hippy`, `leggy`, `dex`, `spin`, `in`, `out`)
- **`[set]` placeholder** permitted at the start when the atom's canonical execution leaves the set flexible (mirage, butterfly, etc.). When the canonical form commits to a specific plant, use that plant unbracketed (`toe >` for ATW, orbit).
- **`ss` / `op`** short-form side markers (same-side / opposite-side).
- **Descriptive movement modifiers** unbracketed before `dex`:
  - `hippy` — hip-led dex
  - `leggy` — knee-led dex
  - `spin` — body-rotation modifier preceding the terminal surface
- **`dex`** as a plain word (no `[DEX]` brackets).
- **Inline qualifiers** in lowercase parens: `(xbd)`, `(no plant)`, etc. — used as descriptive markers, not parser-bound tokens.
- **Sequence operator** `>` only. `>>` is a compound-layer device; atoms don't carry beat-group transitions.
- **No `[DEL]` flag** at the terminal surface — the terminal `> ss toe` or `> op clipper` implicitly carries the delay.

#### Goals

- Compact: 3–8 tokens per row
- Pedagogically readable: each row reads as a movement sentence
- Topologically unique: each atom's symbolic form distinguishes it from every other atom
- Restraint-first: descriptive modifiers (hippy / leggy / spin) are **descriptive symbolic vocabulary**, not parser-bound tokens. The goal is collision avoidance + readability, not mechanical encoding.

#### Examples (curator-authoritative, 2026-05-15)

| Slug | Atom-layer notation |
|---|---|
| `toe-stall` | `[set] > toe` |
...
| `osis` | `[set] > spin > ss clipper` |
| `around-the-world` | `toe > ss leggy in dex > ss toe` |
| `orbit` | `toe > ss leggy out dex > ss toe` (pending dictionary row) |

#### Render-side note

The existing `op-token--*` CSS classes apply to compound-layer notation (UPPERCASE + bracketed tokens). Atom-layer notation renders as plain monospace text by design — the sentence-like style does not require role-coloring to scan. This is a feature: atom rows read as sentences; compound rows read as structured patterns. Both styles share the registry-mode browse surface without competing.

#### Permanence

§13.9 is a **permanent ratification** for the atom layer. Future foundational rows added to the dictionary follow this style; future edits to existing atom rows preserve this style. Re-ratification required for any future stylistic change.
