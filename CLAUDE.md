...
- For read-only exploration prefer the Grep/Glob/Read tools; they never require permission. Read-only Bash inspection, including pipelines and loops, is permitted and should not prompt. Reserve the single-command preference for state-changing commands, where each subcommand is approved separately. Never lead a Bash command with `cd` unless another directory is truly required: the working directory already persists at the repo root, so `cd` is redundant, and a leading `cd` plus any output redirect (even `2>/dev/null`) trips a built-in approval prompt no repo rule can suppress. Use absolute paths, or a tool's own directory flag (`git -C`, `find <dir>`, `make -C`) when needed.
