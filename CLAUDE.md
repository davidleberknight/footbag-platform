...
Verification defaults: confirm what success looks like for the task, prefer route/integration verification first, and verify with `npm run build` plus only the test files the change reaches — the changed module's suite and its importers' suites, named explicitly (`npx vitest run tests/...`). Run the whole suite only at a commit, PR, or deploy gate, when shared test scaffolding or a high-stakes or widely-imported module changed, or when asked (`npm test`; `./run_all_tests.sh` when the human asks for all tests). Doc-only or comment-only changes are verified by re-reading, not `npm test`/`npm run build`.
