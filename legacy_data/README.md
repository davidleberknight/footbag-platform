...
| `inputs/identity_lock/*.csv` | committed (the version-free files, plus the two oldest numbered snapshots; every later `Persons_Truth_Final_v*` / `Placements_ByPerson_v*` snapshot is gitignored, so a patch tool reads the version-free file rather than its numbered predecessor) | `pipeline/identity/build_name_variants.py` | frozen identity lock (patch-toolchain only) |
