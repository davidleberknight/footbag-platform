...
def build_player_stats(wb: Workbook, stats: dict, persons: dict) -> None:
    ...
    for pid, p in sorted(persons.items(),
        ...
        if not p.get("person_name"):
            continue
        s   = stats.get(pid, {})
        # H1 — VISIBLE_PERSON parity: this workbook reads from
        # event_results/canonical_input/persons.csv which has already
        # been filtered by export_canonical_platform.py's VISIBLE_PERSON
        # rule (referenced-by-participant OR in display-name map OR
        # member_id OR BAP OR HOF). No additional filter is applied
        # here; doing so would break parity with the platform DB.
