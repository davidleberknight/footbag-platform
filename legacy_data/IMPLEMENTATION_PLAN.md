...
- **Freestyle pages fixes (James-track: curator content).**
  1. Content (curator): Glossary §1 vocabulary-stabilization claim lacks an inline citation; add one if corpus-backed.
  2. Content (curator): the glossary's closing passage carries a broad nine-name thanks-roll; other freestyle pages attribute contributors only contextually (Ben Job's notation on `jobs-notation-article.hbs` + `compositional-sets.hbs`; Chris Holden's curated list under compositional-sets' "Sources & attribution"), not as a thanks-roll. Decide whether the thanks-roll stays or aligns to the contextual-attribution pattern (some named are living people connected to the project).
