...
- **MIGRATION_PLAN §9 cross-track: Phase H cutover hardening.** Locked: loaders default to `'pending'` (4ca0909); Phase H is sole creator of live `clubs` at cutover in prod (3cc3a97); Phase G no longer writes `mapped_club_id` (Phase H owns the linkage); Phase H stamps `mapped_club_id` for every candidate with matching clubs row, not just `bootstrap_eligible=1`. Remaining gaps land as one PR:
  ...
  2. **Phase H zero-candidate semantics (locked 2026-05-17).** `06_cutover_pre_populated_clubs.py` distinguishes three zero-candidate states. **(a) Empty `legacy_club_candidates` table** (line 165): ERROR + `sys.exit(1)`. Phase G enrichment loader didn't run. **(b) Non-dormant classifications exist but `bootstrap_eligible_count == 0`** (line 209): ERROR + `sys.exit(1)`. The §9.1 classifier ran and silently regressed; downstream wiring would land in a degenerate empty state. **(c) Every candidate carries the seed-default `classification='dormant'`** (line 193): `return 0` with stderr skip notice. Phase G's classifier output CSV (`clubs/out/legacy_club_candidates.csv`) is legitimately absent in sparse local mirrors and CI smoke fixtures; this is the intentional CI-tolerance path added 2026-05-17 to keep `reset-local-db.sh` and the smoke gate working when the classifier CSV is not produced upstream. NOT a loophole — do not tighten without first updating the CI fixture stager (`scripts/ci/stage_loader_smoke_fixtures.sh`) to ship a classifier CSV. The fail-fast in (a) and (b) is the load-bearing regression detector; (c) is intentional and stable.
