...
- **Force-keep / force-junk request handling (cross-track, blocked).** The only open cross-track piece of the `A_Periodic_Club_Cleanup` contract: admin force-keep / force-junk requests (add to force-keep from a junk item; apply / modify / reject a request). The consuming surface is the `legacy_data/overrides/` CSVs the classifier reads, so an export contract with James (format, apply procedure) must exist before any platform-side request table is built. The rest of the candidate contract (demote / archive / defer / junk handling, badge, claim markers, sort / filter) is live; the platform-side remainder (wizard-flag grouping, bulk actions) is tracked in the root `IMPLEMENTATION_PLAN.md`.
