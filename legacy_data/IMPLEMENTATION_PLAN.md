...
- **Pipeline run-mode provenance manifest (proposal; coordinates with the root IP State-4 runbook entry).** `run_pipeline.sh` persists no run-mode marker, so the pre-cutover checklist cannot assert the loaded data came from a full-mode run. Proposal: write `out/pipeline_run_manifest.json` (mode + timestamp) at run end; the checklist gains a gate that reads it and fails on anything but `full`.
