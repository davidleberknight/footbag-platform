...
- **Educational card completion for the remaining first-class families (James-track curator content).** Every first-class family should ultimately have a dedicated educational family card; some remain uncarded because their content has not yet been authored. The 18-family alignment pass renders all 18 in the roster (from `freestylePublicFamilies.ts`), but 9 still lack a rich `freestyleGlossaryFamilyCards.ts` card: eclipse, dada-curve, barfly, dyno, paradon, double-over-down, flurry, flail, butterfly-swirl. Author a `GlossaryFamilyCard` for each (canonicalFormula, familyAnchorAdds, commonDescendants, siblingFamilies, notableCompounds, optional observational notes) — curator-authored doctrine, never fabricated. Touch point: `src/content/freestyleGlossaryFamilyCards.ts`.
