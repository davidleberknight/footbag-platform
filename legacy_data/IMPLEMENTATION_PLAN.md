...
- **Trick-dictionary loaders assume fresh-DB precondition.** Scripts `17_load_trick_dictionary.py`, `18_scrape_footbag_org_moves.py`, `19_load_red_additions.py`, `20_link_footbag_org_sources.py`, and one block in `07_build_mvfp_seed_full.py` (all from commit `bbc3dd7f`, 2026-04-11) carry two latent defects masked by `scripts/reset-local-db.sh`'s `rm -f "${DB_FILE}"` step:
  1. **Off-by-one REPO_ROOT.** `SCRIPT_DIR.parents[3]` resolves to one level above repo root (e.g. `/home/james/projects/database/footbag.db`). Default `--db` path therefore points at a non-existent file. The five later media-pipeline scripts (`21–23`) and `21_load_footbag_org_pending_tricks.py` correctly use `parents[2]`. Standardize all to `parents[2]`.
  2. **Parent-table DELETE before child-table cleanup in `17_load_trick_dictionary.py:131`.** Unscoped `DELETE FROM freestyle_tricks` runs before the script's own scoped DELETEs of `freestyle_trick_aliases` (line 292) and `freestyle_trick_source_links` (line 209). With FK enforcement on (line 322 sets `PRAGMA foreign_keys = ON`), this fails on any populated DB because `freestyle_trick_aliases.trick_slug`, `freestyle_trick_source_links.trick_slug`, `freestyle_trick_modifier_links.trick_slug`, and `freestyle_trick_relations.{from,to}_trick_slug` all reference `freestyle_tricks(slug)` without `ON DELETE CASCADE`. Pattern to follow: `21_load_footbag_org_pending_tricks.py:159–178` does children-first scoped DELETEs in correct order.

  Both defects fail standalone or mid-cycle invocations (e.g. an incremental alias add). Companion: a smoke test that runs each loader twice in sequence against the same DB to catch idempotency / FK-ordering regressions.

  **Forward invariant:** All loaders must be safe on non-empty DBs (no reliance on external DB reset).
