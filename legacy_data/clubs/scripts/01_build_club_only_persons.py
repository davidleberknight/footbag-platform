...
def main() -> None:
    ...
    # --- Filter club-only (unmatched) ---
    club_only = df[df["match_status"] == "NO_MATCH"].copy()

    if club_only.empty:
        print("No club-only persons found.")
        club_only.to_csv(OUT_CSV, index=False)
        return
    ...
    # --- Deduplicate (club + name) ---
    club_only = club_only.sort_values(
        by=["club_key", "person_name_norm", "affiliation_confidence_score"],
    ...
    out.rename(
        columns={
            "affiliation_confidence_score": "confidence",
        },
        inplace=True,
    )
