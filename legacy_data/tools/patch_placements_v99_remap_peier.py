#!/usr/bin/env python3
"""
patch_placements_v99_remap_peier.py

Companion to patch_pt_v55_consolidate_peier.py. Advances
Placements_ByPerson from v98 to v99 by remapping every reference to
0b36838e-ef53-5c81-adf5-5dda95b42e84 (Andreas Grandi Peier) onto
1df65a5e-fdb5-5385-a31e-4ac28b916b28 (Andreas Peier).

Expected v98 → v99 diff (pre-verified against v98):
  - 3 solo rows remapped (single event 1222687523, 2008 Open de France)
  - 0 team rows touch the doomed pid
  - total row count unchanged (27,970)
  - post-run `grep 0b36838e` in v99 must return 0

Usage (from legacy_data/):
    .venv/bin/python tools/patch_placements_v99_remap_peier.py
"""
...
PB_IN  = ROOT / "inputs" / "identity_lock" / "Placements_ByPerson_v98.csv"
PB_OUT = ROOT / "inputs" / "identity_lock" / "Placements_ByPerson_v99.csv"

DOOMED_PID   = "0b36838e-ef53-5c81-adf5-5dda95b42e84"
SURVIVOR_PID = "1df65a5e-fdb5-5385-a31e-4ac28b916b28"
SURVIVOR_DISPLAY = "Andreas Peier"
SURVIVOR_NORM    = "andreas peier"
...
def main() -> None:
    ...
    print(f"v98 rows: {len(rows)}  v99 rows: {len(out_rows)}  delta: 0")
