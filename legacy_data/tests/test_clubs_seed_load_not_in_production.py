...
cutover gating, which admits only the pre_populate class. It survives as a dev-convenience
...
def test_load_clubs_seed_not_invoked_in_production_pipeline():
    ...
    executable position. The bulk-load defeats Phase H gating, which admits
    only the pre_populate class.
    """
    ...
    assert hits == [], (
        ...
        "cutover gating. The production pipeline must "
...
def test_phase_h_still_invoked_in_production_pipeline():
    ...
    gated to the pre_populate-class candidates."""
    text = RUN_PIPELINE.read_text()
